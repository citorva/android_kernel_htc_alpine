...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
define_int_show(lwa_gear, lwa_gear_val);
define_int_store(lwa_gear, lwa_gear_val, null_cb);
power_attr(lwa_gear);
#endif
...
int pnpmgr_lwa_gear_set(int val) {
	if (lwa_gear_val != val) {
		lwa_gear_val = val;
		sysfs_notify(apps_kobj, NULL, "lwa_gear");
	}
	return 0;
};
EXPORT_SYMBOL(pnpmgr_lwa_gear_set);

int pnpmgr_lwa_gear_get(void) {
	return lwa_gear_val;
};
EXPORT_SYMBOL(pnpmgr_lwa_gear_get);
#endif

int screenoff_enabled = 0;
static ssize_t
screenoff_show(struct kobject *kobj, struct kobj_attribute *attr,
                char *buf)
{
	return sprintf(buf, "%d\n", screenoff_enabled);
}
...
static ssize_t
screenoff_store(struct kobject *kobj, struct kobj_attribute *attr,
                const char *buf, size_t n)
{
	unsigned long val;
	...
	printk(KERN_INFO "Change screenoff attr from %d to %ld\n", screenoff_enabled, val);
	screenoff_enabled = val;
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
	if ( val == 1 ) {
		if (lwa_gear_val != 0 ) {
			pnpmgr_lwa_gear_set(0);
		}
	}
#endif
	sysfs_notify(kobj, NULL, "screenoff");
	return n;
}
power_attr(screenoff);


int notify_screen(int val)
{
        if (val != screenoff_enabled) {
		screenoff_enabled = val;
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
        if ( val == 1 ) {
            if (lwa_gear_val != 0 ) {
...
#endif
                sysfs_notify(pnpmgr_kobj, NULL, "screenoff");
        }
