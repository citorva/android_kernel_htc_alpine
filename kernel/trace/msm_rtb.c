...
extern void mrdump_mini_add_entry_ex(unsigned long addr, unsigned long size, unsigned long paddr);
void msm_rtb_add_elf_entries(void) {

#if defined(CONFIG_MSM_RTB_SEPARATE_CPUS)
	unsigned int cpu;
	for_each_possible_cpu(cpu) {
		mrdump_mini_add_entry((unsigned long)&per_cpu(msm_rtb_idx_cpu, cpu), sizeof(msm_rtb_idx_cpu));
	}
#else
	mrdump_mini_add_entry((unsigned long)&msm_rtb_idx, sizeof(msm_rtb_idx));
#endif

	mrdump_mini_add_entry_ex((unsigned long)msm_rtb.rtb, (unsigned long)msm_rtb.size, (unsigned long)msm_rtb.phys);

	mrdump_mini_add_entry((unsigned long)&msm_rtb, sizeof(msm_rtb));
}
EXPORT_SYMBOL(msm_rtb_add_elf_entries);
