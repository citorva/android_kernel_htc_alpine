...
static int htc_print_wakeup_source_stats(struct wakeup_source *ws)
{
	unsigned long flags;
	ktime_t total_time;
	ktime_t max_time;
...
void htc_show_wakeup_sources(void)
{
	struct wakeup_source *ws;
	...
	rcu_read_lock();
	list_for_each_entry_rcu(ws, &wakeup_sources, entry)
		htc_print_wakeup_source_stats(ws);
	rcu_read_unlock();
}
