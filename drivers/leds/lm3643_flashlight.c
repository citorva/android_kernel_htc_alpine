...
#define FLT_DBG_LOG(fmt, ...) \
		printk(KERN_ERR "[FLT_FR]LM3643 " fmt, ##__VA_ARGS__)
...
typedef struct ioexp_gpio_config {  
	struct pinctrl *pinctrl;
	struct pinctrl_state *pins_hwen_ioexp_gpio_high;
	struct pinctrl_state *pins_hwen_ioexp_gpio_low;
}ioexp_gpio_config;

ioexp_gpio_config g_ioexp_gpio;
...
static int flashlight_turn_off(void)
{
	...
	if (this_lm3643->hwen > 0)
		gpio_set_value_cansleep(this_lm3643->hwen, 0);

	if (this_lm3643->hwen_ioexp > 0)
		gpio_set_value(this_lm3643->hwen_ioexp, 0);
...
int lm3643_flashlight_flash(int led1, int led2)
{
	...
	if (led1 == 0 && led2== 0)
		...
		if (this_lm3643->hwen_ioexp > 0)
			gpio_set_value(this_lm3643->hwen_ioexp, 1);
...
static int lm3643_parse_dt(struct device *dev, struct LM3643_flashlight_platform_data *pdata)
{
	...
	g_ioexp_gpio.pinctrl = devm_pinctrl_get(dev);
	if (IS_ERR(g_ioexp_gpio.pinctrl)) {
		g_ioexp_gpio.pinctrl = (struct pinctrl *)NULL;
		PTR_ERR(g_ioexp_gpio.pinctrl);
		FLT_ERR_LOG("Cannot find htc mcu pinctrl!\n");
	}
	...
	if (prop) {
		...
		pdata->lm3643_hwen = 0;
		pdata->lm3643_hwen_ioexp = of_get_named_gpio(dt, "lm3643,lm3643_hwen_ioexp", 0);

		g_ioexp_gpio.pins_hwen_ioexp_gpio_high = pinctrl_lookup_state(g_ioexp_gpio.pinctrl, "hwen_ioexp_gpio_active");
	    if (IS_ERR(g_ioexp_gpio.pins_hwen_ioexp_gpio_high)) {
			g_ioexp_gpio.pins_hwen_ioexp_gpio_high = (struct pinctrl_state *)NULL;
			PTR_ERR(g_ioexp_gpio.pins_hwen_ioexp_gpio_high);
			FLT_ERR_LOG("Cannot find htc mcu pinctrl pins_hwen_ioexp_gpio_high!\n");
		}

		g_ioexp_gpio.pins_hwen_ioexp_gpio_low = pinctrl_lookup_state(g_ioexp_gpio.pinctrl, "hwen_ioexp_gpio_inactive");
	    if (IS_ERR(g_ioexp_gpio.pins_hwen_ioexp_gpio_low)) {
			g_ioexp_gpio.pins_hwen_ioexp_gpio_low = (struct pinctrl_state *)NULL;
			PTR_ERR(g_ioexp_gpio.pins_hwen_ioexp_gpio_low);
			FLT_ERR_LOG("Cannot find htc mcu pinctrl pins_hwen_ioexp_gpio_low!\n");
		}
	}

	prop = of_property_read_u32(dt, "lm3643,lm3643_strobe", &pdata->lm3643_strobe);
	if (prop) {
		printk("Looking up %s property in node %s failed %d, value %d\n",
			"lm3643_strobe", dt->full_name, prop, pdata->lm3643_strobe);
	}else{
		printk("pdata->lm3643_strobe = %d\n", pdata->lm3643_strobe);
	}

	prop = of_property_read_u32(dt, "lm3643,lm3643_torch", &pdata->lm3643_torch);
	if (prop) {
		printk("Looking up %s property in node %s failed %d, value %d\n",
			"lm3643,lm3643_torch", dt->full_name, prop, pdata->lm3643_torch);
	}else{
		printk("pdata->lm3643_torch = %d\n", pdata->lm3643_torch);
	}
					...
					const struct i2c_device_id *id)
{
	...
	htc_flash_main		   = &lm3643_flashlight_flash;
	htc_torch_main		   = &lm3643_flashlight_torch;
#endif
	...
	if(g_ioexp_gpio.pins_hwen_ioexp_gpio_high != NULL) {
		pinctrl_select_state(g_ioexp_gpio.pinctrl, g_ioexp_gpio.pins_hwen_ioexp_gpio_high);
	}

	if (lm3643->hwen_ioexp) {
		ret = gpio_request(lm3643->hwen_ioexp, "flashlight_hwen_ioexp");
		if (ret) {
			FLT_ERR_LOG("%s: unable to request hwen gpio %d (%d)\n",
				__func__, lm3643->hwen, ret);
			return ret;
		}

		ret = gpio_direction_output(lm3643->hwen_ioexp, 0);
		if (ret) {
			FLT_ERR_LOG("%s: Unable to set direction\n", __func__);
			return ret;
		}
	...
	if (lm3643->strobe) {
		...
		if (ret) {
			FLT_ERR_LOG("%s: unable to request strobe gpio %d (%d)\n",
	...
	if (lm3643->torch) {
		...
		if (ret) {
			FLT_ERR_LOG("%s: unable to request torch gpio %d (%d)\n",
...
{
	int rec = 0;
	rec = i2c_add_driver(&lm3643_driver);
	printk("[FLT] %s Enter, rec = %d\n", __func__,rec);
	return rec;
}

static void __exit lm3643_exit(void)
{
	i2c_del_driver(&lm3643_driver);
}

late_initcall(lm3643_init);
