...
extern int suspend_highfreq_check_reason;
#endif 
...
#else
signed int custom_aging_factor = 0;
#endif
...
signed int dodinit_columb = 0;
signed int shutdown_columb = 0;
...
signed int volt_mode_update_time_out = 6;	
...
#define SUSPEND_CURRENT_CHECK_THRESHOLD 500	
#endif
...
struct battery_meter_table_custom_data {
	...
	int battery_profile_t4_size;
	BATTERY_PROFILE_STRUCT battery_profile_t4[100];
	...
	int r_profile_t4_size;
	R_PROFILE_STRUCT r_profile_t4[100];
...
void fgauge_get_profile_id(void)
{
	int id_volt = 0;
	int id = 0;
	id_volt = PMIC_IMM_GetOneChannelValue(PMIC_AUX_TSX, 5, 1);
...
int __batt_meter_init_cust_data_from_cust_header(struct platform_device *dev)
{
	...
	batt_meter_table_cust_data.battery_profile_t4_size =
		sizeof(battery_profile_t4[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.battery_profile_t4,
			&battery_profile_t4[g_fg_battery_id],
			sizeof(battery_profile_t4[g_fg_battery_id]));
	...
	batt_meter_table_cust_data.r_profile_t4_size = sizeof(r_profile_t4[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.r_profile_t4,
			&r_profile_t4[g_fg_battery_id],
			sizeof(r_profile_t4[g_fg_battery_id]));
	...
	batt_meter_table_cust_data.battery_profile_t4_size = sizeof(battery_profile_t4)
	    / sizeof(BATTERY_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.battery_profile_t4,
	       &battery_profile_t4, sizeof(battery_profile_t4));
	...
	batt_meter_table_cust_data.r_profile_t4_size =
		sizeof(r_profile_t4) / sizeof(R_PROFILE_STRUCT);

	memcpy(&batt_meter_table_cust_data.r_profile_t4, &r_profile_t4, sizeof(r_profile_t4));
#endif
	...
	batt_meter_cust_data.temperature_t4 = TEMPERATURE_T4;
	...
	batt_meter_cust_data.q_max_neg_20 = g_Q_MAX_NEG_20[g_fg_battery_id];
	...
	batt_meter_cust_data.q_max_neg_20_h_current = g_Q_MAX_NEG_20_H_CURRENT[g_fg_battery_id];
#else
...
#endif
	batt_meter_cust_data.oam_d5 = OAM_D5;	
...
int __batt_meter_init_cust_data_from_dt(struct platform_device *dev)
{
	...
	if (of_property_read_u32(np, "temperature_t4", &val)) {
		batt_meter_cust_data.temperature_t4 = (int)val;
		bm_debug("Get temperature_t4: %d\n", batt_meter_cust_data.temperature_t4);
	} else {
	...
	if (of_property_read_u32(np, "q_max_neg_20", &val)) {
		batt_meter_cust_data.q_max_neg_20 = (int)val;
		bm_debug("Get q_max_neg_20: %d\n", batt_meter_cust_data.q_max_neg_20);
	} else {
		bm_err("Get q_max_neg_20 failed\n");
	}
	...
	if (of_property_read_u32(np, "q_max_neg_20_h_current", &val)) {
		batt_meter_cust_data.q_max_neg_20_h_current = (int)val;
		bm_debug("Get q_max_neg_20_h_current: %d\n",
			 batt_meter_cust_data.q_max_neg_20_h_current);
	} else {
		bm_err("Get q_max_neg_20_h_current failed\n");
	}
...
int force_get_tbat(kal_bool update)
{
	...
	if (bat_temperature_val > 55 && g_flag_keep_charge_on)
		return 55;
...
void fgauge_algo_run_get_init_data(void)
{
	...
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
	battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CAR, &dodinit_columb);
	bm_info
...
signed int get_dynamic_period(int first_use, int first_wakeup_time, int battery_capacity_level)
{
	...
	ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CAR_ACT, &car_instant);
		...
		if (suspend_highfreq_check_reason & SUSPEND_HIGHFREQ_CHECK_BIT_TALK)
			wake_up_smooth_time = 300; 
		else
			wake_up_smooth_time = NORMAL_WAKEUP_PERIOD;
#endif 
...
#ifdef CONFIG_HTC_BATT_PCN0011
	return (g_spm_timer - g_sleep_total_time.tv_sec);
#else
	return g_spm_timer;
#endif
#else
	...
	if (vbat_val > VBAT_NORMAL_WAKEUP) {
		...
		I_sleep = (((car_wakeup - car_sleep) * 3600) / last_time + 5) / 10;	
...
#else
	int ret;
	signed int ADC_I_SENSE = 1;	
	signed int ADC_BAT_SENSE = 1;	
	int ICharging = 0;
...
static int battery_meter_probe(struct platform_device *dev)
{
	...
	if (get_boot_mode() == LOW_POWER_OFF_CHARGING_BOOT
		...
		temp_strptr =
		    kzalloc(strlen(saved_command_line) + strlen(" androidboot.mode=charger") + 1,
			    GFP_KERNEL);
		strlcpy(temp_strptr, saved_command_line, strlen(saved_command_line));
	...
	if (get_kernel_flag() & KERNEL_FLAG_ENABLE_BMS_CHARGER_LOG)
		Enable_FGADC_LOG = BMLOG_TRACE_LEVEL;
	if (get_kernel_flag() & KERNEL_FLAG_ENABLE_GM20_DEBUG_LOG)
		gFG_daemon_log_level = 8;
#endif 
...
{
	int shutdown_rtc_soc = 0;
	int delta_columb = 0, pre_rtc_columb = 0;
	int ret = 0;

	bm_debug("[battery_meter_shutdown]\n");

	shutdown_rtc_soc = get_rtc_spare_fg_value();
	if (shutdown_rtc_soc == (100 - gFG_DOD0_init)) {
		ret = battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CAR, &shutdown_columb);
		delta_columb = dodinit_columb - shutdown_columb;
		pre_rtc_columb = get_rtc_spare_coulomb_value();
		bm_notice("[battery_meter_shutdown] wxx_pre_rtc_columb=%d,delta_columb=%d,capacity1=%d\n",
			pre_rtc_columb, delta_columb, gFG_BATT_CAPACITY_aging/100);
		if ((delta_columb + pre_rtc_columb) < 0) {
			bm_notice("[battery_meter_shutdown] shutdown_cc < 0(%d) reset coulomb rtc\n",
			(delta_columb + pre_rtc_columb));
			set_rtc_spare_coulomb_value(0);
		} else if (delta_columb + pre_rtc_columb < gFG_BATT_CAPACITY_aging/100) {
			set_rtc_spare_coulomb_value(delta_columb + pre_rtc_columb);
			bm_notice("[battery_meter_shutdown] wxx_set_rtc_delta_columb=%d\n",
				get_rtc_spare_coulomb_value());
		} else {
			set_rtc_spare_fg_value(shutdown_rtc_soc - 1);
			set_rtc_spare_coulomb_value(delta_columb + pre_rtc_columb - gFG_BATT_CAPACITY_aging/100);
			bm_notice("[battery_meter_shutdown] wxx_rtc_fg=%d,rtc_columb=%d\n",
				get_rtc_spare_fg_value(), get_rtc_spare_coulomb_value());
		}
	} else {
		bm_notice("[battery_meter_shutdown] shutdown_rtc_soc=%d gFG_DOD0_init=%d, reset coulomb rtc\n",
			shutdown_rtc_soc, gFG_DOD0_init);
		set_rtc_spare_coulomb_value(0);
	}
}
...
#endif
#ifdef CONFIG_HTC_BATT_PCN0011
	if (g_sleep_total_time.tv_sec < (wake_up_smooth_time - 10)) { 
		bm_info("[battery_meter_resume] Not reach batt wake up time\n");
		return 0;
	}
#endif 
	...
	case FG_DAEMON_CMD_SET_DOD0:
		{
			...
			bm_debug("[fg_res] gFG_DOD0 = %d\n", gFG_DOD0);
			if (gFG_DOD0_init == 0) {
				gFG_DOD0_init = gFG_DOD0;
				bm_notice("[fg_res] gFG_DOD0_init = %d\n", gFG_DOD0_init);
			}
		}
		break;
	...
	case FG_DAEMON_CMD_SET_UI_SOC2:
		{
			...
			htc_battery_info_update(POWER_SUPPLY_PROP_CAPACITY, BMT_status.UI_SOC2);
#endif
			bat_update_thread_wakeup();
...
#ifndef MTK_ENABLE_AGING_ALGORITHM
	case FG_DAEMON_CMD_GET_CUSTOM_AGING_FACTOR:
		{
			ret_msg->fgd_data_len += sizeof(custom_aging_factor);
			memcpy(ret_msg->fgd_data, &custom_aging_factor, sizeof(custom_aging_factor));
			bm_print(BM_LOG_CRTI, "[fg_res] custom_aging_factor = %d\n", custom_aging_factor);
		}
		break;
#endif
