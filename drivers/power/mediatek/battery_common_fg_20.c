...
extern bool g_test_power_monitor;
extern bool g_flag_pa_fake_batt_temp;
extern bool g_flag_disable_temp_protection;
#endif
...
struct timespec battery_duration_time[DURATION_NUM];	
unsigned int wake_up_smooth_time = 0;	
...
#if defined(CONFIG_HTC_BATT_WA_PCN0022) || defined(CONFIG_HTC_BATT_PCN0018)
int chg_error_force_update = 0;
#endif 
#ifdef CONFIG_HTC_BATT_PCN0011
static int spm_timeout_force_update = 0;
#endif 
...
#define Set_META_BAT_Chg _IOW('k', 21, int)
...
kal_bool g_cable_out_happened_in_aicl_checking = KAL_FALSE;

#ifdef CONFIG_HTC_BATT_WA_PCN0023
struct wake_lock weak_charger_wa_lock;
struct delayed_work weak_charger_wa_work;
#endif
int g_status_smb = POWER_SUPPLY_STATUS_NOT_CHARGING;
int g_capacity_smb = 50;
int g_present_smb = 0;
static int cmd_discharging = -1;
static int htc_disable_charger = -1;
...
static void mt_battery_update_EM(struct battery_data *bat_data)
{
	...
	bat_data->BAT_CAPACITY = htc_battery_level_adjust();
#else
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if (g_test_power_monitor) {
		bat_data->BAT_CAPACITY = POWER_MONITOR_BATT_CAPACITY;
		bat_data->BAT_batt_temp = POWER_MONITOR_BATT_TEMP;
	}
	if (g_flag_pa_fake_batt_temp || g_flag_keep_charge_on) {
		if (bat_data->BAT_batt_temp > 550)
			bat_data->BAT_batt_temp = 540;
		if (bat_data->BAT_batt_temp < 0)
			bat_data->BAT_batt_temp = 1;
	}
#endif 
#ifdef CONFIG_FPC_HTC_DISABLE_CHARGING
	if ((BMT_status.charger_exist == KAL_TRUE) &&
		(htc_battery_get_pwrsrc_discharging_reason() == HTC_BATT_PWRSRC_DIS_BIT_FP)) {
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_CHARGING;
	}
#endif 
#ifdef CONFIG_HTC_BATT_PCN0001
	if ((bat_data->BAT_CAPACITY == 100) && (BMT_status.charger_exist == KAL_TRUE))
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_FULL;
	...
	if (fgauge_get_batt_id() == ERR_BATT_ID)
		bat_data->BAT_STATUS = POWER_SUPPLY_STATUS_UNKNOWN;
#endif 

	htc_battery_info_update(POWER_SUPPLY_PROP_HEALTH, bat_data->BAT_HEALTH);
	...
	if (update_cnt == 6) {
		...
		update_cnt = 0;
	} else if (pre_uisoc2 != bat_data->BAT_CAPACITY) {
		
		power_supply_changed(bat_psy);
		pre_uisoc2 = bat_data->BAT_CAPACITY;
		update_cnt = 0;
		...
		cable_in_uevent = 0;
#if defined(CONFIG_HTC_BATT_WA_PCN0022) || defined(CONFIG_HTC_BATT_PCN0018)
	} else if (chg_error_force_update) {
		chg_error_force_update = 0;
		power_supply_changed(bat_psy);
		update_cnt = 0;
#endif 
#ifdef CONFIG_HTC_BATT_PCN0011
	} else if (spm_timeout_force_update) {
		spm_timeout_force_update = 0;
		power_supply_changed(bat_psy);
		update_cnt = 0;
#endif 
	} else {
		...
		if (BMT_status.charger_exist == KAL_TRUE) {
			if (previous_charger_exist == KAL_FALSE) {
				...
				if ((BMT_status.charger_type == STANDARD_CHARGER)
					...
					data = batt_cust_data.non_std_ac_charger_current / 100;	
				else	
...
int htc_charging_avg_voltage_check(void)
{
...
int htc_battery_get_AICL_status(void)
{
...
void mt_battery_GetBatteryData(void)
{
	...
	ICharging = ((battery_meter_get_battery_current_sign())?(-1):1)*(battery_meter_get_battery_current()/10);
	...
	battery_log(BAT_LOG_CRTI,
		    ...
		    SOC:%3d UI_SOC:%3d UI_SOC2:%3d Charging_state=0x%x\n",
		    BMT_status.bat_vol, bat_vol, BMT_status.ICharging, ICharging,
		    BMT_status.charger_vol, BMT_status.temperature, temperature, BMT_status.ZCV,
		    BMT_status.charger_type, BMT_status.SOC, BMT_status.UI_SOC, BMT_status.UI_SOC2,BMT_status.bat_charging_state);
}
...
#ifdef CONFIG_HTC_BATT_PCN0006
	if (g_flag_keep_charge_on || g_flag_disable_temp_protection)
		return PMU_STATUS_OK;
#endif 
...
static PMU_STATUS mt_battery_CheckChargingTime(void)
{
	...
	if ((g_battery_thermal_throttling_flag == 2) || (g_battery_thermal_throttling_flag == 3) || (!g_en_chrTimer)) {
		battery_log(BAT_LOG_FULL,
...
static void mt_battery_CheckBatteryStatus(void)
{
	...
	if (htc_disable_charger == 1) {
		BMT_status.bat_charging_state = CHR_ERROR;
		return;
	} else if(htc_disable_charger == 0){
		BMT_status.bat_charging_state = CHR_PRE;
		htc_disable_charger = -1;
	}
...
#ifdef HTC_ENABLE_AICL
		if(BMT_status.htc_acil_state > AICL_STOP){
...
#ifdef CONFIG_HTC_BATT_WA_PCN0023
static void weak_charger_wa_worker(struct work_struct *work)
{
	battery_log(BAT_LOG_CRTI,"[weak_charger_wa_worker]");
	mt_kpoc_power_off_check();
	wake_unlock(&weak_charger_wa_lock);
}
#endif
...
bool g_is_limit_IUSB = false;
int g_count_same_dischg = 0;
void detect_identical_discharge_time(int charger_exist)
{
	long diff;
	unsigned long dischg_time_ms;
	unsigned long cur_jiffies;
	static int first = 1;
	static int prev_charger_exist = -1;
	...
	if ((charger_exist == 0) && (prev_charger_exist == 1)) {
		
		dischg_last_time = rtc_now;
		dischg_time_ms = 0;
		first = 0;
	} else if ((charger_exist == 1) && (prev_charger_exist == 0) && (first == 0)){
		...
		if ((diff < 1200) && (dischg_time_ms <= OCP_RELEASE_TIME_UPPER_BOUND_MS)) {
			if(++g_count_same_dischg == 3) {
				...
				battery_log(BAT_LOG_CRTI, "[detect_identical_discharge_time] Start to limit USB current\n");
			}
		} else {
			g_is_limit_IUSB = false;
			g_count_same_dischg = 0;
		}
		battery_log(BAT_LOG_CRTI,"[detect_identical_discharge_time] prev_dischg_time_ms(%lu) dischg_time_ms(%lu) "
		...
		battery_log(BAT_LOG_CRTI, "[detect_identical_discharge_time] prev_charger_exist(%d) "
				"charger_exist(%d) \n", prev_charger_exist, charger_exist);
...
void htc_charger_reset(void)
{
	kal_bool unplug_charger;
	CHR_CURRENT_ENUM chg_current;

	if (BMT_status.charger_exist == KAL_FALSE) {
		unplug_charger = KAL_TRUE;
		
		battery_charging_control(CHARGING_CMD_HIZ_EN_CTRL, &unplug_charger);

		
		chg_current = CHARGE_CURRENT_500_00_MA;
		battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &chg_current);
	} else {
		
		if (BMT_status.charger_type == STANDARD_HOST)
			chg_current = CHARGE_CURRENT_500_00_MA;
		else
			chg_current = CHARGE_CURRENT_1000_00_MA;

		battery_charging_control(CHARGING_CMD_SET_CURRENT, &chg_current);
		battery_charging_control(CHARGING_CMD_SET_INPUT_CURRENT, &chg_current);
	}
}

#ifdef CONFIG_HTC_BATT_PCN0017
void htc_redetect_charger(void)
{
	int pre_charger_type = BMT_status.charger_type;

	mt_charger_type_detection();
	if (pre_charger_type != BMT_status.charger_type) {
		battery_log(BAT_LOG_CRTI, "[htc_redetect_charger] pre:%d, new:%d\n",
									pre_charger_type, BMT_status.charger_type);
		htc_battery_info_update(POWER_SUPPLY_PROP_CHARGE_TYPE, BMT_status.charger_type);
		if ((BMT_status.charger_type == STANDARD_HOST)
		    || (BMT_status.charger_type == CHARGING_HOST)) {
			mt_usb_connect();
		}
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_FALSE;
#ifdef HTC_ENABLE_AICL
		battery_log(BAT_LOG_CRTI, "[do_chrdet_int_task] aicl_state=%d\n",BMT_status.htc_acil_state);
			g_cable_out_happened_in_aicl_checking = KAL_FALSE;
			if(BMT_status.htc_acil_state == AICL_CHECKING)
				g_cable_out_happened_in_aicl_checking = KAL_TRUE;
#endif
#if defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
			battery_log(BAT_LOG_CRTI,
			...
			if(g_cable_out_happened_in_aicl_checking){
				battery_log(BAT_LOG_CRTI,
					    "[do_chrdet_int_task] hold weak_charger_wa_lock\n");
				wake_lock(&weak_charger_wa_lock);
				schedule_delayed_work(&weak_charger_wa_work, msecs_to_jiffies(1000));
			}else{
				#ifdef HTC_ENABLE_AICL
				BMT_status.htc_acil_state = 0;
				#endif
...
#ifdef CONFIG_HTC_BATT_PCN0010
		detect_identical_discharge_time(BMT_status.charger_exist);
#endif
		cable_in_uevent = 1;
...
#define LIMITED_POWER_STAGE_ONE_POWER  1799
...
void htc_battery_limited_power(void)
{
	...
	if (temperature <= LIMITED_POWER_STAGE_TWO_TEMPERATURE) {
		mt_ppm_cpu_thermal_protect(LIMITED_POWER_STAGE_TWO_POWER);
		batt_low_temp_protect_en = 1;
	} else if (temperature <= LIMITED_POWER_STAGE_ONE_TEMPERATURE) {
		mt_ppm_cpu_thermal_protect(LIMITED_POWER_STAGE_ONE_POWER);
		batt_low_temp_protect_en = 1;
...
static long adc_cali_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case Set_META_BAT_Chg:
		...
		htc_set_pwr_en(meta_charging_enable);
		wake_up_bat();
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
...
#ifdef CONFIG_HTC_BATT_WA_PCN0022
	static int pre_ovp = 0;
	bool ovp_first_check = false;
	int ovp_state = 0;
#endif 
	...
	do {
		...
		if(hv_status == KAL_TRUE) {
			...
			ovp_state = mt_get_gpio_in_base(OVP_GPIO);
			if (ovp_state && (BMT_status.charger_exist == KAL_FALSE) && mt_usb_is_device()) {
				if (!ovp_first_check) 
					ovp_first_check = true;
				else
					BMT_status.charger_protect_status = charger_OVER_VOL;
			} else {
				BMT_status.charger_protect_status = 0;
				ovp_first_check = false;
			}
		}
		if (pre_ovp != BMT_status.charger_protect_status) {
			chg_error_force_update = 1;
			bat_update_thread_wakeup();
			pre_ovp = BMT_status.charger_protect_status;
		}
#endif 
		if (chargin_hw_init_done)
...
#endif
	pmic_config_interface(MT6351_FGADC_CON14, 0x1, 0x1, 0);
...
static void battery_timer_resume(void)
{
	...
	pmic_config_interface(MT6351_FGADC_CON14, 0x0, 0x1, 0);
	mutex_unlock(&bat_mutex);
...
static int battery_remove(struct platform_device *dev)
{
	...
	if(node){
		if(of_property_read_u32(node, "batt_ovp", &ret) == 0) {
			gpio_free(ret);
		}
		if(of_property_read_u32(node, "batt_chgin", &ret) == 0) {
			gpio_free(ret);
		}
		if(of_property_read_u32(node, "batt_chgen", &ret) == 0) {
			gpio_free(ret);
		}
		if(of_property_read_u32(node, "batt_chgeint", &ret) == 0) {
			gpio_free(ret);
		}
	}
	#endif
...
void htc_set_chg_en(int en)
{
	...
	if (en == 0) {
		...
		htc_disable_charger = 0;
		charging_enable = true;
		adjust_power = -1;
		pre_en = en;
	} else {
		battery_log(BAT_LOG_CRTI, "[htc_set_chg_en] unknown setting (%d)\n", en);
		return;
	}

	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);

	battery_log(BAT_LOG_CRTI, "[htc_set_chg_en] cmd_discharging=%d\n", en);
}

void htc_set_pwr_en(int en)
{
	unsigned int charging_enable = false;
	static int pre_en = -1;
	...
	if (en == 0) {
		...
		battery_log(BAT_LOG_CRTI, "[htc_set_pwr_en] unknown setting (%d)\n", en);
		return;
	}
	...
	battery_log(BAT_LOG_CRTI, "[htc_set_pwr_en] en=%d\n", en);
}

void htc_safety_timer_en(bool en)
{
	g_en_chrTimer = en;
}

int is_over_vchg(void)
{
    return (BMT_status.charger_protect_status == charger_OVER_VOL)?1:0;
}
...
int htc_charger_get_attr_text(char *buf, int size)
{
    ...
    dump_reg.size = size - len;

    battery_charging_control(CHARGING_CMD_HTC_DUMP_REGISTER, (void*)&dump_reg);
    len += dump_reg.size;
...
{
    *result = (BMT_status.bat_charging_state == CHR_BATFULL)?1:0;
    return 0;
}
...
static int battery_pm_suspend(struct device *device)
{
	...
	battery_log(BAT_LOG_CRTI,"[%s]cancel battery timer\n",__FUNCTION__);
...
static int battery_pm_resume(struct device *device)
{
	...
	struct platform_device *pdev = to_platform_device(device);
	ktime = ktime_set(BAT_TASK_PERIOD, 0);	
	battery_log(BAT_LOG_CRTI,"[%s]restart battery timer\n",__FUNCTION__);
	hrtimer_start(&battery_kthread_timer, ktime, HRTIMER_MODE_REL);
	BUG_ON(pdev == NULL);
	return ret;
}
...
static int battery_dts_probe(struct platform_device *dev)
{
	...
	if (IS_ERR_OR_NULL(pinctrl)) {
		...
		gpio_state_init = pinctrl_lookup_state(pinctrl, "batt_gpio_set");
		if (IS_ERR_OR_NULL(gpio_state_init)) {
			pr_err("%s: Cannot get pintctrl state\n", __func__);
			ret = PTR_ERR(gpio_state_init);
			pinctrl = NULL;
		} else {
			ret = pinctrl_select_state(pinctrl, gpio_state_init);
			if (ret) {
				pr_err("%s: Cannot init INT gpio\n", __func__);
			}
		}
	}
...
#ifdef CONFIG_HTC_BATT_WA_PCN0023
	wake_lock_init(&weak_charger_wa_lock, WAKE_LOCK_SUSPEND, "weak charger wa wakelock");
	INIT_DELAYED_WORK(&weak_charger_wa_work, weak_charger_wa_worker);
#endif
