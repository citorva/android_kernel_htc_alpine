...
unsigned int mt_bio_klog_enable = 1;
...
static void mt_bio_throughput_rw_eval(struct mt_bio_throughput_rw *rw)
{
	...
	do_div(usage, 1000000); 

	if (usage && rw->size) {
		rw->speed = (rw->size << SECTOR_SHIFT) / (__u32)usage;  
		rw->speed = (rw->speed*1000) >> 10; 
		rw->usage = usage;
	} else {
		rw->speed = 0;
		rw->size = 0;
...
static const char *mt_pr_speed(char *out, int size, __u64 usage, __u32 sectors)
{
	...
	do_div(usage, 1000); 
	speed = 1000 * (sectors << SECTOR_SHIFT) / (__u32)usage;  
	speed = (speed*1000) >> 10; 
...
void mt_biolog_cmdq_isdone_end(unsigned int task_id)
{
	...
	rw = tsk->arg & (1<<30);  
	sectors = tsk->arg & 0xFFFF;
	busy_time = tsk->transfer_end_t - tsk->transfer_start_t;
...
u32 mt_biolog_req_usage(unsigned int task_id)
{
	...
	req_time = tsk->transfer_end_t - tsk->transfer_start_t;
	msec = do_div(req_time, 1000);
