...
void mmc_blk_remove_card(struct mmc_host *host)
{
...
EXPORT_SYMBOL(mmc_blk_remove_card);
	...
	do {
		...
		case MMC_BLK_DATA_ERR: {
			...
			if (mmc_card_sd(card) && (reset_count > MMC_CARD_RESET_RETRIES)) {
				...
				if (err && mmc_card_sd(card)) {
					pr_err("%s: blk_reset failed : %d, remove sd card\n"
						, mmc_hostname(card->host), err);
					mmc_blk_remove_card(card->host);
					goto cmd_abort;
				} else if (err && mmc_card_mmc(card)) {
					pr_info("%s: blk_reset failed : %d, go to cmd_abort!\n",
						mmc_hostname(card->host), err);
					goto cmd_abort;
				}
				reset_count++;
...
static const struct mmc_fixup blk_fixups[] =
{
	...
	MMC_FIXUP(CID_NAME_ANY, CID_MANFID_SANDISK_NEW, CID_OEMID_ANY, add_quirk_mmc,
