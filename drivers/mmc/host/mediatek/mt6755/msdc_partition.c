...
static int proc_emmc_show(struct seq_file *m, void *v)
{
	...
	while (!host || !host->mmc || !host->mmc->card) {
		if (stime >= MMC_SLEEP_TIMEOUT) {
			ERR_MSG("request timeout (%d ms)", stime);
			break;
		}
		stime += 10;
		msleep(10);
	...
	if (stime > 0)
		ERR_MSG("request wait for %d ms", stime);
...
#endif
	if (!host->mmc->card) {
		pr_err("mmc0: %s: Card is not ready...\n", __func__);
		return -1;
	}
