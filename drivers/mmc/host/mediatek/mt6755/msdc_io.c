...
{
	struct mmc_host *mmc = mtk_msdc_host[id]->mmc;
	u32 val0, val1;
	...
	case 0:
		if (MSDC_PIN_PULL_NONE == mode) {
			...
			if (mmc->gpio_sleep_pupd0 != 0 && mmc->gpio_sleep_pupd1 != 0) {
				val0 = mmc->gpio_sleep_pupd0;
				val1 = mmc->gpio_sleep_pupd1;
			}
			...
			if (mmc->gpio_active_pupd0 != 0 && mmc->gpio_active_pupd1 != 0) {
				val0 = mmc->gpio_active_pupd0;
				val1 = mmc->gpio_active_pupd1;
			}
		}
...
static int msdc_get_gpio_info(struct device *dev, struct msdc_host *host)
{
	struct msdc_pinctrl_data *pctrl_data;
	...
	pctrl_data->pins_active = pinctrl_lookup_state(pctrl, "gpio_active");
	if (IS_ERR(pctrl_data->pins_active)) {
		ret = PTR_ERR(pctrl_data->pins_active);
		pr_err("Could not get active pinstates, err:%d\n", ret);
		ret = 0;
		goto out;
	}

	pctrl_data->pins_sleep = pinctrl_lookup_state(pctrl, "gpio_sleep");
	if (IS_ERR(pctrl_data->pins_sleep)) {
		ret = PTR_ERR(pctrl_data->pins_sleep);
		pr_err("Could not get sleep pinstates, err:%d\n", ret);
		ret = 0;
		goto out;
	}
	host->pctrl_data = pctrl_data;

out:
	if (ret < 0)
		pr_err("%s: gpio state for pinctrl failed with %d\n",
...
int msdc_get_gpio_setting(struct msdc_host *host, int suspend)
{
	int ret = 0;
	...
	if (!suspend)
		ret = pinctrl_select_state(host->pctrl_data->pctrl,
			host->pctrl_data->pins_active);
	else
		ret = pinctrl_select_state(host->pctrl_data->pctrl,
			host->pctrl_data->pins_sleep);

	if (ret < 0)
		pr_err("%s state for pinctrl failed with %d\n",
			suspend ? "Enabling" : "Disabling", ret);
	return ret;
}
EXPORT_SYMBOL(msdc_get_gpio_setting);
...
static int msdc_get_register_settings(struct msdc_host *host, struct device_node *np)
{
	...
	if (register_setting_node) {
		...
		pr_warn("[msdc%d] edge direction of <cmd,rw,wd> = <%u,%u,%u>\n"
			, host->id, host->hw->cmd_edge, host->hw->rdata_edge, host->hw->wdata_edge);
	} else {
...
int msdc_dt_init(struct platform_device *pdev, struct mmc_host *mmc)
{
	...
	ret = msdc_get_gpio_info(&pdev->dev, host);
	if (ret) {
		pr_err("msdc%d of gpio info fail!!: %d\n", id, ret);
		return ret;
	}

	ret = msdc_get_gpio_setting(host, 0);
	if (ret) {
		pr_err("msdc%d of gpio setting fail!!: %d\n", id, ret);
		return ret;
	}
