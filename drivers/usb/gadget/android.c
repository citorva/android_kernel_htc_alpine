...
static void android_work(struct work_struct *data)
{
	...
	struct usb_composite_dev *cdev = (dev == NULL) ? NULL : dev->cdev; 
	...
	static bool pre_is_hwconnected = false;

	if (!cdev){
		return ;
	}

	printk("[USB] android_work: sw_suspended:%d,  suspended:%d, (pre)sw_connected:%d,  connected:%d, config:%d, (pre)connect2pc:%d\n",
			dev->sw_suspended, dev->suspended, dev->sw_connected, dev->connected, cdev->config?1:0, connect2pc);
	...
	if ((is_hwconnected != pre_is_hwconnected) && (connect2pc != dev->sw_connected)) {
		connect2pc = dev->sw_connected;
		pre_is_hwconnected = is_hwconnected;
		switch_set_state(&cdev->sw_connect2pc, connect2pc ? 1 : 0);
		pr_info("[USB] %s: is_hwconnected=%d, set usb_connect2pc = %d\n", __func__, is_hwconnected, connect2pc);
		if (!connect2pc) {
			pr_info("[USB] %s: OS_NOT_YET\n", __func__);
			os_type = OS_NOT_YET;
			first_dt_w_length = 0;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_notice("%s [0]=%d,[1]=%d,[2]=%d,[3]=%d,[4]=%d, num=%d\n", __func__, val[0], val[1], \
...
static int serial_function_init(struct android_usb_function *f, struct usb_composite_dev *cdev)
{
	...
	strlcpy(serial_transports, "tty:modem,tty:autobot,tty:serial,tty:autobot", sizeof(serial_transports));
					...
					const struct usb_ctrlrequest *c)
{
	...
	{
		functions_no++;
		buff += snprintf(buff, sizeof(usb_function_string) - (buff - usb_function_string), "%s,", f_count->name); 
	}
	*(buff-1) = '\n';
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (sscanf(buf, "%02x:%02x:%02x:%02x:%02x:%02x\n",
		    (unsigned int *)&eem->ethaddr[0], (unsigned int *)&eem->ethaddr[1], 
		    (unsigned int *)&eem->ethaddr[2], (unsigned int *)&eem->ethaddr[3], 
		    (unsigned int *)&eem->ethaddr[4], (unsigned int *)&eem->ethaddr[5]) == 6) 
		return size;
	return -EINVAL;
}
...
#define NLUN_ICUSB (0)
	...
	for(i = 1; i < config->fsg.nluns; i++) {
		...
		snprintf(string_lun, sizeof(string_lun), "lun%d",i); 
...
functions_show(struct device *pdev, struct device_attribute *attr, char *buf)
{
	...
	list_for_each_entry(f, &dev->enabled_functions, enabled_list)
		buff += snprintf(buff, PAGE_SIZE - (buff - buf), "%s,", f->name); 
			       ...
			       const char *buff, size_t size)
{
	...
	buffer = buff;
		...
		if (get_debug_flag() & 0x101) {
			buffer = change_charging_to_ums(buff);
			USB_INFO("In debug mode, switch function convert to : %s\n", buffer);
		} else
