...
static void projector2_send_fb(struct projector2_dev *dev)
{
	...
	while (count > 0 || last_pkt == 1) {
		...
		if (req) {
			...
			xfer = count > PROJ2_TXN_MAX? PROJ2_TXN_MAX : count;
			req->length = xfer;
				...
				struct usb_endpoint_descriptor *out_desc)
{
	...
	for (i = 0; i < dev->tx_req_count; i++) {
		req = projector2_request_new(dev->ep_in, PROJ2_TXN_MAX);
		if (!req)
			goto fail;
											...
											const struct usb_ctrlrequest *ctrl)
{
	...
	if ((ctrl->bRequestType & USB_TYPE_MASK) == USB_TYPE_VENDOR) {
		printk(KERN_INFO "[USB][MIRROR_LINK] %s: request(req=0x%02x, wValue=%d, "
						"wIndex=%d, wLength=%d)\n", __func__,
...
static int projector2_bind_config(struct usb_configuration *c)
{
	...
	dev->tx_req_count = (dev->width * dev->height * 2 / PROJ2_TXN_MAX) + 3;
