...
struct tca6418_data *tca6418_pdata;
...
static int tca6418_pinctrl_probe(struct platform_device *pdev)
{
	...
	tca6418_pdata = devm_kzalloc(&pdev->dev, sizeof(*tca6418_pdata), GFP_KERNEL);
	if (!tca6418_pdata)
		return -ENOMEM;

	mutex_init(&tca6418_pdata->reglock);
	tca6418_pdata->devref = find_devref_by_node(pdev->dev.of_node);
	if (!tca6418_pdata->devref) {
		dev_warn(&pdev->dev, "No available devref found, defer probe\n");
		ret = -EPROBE_DEFER;
		goto fail_devref;
	}

	tca6418_pdata->pctrl = pinctrl_register(&tca6418_pinctrl_desc, &pdev->dev, tca6418_pdata);
	if (IS_ERR(tca6418_pdata->pctrl)) {
		dev_err(&pdev->dev, "Couldn't register pinctrl driver\n");
		ret = PTR_ERR(tca6418_pdata->pctrl);
		goto fail_pctrl;
	}

	memcpy(&tca6418_pdata->gpioc, &tca6418_chip, sizeof(tca6418_pdata->gpioc));
	tca6418_pdata->gpioc.dev = &pdev->dev;
	tca6418_pdata->gpioc.base = -1;
	tca6418_pdata->gpioc.ngpio = tca6418_pinctrl_desc.npins;
	tca6418_pdata->gpioc.label = tca6418_pinctrl_desc.name;
	tca6418_pdata->gpioc.owner = THIS_MODULE;
	tca6418_pdata->gpioc.of_node = pdev->dev.of_node;

	ret = gpiochip_add(&tca6418_pdata->gpioc);
	if (ret) {
		dev_err(&pdev->dev, "Failed register gpiochip\n");
		goto fail_gpioc;
	}

	ret = gpiochip_add_pin_range(&tca6418_pdata->gpioc,
			dev_name(&pdev->dev),
			0,
			0,
			tca6418_pdata->gpioc.ngpio);
	if (ret) {
		dev_err(&pdev->dev, "Failed to add pin range\n");
		goto fail_gpioc_pins;
	}

	platform_set_drvdata(pdev, tca6418_pdata);

	dev_info(&pdev->dev, "Probed tca6418 pinctrl driver\n");
	return 0;

fail_gpioc_pins:
	gpiochip_remove(&tca6418_pdata->gpioc);
fail_gpioc:
	pinctrl_unregister(tca6418_pdata->pctrl);
fail_pctrl:
fail_devref:
	devm_kfree(&pdev->dev, tca6418_pdata);
	return ret;
}
		...
		const struct i2c_device_id *id)
{
	...
	if (platform_driver_register(&tca6418_pinctrl_driver)) {
		dev_err(&client->dev, "Fail to register tca6418 pinctrl driver\n");
		return -EINVAL;
	}
