...
static int pins_mux_show(struct seq_file *s, void *what)
{
	unsigned int i;
	const struct mtk_desc_function *func;

	seq_printf(s, "registered pins: %lx\n", ARRAY_SIZE(mtk_pins_mt6755));

	for (i = 0; i < (ARRAY_SIZE(mtk_pins_mt6755)); i++) {

		seq_printf(s, "\tpin:%03d, (%s)\n",
			mtk_pins_mt6755[i].pin.number, mtk_pins_mt6755[i].pin.name ? "name" : "unnamed");

		 func = mtk_pins_mt6755[i].functions;

		while (func && func->name) {
			seq_printf(s, "\t\tfunc %d, (%s)\n",
				func->muxval, func->name);
			func++;
		}
...
static int pins_mux_open(struct inode *inode, struct file *file)
{
	return single_open(file, pins_mux_show, inode->i_private);
}

static const struct file_operations pins_mux_ops = {
	.open		= pins_mux_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
...
static int mt6755_pinctrl_probe(struct platform_device *pdev)
{
	...
	ret = mtk_pctrl_init(pdev, &mt6755_pinctrl_data, NULL);
	if(ret == 0)
	{
#ifdef CONFIG_DEBUG_FS
		debugfs_create_file("chip_pin_mux", S_IFREG | S_IRUGO, NULL, NULL, &pins_mux_ops);
#endif /* CONFIG_DEBUG_FS */
	}
	return ret;
}
