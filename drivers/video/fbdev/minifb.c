...
extern int ion_handle_get_size(struct ion_client *client, struct ion_handle *handle, unsigned long *size);
		...
		size_t count, loff_t *ppos)
{
	...
	pr_debug("%s: dump from %p with %lld/%lu bytes, count=%lu, copylen=%d, ret = %d\n",
...
int minifb_init(struct minifb_session *sess)
{
	...
	minifb_ctrl->iclient = ion_client_create(g_ion_device, "minifb");
...
int minifb_queuebuf(struct minifb_req *data)
{
	...
	if (IS_ERR_OR_NULL(node->ionhdl)) {
		pr_err("%s: import ion buf fail, fd=%d\n", __func__, node->buf_info.memory_id);
		return -EINVAL;
	}
	node->info = sCount++;
