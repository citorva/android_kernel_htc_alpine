...
static void gpiolib_dbg_show(struct seq_file *s, struct gpio_chip *chip)
{
	...
	for (i = 0; i < chip->ngpio; i++, gpio++, gdesc++) {
		...
		seq_printf(s, " gpio-%-3d (%-20.20s) %s %s %s func%d  %s",
			gpio, gdesc->label,
			is_out ? "out" : "in ",
			chip->get
				? (chip->get(chip, i) ? "hi" : "lo")
				: "?  ",
			is_irq ? "IRQ" : "   ", mt_get_gpio_mode_base(gpio), (mt_get_gpio_pull_enable_base(gpio))? (mt_get_gpio_pull_select_base(gpio)? "pull_up" : "pull_down") : "pull_null");
		seq_printf(s, "\n");
	}
}
...
static ssize_t gpiolib_write(struct file *file, const char __user *buf,size_t size, loff_t *ppos)
{
	...
	if(copy_from_user(info,buf, size)){
		printk("copy_from_user Error, return\n");
		return size;
	}
	printk("gpio command:%s\n",info);

	if (!strncmp(p_info, "-h", 2)) {
		printk("write: echo \"-w port value\" > /d/gpio\n");
		printk("read:  1: echo \"-r port\" > /d/gpio; 2: dmesg\n");
	}
	else if (!strncmp(p_info, "-w", 2)) {
		p_info += 2;
		if (2 == sscanf(p_info, "%d %d", &port, &value)){
			printk("write port:%d, value:%d\n", port, value);
			gpio_direction_output(port,value);
			__gpio_set_value(port,value);
		}
	}
	else if (!strncmp(p_info, "-r", 2)) {
		p_info += 2;
		if (1 == sscanf(p_info, "%d", &port)){
			gpio_direction_input(port);
			printk("read port:%d, value:%d\n", port, __gpio_get_value(port));
		}
	}
	else
		printk("unknow command, echo -h > /d/gpio\n");
