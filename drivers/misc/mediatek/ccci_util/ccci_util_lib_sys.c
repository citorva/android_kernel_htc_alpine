...
static ssize_t traffic_debug_enable_show(char *buf)
{
	int curr = 0;

	curr = snprintf(buf, 16, "%d\n", ccci_traffic_debug_enable);
	return curr;
}

static ssize_t traffic_debug_enable_store(const char *buf, size_t count)
{
	ccci_traffic_debug_enable = buf[0] - '0';
	return count;
}

CCCI_ATTR(traffic_debug, 0660, &traffic_debug_enable_show, &traffic_debug_enable_store);
...
static void cmd_process(char cmd[], unsigned long para[], int para_num)
{
	if ((para_num == 2) && (strcmp(cmd, "IOR") == 0)) {
		...
		aat_show_case = 0;	
	} else if ((para_num == 1) && (strcmp(cmd, "MDFU") == 0)) {	
		CCCI_UTIL_DBG_MSG("Command:%s\n", cmd);
		...
		aat_err_no = 0x00001000;
		aat_show_case = 0;	
	} else if ((para_num == 1) && (strcmp(cmd, "MDFL") == 0)) {	
		CCCI_UTIL_DBG_MSG("Command:%s\n", cmd);
