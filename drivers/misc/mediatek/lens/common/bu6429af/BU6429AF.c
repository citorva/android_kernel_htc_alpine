...
static int init_setting(void)
{
	...
	for (i = 0; i < 21; i++) {
		i4RetValue = i2c_master_send(g_pstAF_I2Cclient, adoptive_tbl[i], 2);
		if (i4RetValue < 0) {
			LOG_INF("I2C send adoptive_tbl %d failed!!\n", i);
			return -1;
		}
	}
