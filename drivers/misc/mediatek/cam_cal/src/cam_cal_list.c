...

{
	...
	{printk("fhyfhy1:cam_get_sensor_list\n");
		return 1;
	}

	printk("fhyfhy:cam_get_sensor_list\n");
...
unsigned int cam_cal_check_mtk_cid(struct i2c_client *client, cam_cal_cmd_func readCamCalData)
{
	...
	printk("fhyfhy:cam_cal_check_mtk_cid");
	if (readCamCalData != NULL) {
		readCamCalData(client, 1, (unsigned char *)&calibrationID, 4);
		CAM_CALDB("calibrationID = %x\n", calibrationID);
		printk("calibrationID = %x\n", calibrationID);
	}

	if (calibrationID != 0)
		for (j = 0; j < MTK_MAX_CID_NUM; j++) {
			CAM_CALDB("[ryf-ov16880-1]mtkCidList[%d] = 0x%x,calibrationID=0x%x\n", j,mtkCidList[j], calibrationID);
			printk("[ryf-ov16880-1]mtkCidList[%d] = 0x%x,calibrationID=0x%x\n", j,mtkCidList[j], calibrationID);
