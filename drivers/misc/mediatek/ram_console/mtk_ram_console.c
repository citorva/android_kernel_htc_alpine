...
{
#ifdef CONFIG_DISABLE_LAST_KMSG2
	pr_err("ram_console: not support to save kernel log to emmc!\n");
	return;
#else //CONFIG_DISABLE_LAST_KMSG2
	int buff_size;
...
#endif //!CONFIG_DISABLE_LAST_KMSG2
}
...
#ifndef CONFIG_DISABLE_LAST_KMSG2
...
#if defined(CONFIG_HTC_DEBUG_BOOTLOADER_LOG)
	if (bl_last_log_buf != NULL) {
		seq_write(m, bl_last_log_buf, bl_last_log_buf_size);
	}
#endif
...
#endif
#if defined(CONFIG_HTC_DEBUG_BOOTLOADER_LOG)
	if (bl_cur_log_buf != NULL) {
		seq_write(m, bl_cur_log_buf, bl_cur_log_buf_size);
	}
#endif
	return 0;
}
...
#ifdef CONFIG_MTK_AEE_IPANIC
#ifdef CONFIG_DISABLE_LAST_KMSG2
	pr_alert("ram_console: not support to read kernel log from emmc!\n");
#else //CONFIG_DISABLE_LAST_KMSG2
	int err;
