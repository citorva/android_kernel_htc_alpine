...
#define	DPIDLE_LOG_DISCARD_CRITERIA			5000	
...
static wake_reason_t spm_output_wake_reason(struct wake_status *wakesta, struct pcm_desc *pcmdesc, u32 dump_log)
{
	...
	if ((dump_log == DEEPIDLE_LOG_FULL) || (dump_log == DEEPIDLE_LOG_REDUCED2)) {
		wr = __spm_output_wake_reason(wakesta, pcmdesc, (WAKE_TYPE_DP << 1) + false);
...
wake_reason_t spm_go_to_sleep_dpidle(u32 spm_flags, u32 spm_data)
{
	...
	last_wr = __spm_output_wake_reason(&wakesta, pcmdesc, (WAKE_TYPE_DP << 1) + true);
