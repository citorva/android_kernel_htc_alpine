...
int spm_eint_max_number;
int *wake_eint_status;
u32 gpio_base_addr;
...
static void spm_register_init(void)
{
	...
	if (of_property_read_u32(node, "mediatek,max_eint_num",
				&spm_eint_max_number))
		spm_err("get max_eint_num failed\n");

	if (spm_eint_max_number) {
		wake_eint_status = kmalloc(sizeof(unsigned int) *
				spm_eint_max_number , GFP_KERNEL);
		if (!wake_eint_status)
			spm_eint_max_number = 0;
	}
	...
	BUG_ON((spm_read(PCM_FSM_STA) & 0x7fffff) != PCM_FSM_STA_DEF);	
...
#define VCORE_VOSEL_SLEEP_0P6	0x00	
#define VCORE_VOSEL_SLEEP_0P7	0x10	
#define VCORE_VOSEL_SLEEP_0P77	0x1C	
#define VCORE_VOSEL_SLEEP_0P9	0x30	
...
static void spm_pmic_set_buck(u32 addr, int en_ctrl, int en, int vsleep_en, int vosel_sel, int lock)
{
	if (lock == 0) {
		if (en_ctrl != -1)
			pmic_config_interface_nolock(addr,	
						     en_ctrl, 0x1,	
						     0);	
		if (en != -1)
			pmic_config_interface_nolock(addr + 0x4,	
						     en, 0x1,	
						     0);	
		if (vsleep_en != -1)
			pmic_config_interface_nolock(addr + 0x12,	
						     vsleep_en, 0x1,	
						     8);	
		if (vosel_sel != -1)
			pmic_config_interface_nolock(addr + 0x2,	
						     vosel_sel, 0x7,	
						     3);	
	} else {
		if (en_ctrl != -1)
			pmic_config_interface(addr,	
					      ...
					      0);	
		if (en != -1)
			pmic_config_interface(addr + 0x4,	
					      en, 0x1,	
					      0);	
		if (vsleep_en != -1)
			pmic_config_interface(addr + 0x12,	
					      vsleep_en, 0x1,	
					      8);	
		if (vosel_sel != -1)
			pmic_config_interface(addr + 0x2,	
					      vosel_sel, 0x7,	
					      3);	
	}
}
			     ...
			     int secclk_mode_sel, int lock)
{
	if (lock == 0) {
		if (on_ctrl != -1)
			pmic_config_interface_nolock(addr, on_ctrl, 0x1,	
						     3);	
		if (en != -1)
			pmic_config_interface_nolock(addr, en, 0x1,	
						     1);	
		if (mode_ctrl != -1)
			pmic_config_interface_nolock(addr, mode_ctrl, 0x1,	
						     2);	
		if (secclk_mode_sel != -1)
			pmic_config_interface_nolock(addr, secclk_mode_sel, 0x7,	
						     5);	
	} else {
		if (on_ctrl != -1)
			pmic_config_interface(addr, on_ctrl, 0x1,	
					      3);	
		if (en != -1)
			pmic_config_interface(addr, en, 0x1,	
					      1);	
		if (mode_ctrl != -1)
			pmic_config_interface(addr, mode_ctrl, 0x1,	
					      2);	
		if (secclk_mode_sel != -1)
			pmic_config_interface(addr, secclk_mode_sel, 0x7,	
					      5);	
	}
}
...
void spm_pmic_power_mode(int mode, int force, int lock)
{
	...
	case PMIC_PWR_DEEPIDLE:
		...
		spm_pmic_set_ldo(MT6351_LDO_VUSB33_CON0, 0, 1, 0, PMIC_LDO_SRCLKEN_NA, lock);	
		...
		spm_pmic_set_ldo(MT6351_LDO_VA18_CON0, 0, 1, 0, PMIC_LDO_SRCLKEN_NA, lock);	
	...
	case PMIC_PWR_SODI3:
		...
		spm_pmic_set_ldo(MT6351_LDO_VLDO28_CON0, 0, 1, 0, PMIC_LDO_SRCLKEN_NA, lock);	
