...
static void spm_get_wake_eint_status(void)
{
	...
	for (reg_base = 0; reg_base < spm_eint_max_number; reg_base += 32) {
		status = spm_read((reg_base / 32) * 4 + SPM_EINT_BASE);
		for (offset = 0; offset < 32; offset++) {
			index = reg_base + offset;
			if (index >= spm_eint_max_number)
				break;
...
#define WAKE_TYPE_SLEEP 3
	wr = __spm_output_wake_reason(wakesta, pcmdesc, (WAKE_TYPE_SLEEP << 1) + true);
...
{
	if (eint_wake)
		return (bool)wake_eint_status[eint_num];
	return 0;
}
