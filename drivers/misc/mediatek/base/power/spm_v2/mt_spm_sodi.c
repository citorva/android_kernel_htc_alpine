...
#define SODI_LOGOUT_INTERVAL_CRITERIA	(5000U)	
...
spm_sodi_output_log(struct wake_status *wakesta, struct pcm_desc *pcmdesc, int vcore_status, u32 sodi_flags)
{
	...
	} else if (sodi_flags & SODI_FLAG_REDUCE2_LOG) {
		wr = __spm_output_wake_reason(wakesta, pcmdesc, (WAKE_TYPE_SO << 1) + false);
