...
static ssize_t hps_num_base_pnpmgr_proc_write(struct file *file,
						 const char __user *buffer,
						 size_t count, loff_t *pos)
{
	...
	if ((hps_ctxt.is_hmp || hps_ctxt.is_amp)
	    && (sscanf(desc, "%u %u", &little_num_base_pnpmgr, &big_num_base_pnpmgr) == 2)) {
		if (little_num_base_pnpmgr > num_possible_little_cpus()
		    || little_num_base_pnpmgr < 0) {
		...
		return count;
		   ...
		   && !kstrtouint(desc, 0, &little_num_base_pnpmgr)) {
		if (little_num_base_pnpmgr > num_possible_little_cpus()
		    || little_num_base_pnpmgr < 0) {
...
static ssize_t hps_num_limit_pnpmgr_proc_write(struct file *file,
						   const char __user *buffer,
						   size_t count, loff_t *pos)
{
	int len = 0;
	int little_num_limit_pnpmgr = 0, big_num_limit_pnpmgr = 0;
	char desc[32];
	...
	if ((hps_ctxt.is_hmp || hps_ctxt.is_amp)
	    &&
	    (sscanf(desc, "%u %u", &little_num_limit_pnpmgr, &big_num_limit_pnpmgr) == 2)) {
		if (little_num_limit_pnpmgr > num_possible_little_cpus()
		    || little_num_limit_pnpmgr < 0) {
		...
		return count;
		   ...
		   && !kstrtouint(desc, 0, &little_num_limit_pnpmgr)) {
                if (little_num_limit_pnpmgr > num_possible_little_cpus()
                    || little_num_limit_pnpmgr < 0) {
