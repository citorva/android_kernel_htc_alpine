...
#define RT1711H_DRV_VERSION	"1.1.6_G"
...
static int rt1711_init_alert(struct tcpc_device *tcpc)
{
	...
	if (node) {
		node = of_find_compatible_node(NULL, NULL, "mediatek,usb_type_c_eint");
		if (node)
			chip->irq = irq_of_parse_and_map(node, 0);
		else
			pr_err("%s cannot get node by compatible usb_type_c_eint\n", __func__);
	} else
		pr_err("%s cannot get node by name type_c_port0\n", __func__);
#endif
...
static int rt_parse_dt(struct rt1711_chip *chip, struct device *dev)
{
	...
	if (chip->irq_gpio < 0) {
		chip->irq_gpio = -1;
		pr_err("%s failed to find rt1711h,irq_gpio\n", __func__);
		return -EINVAL;
	}
...
static int rt1711_pinctrl_configure(struct rt1711_chip *chip, bool active)
{
        ...
        if (active) {
                set_state =
                        pinctrl_lookup_state(chip->rt1711_pinctrl,
                                "usb_type_c_active");
                if (IS_ERR(set_state)) {
                        pr_err("%s cannot get rt1711 pinctrl active state\n", __func__);
                ...
                set_state =
                        pinctrl_lookup_state(chip->rt1711_pinctrl,
                                "usb_type_c_suspend");
                if (IS_ERR(set_state)) {
                        pr_err("%s cannot get rt1711 pinctrl suspend state\n", __func__);
                        return PTR_ERR(set_state);
                }
        }
        retval = pinctrl_select_state(chip->rt1711_pinctrl, set_state);
        if (retval) {
                pr_err("%s cannot set rt1711 pinctrl state (err %d)\n", __func__, retval);
                return retval;
        }
				...
				const struct i2c_device_id *id)
{
	...
	pr_info("%s %s, defer_count=%d\n", __func__, RT1711H_DRV_VERSION, defer_count); 
	...
	chip->rt1711_pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR(chip->rt1711_pinctrl)) {
		if (PTR_ERR(chip->rt1711_pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		pr_err("rt1711 Target does not use pinctrl\n");
		chip->rt1711_pinctrl = NULL;
	}

	if (chip->rt1711_pinctrl) {
		ret = rt1711_pinctrl_configure(chip, true);
		if (ret)
			pr_err("rt1711 cannot set pinctrl active state\n");
		else
			pr_info("rt1711 successfully set pinctrl active state\n");
	}
...
{
	struct rt1711_chip *chip = i2c_get_clientdata(client);
	struct tcpc_device *tcpc = chip ? chip->tcpc : NULL; 
