...
#ifdef CONFIG_SWITCH_FSA3030
static int tcpc_fsa3030_handle = 0;
#endif
static char *TCP_NOTIFY_STRING[TCP_NOTIFY_NUM] = {
				...
				unsigned long event, void *data)
{
	...
	pr_info("%s event: %02lu (%s)\n", __func__, event, (event < TCP_NOTIFY_NUM) ? TCP_NOTIFY_STRING[event] : "NULL"); 
	switch (event) {
	case TCP_NOTIFY_TYPEC_STATE:
		...
		case TYPEC_UNATTACHED: 
			if (noti->typec_state.old_state == TYPEC_ATTACHED_SNK)
				...
				if (tcpc_fsa3030_handle)
					fsa3030_switch_set(tcpc_fsa3030_handle, FSA3030_SWITCH_TYPE_AUDIO, false);
#ifdef CONFIG_HTC_TYPEC_HEADSET_DETECT
				htc_aud_hp_det_set (false);
#endif
			}
#endif
		...
		case TYPEC_ATTACHED_AUDIO: 
			if (tcpc_fsa3030_handle) {
				fsa3030_switch_set(tcpc_fsa3030_handle, FSA3030_SWITCH_TYPE_AUDIO, true);
			}
#ifdef CONFIG_HTC_TYPEC_HEADSET_DETECT
			htc_aud_hp_det_set (true);
#endif
			break;
#endif
...
#ifdef CONFIG_SWITCH_FSA3030
    tcpc_fsa3030_handle = fsa3030_switch_register(FSA3030_SWITCH_TYPE_AUDIO);
#endif
