...
struct fm_flag_event *fm_flag_event_create(const fm_s8 *name)
{
	...
	tmp = kzalloc(sizeof(struct fm_flag_event), GFP_KERNEL);
	if (!tmp) {
...
struct fm_lock *fm_lock_create(const fm_s8 *name)
{
	...
	tmp = kzalloc(sizeof(struct fm_lock), GFP_KERNEL);
	if (!tmp) {
		WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_lock) -ENOMEM\n");
		return NULL;
	}

	mutex = kzalloc(sizeof(struct semaphore), GFP_KERNEL);
	if (!mutex) {
...
struct fm_lock *fm_spin_lock_create(const fm_s8 *name)
{
	...
	spin_lock = kzalloc(sizeof(spinlock_t), GFP_KERNEL);
	if (!spin_lock) {
...
struct fm_timer *fm_timer_create(const fm_s8 *name)
{
	...
	tmp = kzalloc(sizeof(struct fm_timer), GFP_KERNEL);
	if (!tmp) {
		WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_timer) -ENOMEM\n");
		return NULL;
	}

	timerlist = kzalloc(sizeof(struct timer_list), GFP_KERNEL);
	if (!timerlist) {
...
struct fm_work *fm_work_create(const fm_s8 *name)
{
	...
	my_work = kzalloc(sizeof(struct fm_work), GFP_KERNEL);
	if (!my_work) {
		WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_work) -ENOMEM\n");
		return NULL;
	}

	sys_work = kzalloc(sizeof(struct work_struct), GFP_KERNEL);
	if (!sys_work) {
...
struct fm_workthread *fm_workthread_create(const fm_s8 *name)
{
	...
	my_thread = kzalloc(sizeof(struct fm_workthread), GFP_KERNEL);
	if (!my_thread) {
...
struct fm_fifo *fm_fifo_create(const fm_s8 *name, fm_s32 item_size, fm_s32 item_num)
{
	...
	tmp = kzalloc(sizeof(struct fm_fifo), GFP_KERNEL);
	if (!tmp) {
		WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_fifo) -ENOMEM\n");
		return NULL;
	}

	buf = kzalloc(item_size * item_num, GFP_KERNEL);
	if (!buf) {
		WCN_DBG(FM_ALT | MAIN, "fm_zalloc(fm_fifo) -ENOMEM\n");
		kfree(tmp);
		return NULL;
