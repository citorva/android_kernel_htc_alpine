...
void mt_mt65xx_led_blink_work(struct work_struct *work)
{
	...
	led_blink_set(&led_data->cdev, (unsigned long*)&led_data->delay_on, (unsigned long*)&led_data->delay_off);
}
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	LEDS_DRV_DEBUG("%s\n", __func__);
	get_div_array();
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL && cust_led_list != NULL ; i++) {
		...
		if (strcmp(g_leds_data[i]->cdev.name, "lcd-backlight") == 0) {
			rc = device_create_file(g_leds_data[i]->cdev.dev,
						&dev_attr_backlight_info);
			if (rc) {
				LEDS_DRV_DEBUG
				    ("device_create_file backlight_info fail!\n");
			}
		}
		else{
