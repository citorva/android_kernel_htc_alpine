...
extern unsigned int get_sku_data(int index);

static void report_cover_event(int pole, int irq, struct ak_hall_data *hl);
static ssize_t debug_level_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
static ssize_t read_att(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0, pos = 0;
	char string[100] = {0};
	struct ak_hall_data *hl = g_hl;
...
static ssize_t write_att(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	struct ak_hall_data *hl = g_hl;
	if (buf[0] == '0' || buf[0] == '1') {
...
static int gpio_hall_pinctrl_configure(struct ak_hall_data *ddata, bool active)
{
	...
	if (active) {
		set_state =
			pinctrl_lookup_state(ddata->hall_pinctrl,
				"gpio_hall_active");
		if (IS_ERR(set_state)) {
			dev_err(&ddata->input_dev->dev,
	...
	retval = pinctrl_select_state(ddata->hall_pinctrl, set_state);
	if (retval) {
		dev_err(&ddata->input_dev->dev,
...
static int hall_sensor_dt_parser(struct device_node *dt, struct hall_platform_data *pdata)
{
	...
	for_each_child_of_node(dt, child) {
		ret = of_property_read_u32(child, parser_st[i], &gpio_att);
		if(ret < 0) {
			HL_ERR("DT: %s parser failure, ret=%d", parser_st[i], ret);
			goto parser_failed;
		}
		
		irq_att = gpio_to_irq(gpio_att);

		HL_LOG("DT:%s[%d] read, irq=%d", parser_st[i], gpio_att, irq_att);
		if(i == 1) {
			pdata->gpio_att = gpio_att;
			pdata->irq_att = irq_att;
		}
		if(i == 2) {
			pdata->gpio_att_s = gpio_att;
			pdata->irq_att_s = irq_att;
		}
		i++;
	}
...
static void report_cover_event(int pole, int irq, struct ak_hall_data *hl)
{
	...
	if (pole == HALL_POLE_N) {		
		...
		tmp = gpio_get_value(hl->gpio_att_s);
#ifdef LEVEL_TRIGGER
		irq_set_irq_type(irq, val_n?IRQF_TRIGGER_LOW|IRQF_ONESHOT : IRQF_TRIGGER_HIGH|IRQF_ONESHOT);
#endif
		HL_LOG("att_n, n=%d , s=%d, gpio_%d\n",val_n,tmp,hl->gpio_att);
		wake_lock_timeout(&hl->wake_lock, (2 * HZ));
		if (prev_val_n != val_n) {
			input_report_key(hl->input_dev, HALL_N_POLE, -1);
		...
		tmp = gpio_get_value(hl->gpio_att);
#ifdef LEVEL_TRIGGER
		irq_set_irq_type(irq, val_s?IRQF_TRIGGER_LOW | IRQF_ONESHOT : IRQF_TRIGGER_HIGH|IRQF_ONESHOT);
#endif
		HL_LOG("att_s, n=%d , s=%d, gpio_%d\n",tmp,val_s,hl->gpio_att_s);
		wake_lock_timeout(&hl->wake_lock, (2 * HZ));
		if (prev_val_s != val_s) {
			input_report_key(hl->input_dev, HALL_S_POLE, -1);
...
static irqreturn_t hall_npole_irq_thread(int irq, void *ptr)
{
	...
	HL_LOG("N-pole interrupt trigger");
...
{
	struct ak_hall_data *hl = ptr;
	HL_LOG("S-pole interrupt trigger");
...
static int hall_sensor_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node) {
		...
		if (pdata) {
			...
			hl->irq_att    = pdata->irq_att;
			hl->irq_att_s  = pdata->irq_att_s;

			HL_LOG("[HL] %d %d, %d %d",
			hl->gpio_att, hl->gpio_att_s, hl->irq_att, hl->irq_att_s);
		}
	} else {
	...
	hl->hall_pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(hl->hall_pinctrl)) {
		if (PTR_ERR(hl->hall_pinctrl) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		HL_ERR("Target does not use pinctrl\n");
		hl->hall_pinctrl = NULL;
	}

	if (hl->hall_pinctrl) {
		error = gpio_hall_pinctrl_configure(hl, true);
		if (error)
			HL_ERR("cannot set ts pinctrl active state\n");
	}

	if (gpio_is_valid(pdata->gpio_att)) {
		ret = gpio_request(pdata->gpio_att, "hall_n");
		if (ret) {
	...
	if (hl->att_used > 1){
		if (gpio_is_valid(pdata->gpio_att_s)){
			gpio_request(pdata->gpio_att_s, "hall_s");
			if (ret) {
...
#ifdef LEVEL_TRIGGER
	if(prev_val_n == 1){
		ret = request_irq(hl->irq_att, hall_npole_irq_thread,IRQF_TRIGGER_LOW|IRQF_ONESHOT, "mediatek,hall_1-eint", hl);
	}
	else if(prev_val_n == 0){
		ret = request_irq(hl->irq_att, hall_npole_irq_thread,IRQF_TRIGGER_HIGH|IRQF_ONESHOT, "mediatek,hall_1-eint", hl);
	}
#else
	ret = request_irq(hl->irq_att, hall_npole_irq_thread,IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING|IRQF_ONESHOT, "mediatek,hall_1-eint", hl);
#endif

	if (ret == 0)
	{
		irq_set_irq_wake(hl->irq_att, 1);
		hl->irq_n = hl->irq_att;
		HL_LOG("[HL]Operate in [Interrupt] mode, irq[%d]", hl->irq_att);
	}
	else
	{
...
#ifdef LEVEL_TRIGGER
		if(prev_val_s == 1){
			ret = request_irq(hl->irq_att_s, hall_spole_irq_thread,IRQF_TRIGGER_LOW|IRQF_ONESHOT, "mediatek,hall_2-eint", hl);
		}else if(prev_val_s == 0){
			ret = request_irq(hl->irq_att_s, hall_spole_irq_thread,IRQF_TRIGGER_HIGH|IRQF_ONESHOT, "mediatek,hall_2-eint", hl);
		}
#else
		ret = request_irq(hl->irq_att_s, hall_spole_irq_thread,IRQF_TRIGGER_FALLING|IRQF_TRIGGER_RISING|IRQF_ONESHOT, "mediatek,hall_2-eint", hl);
#endif

		if (ret == 0)
		{
			irq_set_irq_wake(hl->irq_att_s, 1);
			hl->irq_s = hl->irq_att_s;
			HL_LOG("[HL]Operate in [Interrupt] mode, irq[%d]", hl->irq_att_s);
		}
		else
		{
			free_irq(hl->irq_att_s, hl);
