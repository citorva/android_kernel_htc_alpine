...
static irqreturn_t generic_interrupt(int irq, void *__hci)
{
	...
	if (dwL1Value & MAC2_INTR) {
		...
		os_printk(K_ALET, "===L1[%x] U2[%x] U3D_COMMON_USB_INTR[%x]\n", dwL1Value, dwIntrUsbValue, os_readl(U3D_COMMON_USB_INTR));
	}
	...
	if (dwL1Value & MAC3_INTR) {
		...
		os_printk(K_ALET, "===L1[%x] LTSSM[%x] U3D_LTSSM_INTR_ENABLE[%x]\n", dwL1Value, dwLtssmValue, os_readl(U3D_LTSSM_INTR_ENABLE));
	}
#ifdef USE_SSUSB_QMU
	...
	if ((dwL1Value & (BMU_INTR | MAC2_INTR)) || dwIntrUsbValue) {
		...
		if (musb->int_usb || musb->int_tx || musb->int_rx)
			...
			os_printk(K_ALET, "===L1[%x] Nothing can do?? Tx[%x] Rx[%x] U2[%x]===\n",
...
static void mtu3d_musb_reg_init(struct musb *musb)
{
	...
	if (ret || u3phy_ops) {
		...
		musb->is_clk_on = 1;
		os_printk(K_ALET, "%s is_clk_on %d\n", __func__, musb->is_clk_on);
...
static int mtu3d_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl))
		...
		pinctrl_iddig = pinctrl_lookup_state(pinctrl, "idpin_input");
