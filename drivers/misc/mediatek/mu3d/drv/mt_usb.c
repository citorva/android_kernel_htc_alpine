...
void connection_work(struct work_struct *data)
{
	...
	if (!usb_phy_check_in_uart_mode()) {
		...
		if (!mt_usb_is_device()) {
			...
			musb->is_clk_on = 0;
			os_printk(K_ALET, "%s, Host mode. directly return. is_clk_on %d\n", __func__, musb->is_clk_on);
			return;
		}
#endif
...
void mt_usb_connect(void)
{
	...
	if (_mu3d_musb) {
		...
		os_printk(K_WARNIN, "%s musb_musb not ready\n", __func__);
	}
	os_printk(K_ALET, "%s-\n", __func__);
}
...
int typec_switch_usb_connect(void *data)
{
	...
	if (musb && musb->gadget_driver) {
		...
		pr_warn("%s musb_musb not ready\n", __func__); 
	}
	pr_warn("%s-\n", __func__); 
#endif
	return 0;
}
EXPORT_SYMBOL_GPL(typec_switch_usb_connect);
...
EXPORT_SYMBOL_GPL(typec_switch_usb_disconnect);
#endif
...
{
	unsigned int cmode;
	struct musb *musb = dev ? dev_to_musb(dev) : NULL; 

	if (!dev) {
		os_printk(K_ERR, "dev is null!!\n");
		return count;
	} else if (1 == sscanf(buf, "%ud", &cmode)) {
		if (cmode >= 0 && cmode < CABLE_MODE_MAX) 
