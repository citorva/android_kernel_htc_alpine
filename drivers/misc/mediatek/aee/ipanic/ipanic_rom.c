...
const struct ipanic_dt_op ipanic_dt_ops[] = {
	...
	{"SYS_MINI_RDUMP", MRDUMP_MINI_BUF_SIZE, NULL},	
#ifdef CONFIG_MMPROFILE
...
static int ipanic_die(struct notifier_block *self, unsigned long cmd, void *ptr)
{
	...
	int aee_exp_type = aee_rr_curr_exp_type();
	pr_emerg("aee_rr_curr_exp_type:%d\n", aee_exp_type);
	sprint_symbol(sym_pc, dargs->regs->pc);
	sprint_symbol(sym_lr, (compat_user_mode(dargs->regs))? dargs->regs->compat_lr:dargs->regs->regs[30]);
	if (aee_exp_type != 1) {
		pr_emerg("Kernel panic - not syncing: %.*s PC:%s LR:%s\n", TASK_COMM_LEN, thread->task->comm, sym_pc, sym_lr);
	}
#endif
	aee_disable_api();
	...
	if (aee_exp_type == 1) {
		__mrdump_create_oops_dump(AEE_REBOOT_MODE_WDT, dargs->regs, "FIQ: Watchdog Timeout");
	}
	else if (aee_rr_curr_exp_type() == 2)
#ifdef CONFIG_MTK_AEE_MRDUMP
		__mrdump_create_oops_dump(AEE_REBOOT_MODE_KERNEL_OOPS, dargs->regs, "Kernel panic - not syncing: %.*s PC:%s LR:%s", TASK_COMM_LEN, thread->task->comm, sym_pc, sym_lr);
#else
