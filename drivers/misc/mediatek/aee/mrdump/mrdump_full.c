...
int mrdump_set_ramdump_reason(AEE_REBOOT_MODE reset_reason, char *msg)
{
	...
	printk(KERN_INFO "[%s] reboot_mode:%d\n", __func__, reset_reason);
		...
		if (0 == strcmp(msg, "Powerkey Hard Reset"))
			...
			snprintf(mrdump_cblock.crash_record.msg, sizeof(mrdump_cblock.crash_record.msg), "%s", msg);
		else
			
			snprintf(mrdump_cblock.crash_record.msg, sizeof(mrdump_cblock.crash_record.msg), "KP: %s", msg);
#ifdef CONFIG_HTC_REBOOT_REASON_INFORMATION
                htc_set_reboot_reason_ramdump(mrdump_cblock.crash_record.msg);
#endif
                break;
        case AEE_REBOOT_MODE_NESTED_EXCEPTION:
		snprintf(mrdump_cblock.crash_record.msg, sizeof(mrdump_cblock.crash_record.msg),
				"NESTED EXP: %s", msg);
#ifdef CONFIG_HTC_REBOOT_REASON_INFORMATION
                htc_set_reboot_reason_ramdump(mrdump_cblock.crash_record.msg);
#endif
...
static void __mrdump_reboot_va(AEE_REBOOT_MODE reboot_mode, struct pt_regs *regs, const char *msg, va_list ap)
{
	...
	char reset_messsage[128]={0};
	vsnprintf(reset_messsage, sizeof(reset_messsage), msg, ap);
#endif
	...
	printk(KERN_ERR "[%s] crash_record->reboot_mode = %d, reboot_mode = %d\n",__func__, crash_record->reboot_mode, reboot_mode);
...
#ifdef CONFIG_MTK_AEE_MRDUMP
	switch(crash_record->reboot_mode) {
		case AEE_REBOOT_MODE_KERNEL_PANIC:
			
			break;
		default:
			mrdump_set_ramdump_reason(reboot_mode, reset_messsage);
			break;
	}
#elif CONFIG_HTC_REBOOT_REASON_INFORMATION
	htc_set_reboot_reason_ramdump(crash_record->msg);
#endif
