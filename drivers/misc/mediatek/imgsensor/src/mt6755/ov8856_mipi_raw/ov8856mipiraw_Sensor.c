...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =	 
{
 ...
 { 3296, 2480,    0,	12, 3296, 2456, 3296, 2456,   4,	2, 3264, 2448,	 0, 8, 3264, 2432}, 
 { 3296, 2480,    0,	12, 3296, 2456, 3296, 2456,   4,	2, 3264, 2448,	 0, 8, 3264, 2432}, 
 { 3296, 2480,	  336,	272, 2624, 1936,  656,  484,   8,	2,  640,  480,	 0, 0,  640,  480}, 
 { 3296, 2480,	  0,	12, 3296, 2456, 1648, 1228,   2,	2, 1632, 1224,	 0, 0, 1632, 1224}   
#if 0
 { 1632, 1224,	  0,	0, 1632, 1224, 1632, 1224,   0,	0, 1632, 1224,	 0, 0, 1632, 1224}, 
 { 3264, 2448,	  0,	0, 3264, 2448, 3264, 2448,   0,	0, 3264, 2448,	 0, 0, 3264, 2448}, 
 { 3296, 2480,	  12,	12, 3272, 2456, 3264, 2452,   0,	0, 3264, 2448,	 0, 0, 3264, 2448}, 
 { 3296, 2480,	  12,	12, 3272, 2456,  640,  480,   0,	0,  640,  480,	 0, 0,  640,  480}, 
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	LOG_INF("gain = 0x%x , reg_gain = 0x%x\n ", gain, reg_gain);
...
static const char *ov8856Vendor = "GuangZhen";
static const char *ov8856NAME = "ov8856";
static const char *ov8856Size = "8M";
		...
		struct device_attribute *attr, char *buf)
{
	...
	sprintf(buf, "%s %s %s\n", ov8856Vendor, ov8856NAME, ov8856Size);
	ret = strlen(buf) + 1;
...
static struct kobject *android_ov8856;
static int first = true;
static int ov8856_sysfs_init(void)
{
	int ret ;
	if(first)
	{
		LOG_INF("kobject creat and add\n");
		android_ov8856 = kobject_create_and_add("android_camera2", NULL);
		if (android_ov8856 == NULL) {
			LOG_INF("subsystem_register failed\n");
			ret = -ENOMEM;
			return ret ;
		}
		LOG_INF("sysfs_create_file\n");
		ret = sysfs_create_file(android_ov8856, &dev_attr_sensor.attr);
		if (ret) {
			LOG_INF("sysfs_create_file failed\n");
			kobject_del(android_ov8856);
		}
		else
			first = false;
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	kal_uint8 i = 0;
	kal_uint8 retry = 2;
	printk("\n[ryf][ov8856]This is function get_imgsensor_id!\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			printk("\n[ryf][ov8856]This is function get_imgsensor_id!the value of sensor_id is 0x%x\n",*sensor_id);
...
#define RG_Ratio_Typical   0x13D
#define BG_Ratio_Typical   0x141
...
static kal_uint32 open(void)
{
	...
	printk("\n[ryf][ov8856]This is function open!\n");
						  ...
						  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_INF("Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",(imgsensor_info.cap.max_framerate)/10,(imgsensor_info.cap1.max_framerate)/10);
