...
{{5384, 3752, 0, 0, 5384, 3752, 2692, 1876, 0, 0, 2692, 1876, 2, 0, 2688, 1876},  
{ 5384, 3752, 0, 0, 5384, 3752, 5384, 3752, 0, 0, 5384, 3752, 4, 4, 5376, 3744},  
{ 5384, 3752, 0, 0, 5384, 3752, 3500, 2160, 0, 0, 3840, 2176, 0, 0, 3840, 2176}, 
...
{ 5384, 3752, 0, 0, 5384, 3752, 5384, 3752, 0, 0, 5384, 3752, 4, 362, 5376, 3028},  
{ 5384, 3752, 0, 0, 5384, 3752, 5384, 3752, 0, 0, 5384, 3752, 36, 0, 5312, 3752},  
{ 5384, 3752, 0, 0, 5384, 3752, 2692, 1876, 0, 0, 2692, 1876, 2, 0, 2688, 1876},  
};
...
static kal_uint32 return_sensor_id_ov4688(void)
{
    imgsensor.i2c_write_id=0x20;
    return ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
}
static kal_uint32 return_sensor_id_ov13850(void)
{
imgsensor.i2c_write_id=0x6C;
    return  ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
}
static kal_uint32 return_sensor_id_ov2722(void)
{
imgsensor.i2c_write_id=0x6C;
    return  ((read_cmos_sensor(0x300A) << 8) | read_cmos_sensor(0x300B));
}
static kal_uint32 return_sensor_id(void)
{
    ...
    if(cmos_id == 0x0)
    {
        ...
        {
            printk(" htc workround for T4KA7 no sensor_id module. \n");
...
{
    printk("T4KA7 Load Preview sensor setting \n");
...
{
    printk("T4KA7 Load Preview 24fps binning sensor setting \n");
    write_cmos_sensor(0x0100,0x00);
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                t4ka7_sysfs_init();
                read_otp();
                printk("tff: i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("tff: Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
...
static kal_uint32 control(MSDK_SCENARIO_ID_ENUM scenario_id, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    printk("t4ka7_scenario_id = %d ; preview=0 , capture=1 ,  stereo_mode=11(custim2) ; 16:9_id=10(custim1).\n", scenario_id);
    spin_lock(&imgsensor_drv_lock);
