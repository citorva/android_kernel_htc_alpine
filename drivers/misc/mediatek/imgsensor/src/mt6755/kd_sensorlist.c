...
#ifdef ALPINE_CAMERA
       struct regulator *regVCAMAF_2 = NULL;
#endif
...
#ifdef ALPINE_CAMERA
			regVCAMAF_2 = regulator_get(sensor_device, "vcama"); 
#endif

			 regSubVCAMD = regulator_get(sensor_device, "vcamd_sub"); 
			if (regSubVCAMD == NULL) {
#ifndef ALPINE_CAMERA
			    if (regVCAMA == NULL) {
	...
	reg = regSubVCAMA;
    	...
    	return ret;
			...
			if(type == AFVDD){
				...
				if (regulator_set_voltage(reg2 , powerVolt, powerVolt) != 0) {
					PK_ERR("[_hwPowerOn]fail to regulator_set_voltage, powertype:%d powerId:%d\n", type, powerVolt);
					return ret;
				}
				if (regulator_enable(reg2) != 0) {
...
bool _hwPowerDown(PowerType type)
{
    ...
    if(type == AFVDD){
	    if (!IS_ERR(reg2)) {
			if (regulator_is_enabled(reg2) != 0) {
				PK_DBG("[_hwPowerDown]%d is enabled\n", type);
				if (regulator_disable(reg2) != 0) {
					PK_ERR("[_hwPowerDown]fail to regulator_disable, powertype: %d\n\n", type);
					return ret;
				}
			}
