...
static int read_otp(void)
{
	...
	if(read_cmos_sensor(0x723E) != 0){
	    for(i=0x723E;i<=0x7247;i++)                                                                                                                 
	    {                                                                                                                                   
	       
	       LOG_INF("i = 0x%x, flag = 0x%x\n ", i, read_cmos_sensor(i));                                                            
	    } 
	}
	else{
        if (read_cmos_sensor(0x722F) != 0){
			for(i=0x722F;i<=0x7238;i++)                                                                                                                 
			{                                                                                                                                   
			...
			if (read_cmos_sensor(0x7220) != 0){
				for(i=0x7220;i<=0x7229;i++)                                                                                                                 
				{                                                                                                                                   
...
static void set_mirror_flip(kal_uint8 image_mirror)
{
    ...
    switch (image_mirror) {
				case IMAGE_NORMAL:
					write_cmos_sensor(0x3820,((read_cmos_sensor(0x3820) & 0xC7) | 0x10));
					
					break;
				case IMAGE_H_MIRROR:
					write_cmos_sensor(0x3820,((read_cmos_sensor(0x3820) & 0xC7) | 0x18));
...
static void hs_video_setting(void)
{
	...
	write_cmos_sensor(0x3803, 0x58); 
	write_cmos_sensor(0x3805, 0x9f); 
	write_cmos_sensor(0x3806, 0x09); 
	write_cmos_sensor(0x3807, 0x01); 
	write_cmos_sensor(0x3808, ((imgsensor_info.hs_video.grabwindow_width >> 8) & 0xFF)); 
	write_cmos_sensor(0x3809, (imgsensor_info.hs_video.grabwindow_width & 0xFF)); 
	write_cmos_sensor(0x380a, ((imgsensor_info.hs_video.grabwindow_height >> 8) & 0xFF)); 
	write_cmos_sensor(0x380b, (imgsensor_info.hs_video.grabwindow_height & 0xFF)); 
...
static const char *ov13855NAME = "ov13855_front";
static const char *ov13855Size = "13.0M";
static const char *ov13855Version1 = "ov13855_MP";
static const char *ov13855Version2 = "ov13855_ES";
...
static struct kobject *android_ov13855_1;
...
static int ov13855_sysfs_init(void)
{
	...
	if(first){
		...
		android_ov13855 = kobject_create_and_add("android_camera2", NULL);
	...
	if(second){
		LOG_INF("kobject creat and add\n");
		android_ov13855_1 = kobject_create_and_add("camera2_sensor_version", NULL);
		if (android_ov13855_1 == NULL) {
			LOG_INF(" subsystem_register " \
			"failed\n");
			ret = -ENOMEM;
			return ret ;
		...
		LOG_INF("sysfs_create_file\n");
		ret = sysfs_create_file(android_ov13855_1, &dev_attr_version.attr);
		if (ret) {
			LOG_INF("sysfs_create_file " \
			"failed\n");
			kobject_del(android_ov13855_1);
		}else
			second = false;
...
UINT32 OV13855_Front_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
