...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	OV13855_R2A_write_i2c(0x3d88, 0x72); 
	OV13855_R2A_write_i2c(0x3d89, 0x20);
	OV13855_R2A_write_i2c(0x3d8A, 0x73); 
	OV13855_R2A_write_i2c(0x3d8B, 0xBE);
	OV13855_R2A_write_i2c(0x3d81, 0x01); 
	Delay(10);
...
int apply_otp(struct otp_struct *otp_ptr)
{
	...
	if ((*otp_ptr).flag & 0x40) {
		...
		if (R_gain>0x400) {
			OV13855_R2A_write_i2c(0x5056, R_gain>>8);
			OV13855_R2A_write_i2c(0x5057, R_gain & 0x00ff);
		}

		if (G_gain>0x400) {
			OV13855_R2A_write_i2c(0x5058, G_gain>>8);
			OV13855_R2A_write_i2c(0x5059, G_gain & 0x00ff);
		}

		if (B_gain>0x400) {
			OV13855_R2A_write_i2c(0x505A, B_gain>>8);
			OV13855_R2A_write_i2c(0x505B, B_gain & 0x00ff);
		}
	}
	...
	if ((*otp_ptr).flag & 0x10) {
		...
		if(Decode_13855R2A((*otp_ptr).lenc,  lenc_out))
		{
			for(i=0;i<360 ;i++) {
				...
				OV13855_R2A_write_i2c(0x5200 + i, lenc_out[i]);
			}
		}
	}
	return 0;
