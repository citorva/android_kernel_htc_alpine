...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
extern int md_cd_enable_lwa_traffic_monitor(struct ccci_modem *md);
extern int md_cd_disable_lwa_traffic_monitor(struct ccci_modem *md);

unsigned int md_lwa_state = 0;
unsigned int md_lwa_monitor_enable = 1;

unsigned int port_ch_get_lwa_state(void)
{
	return md_lwa_state;
}
EXPORT_SYMBOL(port_ch_get_lwa_state);

int port_ch_set_lwa_state(struct ccci_modem *modem, unsigned int new_lwa_state)
{
	int r = 0;

	pr_info("[LWA monitor] %s: md_lwa_state=[%d], new_lwa_state=[%d]\n", __func__, md_lwa_state, new_lwa_state);

	switch ( new_lwa_state ) {
		case 0:
			md_lwa_state = new_lwa_state;
			
			md_cd_disable_lwa_traffic_monitor(modem);
			break;
		case 1:
			md_lwa_state = new_lwa_state;
			
			md_cd_enable_lwa_traffic_monitor(modem);
			break;
		default:
			break;
	}
...
EXPORT_SYMBOL(port_ch_set_lwa_state);

unsigned int port_ch_get_lwa_monitor_enable(void)
{
	return md_lwa_monitor_enable;
}
EXPORT_SYMBOL(port_ch_get_lwa_monitor_enable);

unsigned int port_ch_set_lwa_monitor_enable(struct ccci_modem *modem, unsigned int lwa_monitor_enable)
{
	int r = 0;
	md_lwa_monitor_enable = lwa_monitor_enable;
	if ( md_lwa_monitor_enable == 0 ) {
		r = port_ch_set_lwa_state(modem, 0);
	}
	pr_info("[LWA monitor] %s: r=[%d], md_lwa_monitor_enable=[%d], md_lwa_state=[%d]\n", __func__, r, md_lwa_monitor_enable, md_lwa_state);
	return r;
}
EXPORT_SYMBOL(port_ch_set_lwa_monitor_enable);

static unsigned int port_ch_is_lwa_enable(struct ccci_port *port, void *msg_buf, int len)
{
	int md_id;
	unsigned int r = -1;
	unsigned int new_lwa_state = 0;
	unsigned int lwa_monitor_enable = 0;
	ilm_struct* ilmMsg;
	struct ccci_modem *modem = NULL;

	lwa_monitor_enable = port_ch_get_lwa_monitor_enable();

	if ( lwa_monitor_enable != 1 ) {
		pr_info("[LWA monitor] %s: lwa_monitor_enable=[%d]\n", __func__, lwa_monitor_enable);
		goto error;
	}

	if ( port == NULL ) {
		pr_err("[LWA monitor] %s: port is null\n", __func__);
		goto error;
	}

	if ( port->modem == NULL ) {
		pr_err("[LWA monitor] %s: modem is null\n", __func__);
		goto error;
	}
	...
	if ( msg_buf == NULL ) {
		pr_err("[LWA monitor] %s: msg is null\n", __func__);
		goto error;
	}

	ilmMsg = (ilm_struct*)msg_buf;

	switch (ilmMsg->msg_id & 0xFF) {
		case MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_CNF & 0xFF:
			do {
				waal_errc_wlan_lwa_connect_cnf_struct* connRes;
				connRes = (waal_errc_wlan_lwa_connect_cnf_struct*)(msg_buf + sizeof(ilm_struct));
				if ( connRes->result == WAAL_ERRC_WLAN_LWA_CONNECTION_SUCCESS ) {
					new_lwa_state = 1;
					r = 1;
				} else {
					new_lwa_state = 0;
				...
				port_ch_set_lwa_state(modem, new_lwa_state);
			} while ( 0 );
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_CNF & 0xFF:
		case MSG_ID_LWAD_WAAL_WLAN_LWA_LOST_IND & 0xFF:
			new_lwa_state = 0;
			r = 1;
			port_ch_set_lwa_state(modem, new_lwa_state);
			break;
	...
	if ( r == 1 )
		pr_info("[LWA monitor] %s: r=[%d], lwa_state=[%d], new_lwa_state=[%d]\n", __func__, r, port_ch_get_lwa_state(), new_lwa_state);
error:
	return r;
}
...
static void hexdump(int md_id,  const unsigned char *buf, unsigned short len )
{
	...
	if ( ofs / 16 > 10 ) {
		CCCI_NORMAL_LOG(md_id, CHAR, "port_ch_dump_lwa: only dump first 160 bytes\n");
		break;
	}
	...
	CCCI_NORMAL_LOG(md_id, CHAR, "port_ch_dump_lwa: %s\n", str);

    }
}

static void port_ch_dump_lwa(struct ccci_port *port, char *str, void *msg_buf, int len)
{
	int md_id;
	ilm_struct* ilmMsg;
	...
	md_id = port->modem->index;

	if ( msg_buf == NULL ) {
		CCCI_ERROR_LOG(md_id, CHAR, "%s: msg is null\n", __func__);
		goto error;
	}

	ilmMsg = (ilm_struct*)msg_buf;

	CCCI_NORMAL_LOG(md_id, CHAR, "%s: [%s] md_id=[%d], len=[%d]\n", __func__, str, md_id, len);
	CCCI_NORMAL_LOG(md_id, CHAR, "%s: ilmMsg->src_mod_id=[%d], ilmMsg->dest_mod_id=[%d], ilmMsg->sap_id=[%d], ilmMsg->msg_id=[0x%x], ilmMsg->local_para_ptr=[0x%x], ilmMsg->peer_buff_ptr=[%d]\n", __func__, ilmMsg->src_mod_id, ilmMsg->dest_mod_id, ilmMsg->sap_id, ilmMsg->msg_id & 0xFF, ilmMsg->local_para_ptr, ilmMsg->peer_buff_ptr);

	port_ch_is_lwa_enable(port, ilmMsg, len);

	switch (ilmMsg->msg_id & 0xFF) {
		case MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_REQ & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_REQ[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_REQ & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_IND & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_IND[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_CAPABILITY_IND & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_REQ & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_REQ[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_REQ & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_CNF & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_CNF[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_LWA_CONNECT_CNF & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_REQ & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_REQ[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_REQ & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_CNF & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_CNF[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_LWA_DISCONNECT_CNF & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_LWA_LOST_IND & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_LWA_LOST_IND[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_LWA_LOST_IND & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_REQ & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_REQ[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_REQ & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_IND & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_IND[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_IND & 0xFF);
			break;
		case MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_STOP_REQ & 0xFF:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_STOP_REQ[0x%x]\n", __func__, MSG_ID_LWAD_WAAL_WLAN_MEASUREMENT_STOP_REQ & 0xFF);
			break;
		default:
			CCCI_NORMAL_LOG(md_id, CHAR, "%s: msg->id=[0x%x][0x%x]\n", __func__, ilmMsg->msg_id, ilmMsg->msg_id & 0xFF);
			break;
                }

	hexdump(md_id, msg_buf, len);
...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
	if ( port->rx_ch == CCCI_IPC_RX && port->minor == AP_IPC_LWAPROXY + CCCI_IPC_MINOR_BASE )
		port_ch_dump_lwa(port, "chr_read", req->skb->data, read_len);
#endif
...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
		if ( port->tx_ch == CCCI_IPC_TX && port->minor == AP_IPC_LWAPROXY + CCCI_IPC_MINOR_BASE )
			port_ch_dump_lwa(port, "chr_write", req->skb->data + sizeof(struct ccci_header), actual_count);
#endif
...
static long dev_char_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CCCI_IOC_LEAVE_DEEP_FLIGHT:
		...
		md->flight_mode = MD_FIGHT_MODE_LEAVE; 
	...
	case CCCI_IOC_GET_MD_IMG_EXIST:
		md_type = get_md_type_from_lk(md->index); 
		if (md_type) {
