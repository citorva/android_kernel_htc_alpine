...
#define EX_EE_WHOLE_TIMEOUT  (EX_TIMER_SWINT + EX_TIMER_MD_EX + EX_TIMER_MD_EX_REC_OK + 2) 
...
typedef struct _ex_exp_PL_log_t {
	EX_PL_HEADER_T header; 
	char sw_ver[32]; 
	char sw_project_name[32];  
	char sw_flavor[32];
	char sw_buildtime[16];
	EX_PL_ENVINFO_T envinfo;
	EX_PL_DIAGNOSISINFO_T diagnoinfo; 
...
{
	CCCI_REPEAT_ALWAYS_LOG(md->index, CORE, "traffic(0): tx:[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld rx:[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld\n",
			CCCI_CONTROL_TX, md->logic_ch_pkt_cnt[CCCI_CONTROL_TX],
			CCCI_SYSTEM_TX, md->logic_ch_pkt_cnt[CCCI_SYSTEM_TX],
			CCCI_STATUS_TX, md->logic_ch_pkt_cnt[CCCI_STATUS_TX],
			CCCI_UART1_TX, md->logic_ch_pkt_cnt[CCCI_UART1_TX],
			CCCI_RPC_TX, md->logic_ch_pkt_cnt[CCCI_RPC_TX],
			CCCI_IPC_TX, md->logic_ch_pkt_cnt[CCCI_IPC_TX],
			...
			CCCI_SYSTEM_RX, md->logic_ch_pkt_cnt[CCCI_SYSTEM_RX],
			CCCI_STATUS_RX, md->logic_ch_pkt_cnt[CCCI_STATUS_RX],
			CCCI_UART1_RX, md->logic_ch_pkt_cnt[CCCI_UART1_RX],
			CCCI_RPC_RX, md->logic_ch_pkt_cnt[CCCI_RPC_RX],
			CCCI_IPC_RX, md->logic_ch_pkt_cnt[CCCI_IPC_RX],
			CCCI_IPC_UART_RX, md->logic_ch_pkt_cnt[CCCI_IPC_UART_RX],
			CCCI_MD_LOG_RX, md->logic_ch_pkt_cnt[CCCI_MD_LOG_RX]
			);
	CCCI_REPEAT_ALWAYS_LOG(md->index, CORE, "traffic(ch): tx:[%d]%ld, [%d]%ld, [%d]%ld rx:[%d]%ld, [%d]%ld, [%d]%ld\n",
