...
static struct c2k_port c2k_ports[] = {
	...
	{DATA_PPP_CH_C2K, DATA_PPP_CH_C2K, CCCI_C2K_PPP_DATA, CCCI_C2K_PPP_DATA,},	
...
void md_cd_traffic_monitor_func_c2k(unsigned long data)
{
	struct ccci_modem *md = (struct ccci_modem *)data;
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)md->private_data;

	CCCI_REPEAT_LOG(md->index, TAG,
		     "traffic_c2k(%d):Tx %d,%d,%d,%d,%d,%d,%d,%d:Rx %d,%d,%d,%d,%d,%d,%d,%d\n",
			md->md_state,
	...
	CCCI_REPEAT_ALWAYS_LOG(md->index, CORE,
		     "traffic_c2k(ch): tx:[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld rx:[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld,[%d]%ld\n",
			CCCI_CONTROL_TX, md->logic_ch_pkt_cnt[CCCI_CONTROL_TX],
			CCCI_STATUS_TX, md->logic_ch_pkt_cnt[CCCI_STATUS_TX],
			CCCI_UART1_TX, md->logic_ch_pkt_cnt[CCCI_UART1_TX],
			CCCI_PCM_TX, md->logic_ch_pkt_cnt[CCCI_PCM_TX],
			CCCI_MD_LOG_TX, md->logic_ch_pkt_cnt[CCCI_MD_LOG_TX],
			CCCI_FS_TX, md->logic_ch_pkt_cnt[CCCI_FS_TX],
			CCCI_IPC_UART_TX, md->logic_ch_pkt_cnt[CCCI_IPC_UART_TX],
			CCCI_CONTROL_RX, md->logic_ch_pkt_cnt[CCCI_CONTROL_RX],
			CCCI_STATUS_RX, md->logic_ch_pkt_cnt[CCCI_STATUS_RX],
			CCCI_UART1_RX, md->logic_ch_pkt_cnt[CCCI_UART1_RX],
			CCCI_PCM_RX, md->logic_ch_pkt_cnt[CCCI_PCM_RX],
			CCCI_MD_LOG_RX, md->logic_ch_pkt_cnt[CCCI_MD_LOG_RX],
			CCCI_FS_RX, md->logic_ch_pkt_cnt[CCCI_FS_RX],
			CCCI_IPC_UART_RX, md->logic_ch_pkt_cnt[CCCI_IPC_UART_RX]);

	CCCI_REPEAT_ALWAYS_LOG(md->index, CORE,
		     "traffic_c2k(c2k): [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld, [%d]%ld\n",
			CCCI_C2K_PPP_DATA, md->logic_ch_pkt_cnt[CCCI_C2K_PPP_DATA],
	...
	CCCI_REPEAT_ALWAYS_LOG(md->index, CORE,
		     "traffic_c2k(net): tx: [%d]%ld, [%d]%ld, [%d]%ld, rx:[%d]%ld, [%d]%ld, [%d]%ld\n",
			CCCI_CCMNI1_TX, md->logic_ch_pkt_cnt[CCCI_CCMNI1_TX],
			CCCI_CCMNI2_TX, md->logic_ch_pkt_cnt[CCCI_CCMNI2_TX],
			CCCI_CCMNI3_TX, md->logic_ch_pkt_cnt[CCCI_CCMNI3_TX],
...
static void md_cd_clear_traffic_data_c2k(struct ccci_modem *md)
{
	struct md_ccif_ctrl *md_ctrl = (struct md_ccif_ctrl *)md->private_data;
			   ...
			   int blocking, int *result)
{
	...
	while (1) {
		...
		if (ret >= 0 || ret == -CCCI_ERR_DROP_PACKET) {
			...
			if ((ccci_hdr.channel & 0xFF) < CCCI_MAX_CH_NUM)
				md->logic_ch_pkt_cnt[ccci_hdr.channel]++;
#endif
		} else {
...
static int ccif_queue_broadcast_state(struct ccci_modem *md, MD_STATE state, DIRECTION dir, int index)
{
...
static void ccif_rx_work(struct work_struct *work)
{
	...
	if (ret == -EAGAIN) {
		...
		ccif_queue_broadcast_state(queue->modem, RX_FLUSH, IN, queue->index);
	}
}
...
#ifdef TRAFFIC_MONITOR_INTERVAL_C2K
	md_cd_clear_traffic_data_c2k(md);
#endif
				   ...
				   struct sk_buff *skb)
{
...
#ifdef TRAFFIC_MONITOR_INTERVAL_C2K
	if ((jiffies - md_ctrl->traffic_stamp) / HZ >= TRAFFIC_MONITOR_INTERVAL_C2K) {
 ...
 retry:
	...
	if (ccci_ringbuf_writeable(md->index, queue->ringbuf, skb->len) > 0) {
		...
		if ((c2k_ch_to_ccci_ch(md, ccci_h->channel, OUT) & 0xFF) < CCCI_MAX_CH_NUM)
			md->logic_ch_pkt_cnt[c2k_ch_to_ccci_ch(md, ccci_h->channel, OUT)]++;
#endif
	} else {
		spin_unlock_irqrestore(&queue->tx_lock, flags);
		if (queue->debug_id == 0) {
...
static ssize_t md_cd_control_show(struct ccci_modem *md, char *buf)
{
	...
	count = snprintf(buf, 256, "support: ccif_assert\n");
	return count;
}
...
static int md_ccif_probe(struct platform_device *dev)
{
	...
	md_ctrl->traffic_monitor.function = md_cd_traffic_monitor_func_c2k;
