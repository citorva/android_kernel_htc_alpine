...
void ccci_set_mem_access_protection(struct ccci_modem *md)
{
	...
	case MD_SYS3:
		...
		emi_mpu_set_region_protection(shr_mem13_phy_start,	
					      shr_mem13_phy_end,	
					      shr_mem13_mpu_id,	
					      shr_mem13_mpu_attr);
	...
	case MD_SYS1:
		if (modem_run_env_ready(MD_SYS1)) {
			...
			region_mpu_start = (unsigned int)md_layout->md_region_phy +
				img_info->rmpu_info.region_info[2].region_offset; 
			region_mpu_end =
				((region_mpu_start + img_info->rmpu_info.region_info[2].region_size 
				 + 0xFFFF)&(~0xFFFF)) - 0x1;

			CCCI_BOOTUP_LOG(md->index, TAG, "Start MPU protect region <%d:%08X:%08X> %X\n",
				region_mpu_id, region_mpu_start, region_mpu_end, region_mpu_attr);
			emi_mpu_set_region_protection(region_mpu_start,	
						      region_mpu_end,	
						      region_mpu_id,	
						      region_mpu_attr);
...
void ccci_set_mem_access_protection_second_stage(struct ccci_modem *md)
{
	...
	case MD_SYS1:
		img_info = &md->img_info[IMG_MD];
		md_layout = &md->mem_layout;
		region_id = MD_SET_REGION_MD1_MCURO_HWRW;
