...
#define NET_TX_QUEUE_MASK 0x38	
#define NET_RX_QUEUE_MASK 0x38	
#define NAPI_QUEUE_MASK NET_RX_QUEUE_MASK	
#define NORMAL_TX_QUEUE_MASK 0xCF	
#define NORMAL_RX_QUEUE_MASK 0xCF	
...
#define LWA_BYTES_TO_MBPS (1024 * 1024 / 8)
#define LWA_HIGH_THRESHOLD 200;
#define LWA_LOW_THRESHOLD 180;
#define LWA_HIGH_THRESHOLD2 320;
#define LWA_LOW_THRESHOLD2 300;
extern int pnpmgr_lwa_gear_set(int val);
extern int pnpmgr_lwa_gear_get(void);

int md_cd_lwa_send_lock_thread_command(struct ccci_modem *md, int command, int pid)
{
	int r = 0;
	struct cpumask cmask_new;

	if ( command == 0 )
		cpumask_full(&cmask_new);
	else if ( command == 1 )
		cmask_new.bits[0] = 0x01;

	r = sched_setaffinity(pid, &cmask_new);

	pr_info("[LWA monitor]%s: command=[%d], pid=[%d], r=[%d]\n", __func__, command, pid, r);
	return r;
}

int md_cd_lwa_send_pnpmgr_command(int command)
{
	int r = 0;

	if ( command >= LWA_GEAR_LEVEL_MAX || command < LWA_GEAR_LEVEL0) {
		pr_info("[LWA monitor]%s: command(%d) not support, r=[%d]\n", __func__, command, r);
		return r;
	}

	pnpmgr_lwa_gear_set(command);

	pr_info("[LWA monitor]%s: command=[%d], r=[%d]\n", __func__, command, r);
	return r;
}

int md_cd_lwa_send_enable_cpu_lock_command(struct ccci_modem *md, int lwa_command)
{
	int r = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	struct md_cd_queue *queue = NULL;

	pr_info("[LWA monitor] %s: lwa_command=[%d]\n", __func__, lwa_command);

	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	if ( md_ctrl == NULL ) {
		pr_info("[LWA monitor] %s: md_ctrl == NULL\n", __func__);
		goto error;
	}

	queue = &md_ctrl->rxq[3];
	if ( queue == NULL ) {
		pr_info("[LWA monitor] %s: queue == NULL\n", __func__);
		goto error;
	}

	
	md_cd_lwa_send_pnpmgr_command(lwa_command);

	
	switch ( lwa_command ) {
		case LWA_GEAR_LEVEL0:
			md_cd_lwa_send_lock_thread_command( md, 0, queue->rx_thread->pid );
			break;
		case LWA_GEAR_LEVEL1:
		case LWA_GEAR_LEVEL2:
			md_cd_lwa_send_lock_thread_command( md, 1, queue->rx_thread->pid );
			break;
		default:
			pr_info("[LWA monitor] %s: lwa_command not support\n", __func__);
			break;
	}
error:
	return r;
...
int md_cd_lwa_check_lock_thread(int pid)
{
	int r = 0;
	struct cpumask cmask_curr;
	r = sched_getaffinity(pid, &cmask_curr);

	
	if ( cmask_curr.bits[0] == 0x01 ) {
		r = 1;
	} else {
		pr_info("[LWA monitor]%s: pid=[%d], cmask_curr=[%ld]\n", __func__, pid, cmask_curr.bits[0]);
		r = 0;
	}
	return r;
}

int md_cd_lwa_check_lock_cpu(void)
{
	int r = 0;
	int cpu_online_curr[8] = {0};

	cpu_online_curr[0] = cpu_online(0);
	cpu_online_curr[1] = cpu_online(1);
	cpu_online_curr[2] = cpu_online(2);
	cpu_online_curr[3] = cpu_online(3);
	cpu_online_curr[4] = cpu_online(4);
	cpu_online_curr[5] = cpu_online(5);
	cpu_online_curr[6] = cpu_online(6);
	cpu_online_curr[7] = cpu_online(7);

	
	if ( cpu_online_curr[0] == 1
		&& cpu_online_curr[1] == 1
		&& cpu_online_curr[2] == 1
		&& cpu_online_curr[3] == 1
		&& cpu_online_curr[4] == 1
		...
		pr_info("[LWA monitor] %s: cpu_online_curr=[%d, %d, %d, %d, %d, %d, %d, %d]\n", __func__, cpu_online_curr[0], cpu_online_curr[1], cpu_online_curr[2], cpu_online_curr[3], cpu_online_curr[4], cpu_online_curr[5], cpu_online_curr[6], cpu_online_curr[7]);
	}
	return r;
}

void md_cd_lwa_enable_cup_lock(struct ccci_modem *md, int enable_cup_lock)
{
	int lwa_state = 0;
	struct md_cd_ctrl *md_ctrl = NULL;

	lwa_state = port_ch_get_lwa_state();
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	if ( md_ctrl == NULL ) {
		pr_info("[LWA monitor] %s: md_ctrl == NULL\n", __func__);
		return;
	...
	if ( enable_cup_lock >= LWA_GEAR_LEVEL_MAX || enable_cup_lock < LWA_GEAR_LEVEL0) {
		pr_info("[LWA monitor] %s: enable_cup_lock(%d) wrong value\n", __func__, enable_cup_lock);
		return;
	}

	if ( enable_cup_lock == LWA_GEAR_LEVEL0 || lwa_state == LWA_STATE_DISABLE) {
		
		if ( md_ctrl->enable_cup_lock != LWA_GEAR_LEVEL0 ) {
			md_cd_lwa_send_enable_cpu_lock_command(md, LWA_GEAR_LEVEL0);
			md_ctrl->enable_cup_lock = LWA_GEAR_LEVEL0;
		}
	} else if ( enable_cup_lock > LWA_GEAR_LEVEL0 && lwa_state == LWA_STATE_ENABLE ) {

		if ( md_ctrl->enable_cup_lock > LWA_GEAR_LEVEL0 ) {
			int curr_pnpmgr_lwa_gear_state = 0;
			curr_pnpmgr_lwa_gear_state = pnpmgr_lwa_gear_get();
			
			if ( curr_pnpmgr_lwa_gear_state == LWA_GEAR_LEVEL0 ) {
				
				pr_info("[LWA monitor] %s: curr_pnpmgr_lwa_gear_state = 0\n", __func__);
				md_ctrl->enable_cup_lock = LWA_GEAR_LEVEL0;
			}
		}
		...
		do {
			if ( md_ctrl->enable_cup_lock > LWA_GEAR_LEVEL0 ) {
				...
				is_thread_locked = md_cd_lwa_check_lock_thread( queue->rx_thread->pid );
				is_cpu_locked = md_cd_lwa_check_lock_cpu();
				if ( is_thread_locked == 0 || is_cpu_locked== 0 ) {
					pr_info("[LWA monitor] %s: is_thread_locked=[%d], is_cpu_locked=[%d], lwa_pnp_cpu_lock_fail_count=[%d]\n", __func__, is_thread_locked, is_cpu_locked, md_ctrl->lwa_pnp_cpu_lock_fail_count);
					
					md_ctrl->enable_cup_lock = LWA_GEAR_LEVEL0;
				}
				if ( is_cpu_locked == 0 ) {
					
					if ( md_ctrl->lwa_pnp_cpu_lock_fail_count >= 3 ) {
						md_ctrl->lwa_pnp_cpu_lock_fail_count = 0;
						md_cd_lwa_send_pnpmgr_command(LWA_GEAR_LEVEL0);
					} else {
						md_ctrl->lwa_pnp_cpu_lock_fail_count++;
					}
				} else {
					md_ctrl->lwa_pnp_cpu_lock_fail_count = 0;
				}
		...
		if ( md_ctrl->enable_cup_lock == LWA_GEAR_LEVEL0 ) {
			...
			md_cd_lwa_send_enable_cpu_lock_command(md, enable_cup_lock);
			md_ctrl->enable_cup_lock = enable_cup_lock;
		}
	} else {
		pr_info("[LWA monitor] %s: enable_cup_lock=[%d], lwa_state=[%d]\n", __func__, enable_cup_lock, lwa_state);
	}
}

static void md_cd_lwa_traffic_check_work(struct work_struct *work)
{
	u64 byte_total = 0;
	u64 byte_now = 0;
	u64 byte_last = 0;
	unsigned int lwa_rx_threshold_high = 0;
	unsigned int lwa_rx_threshold_low = 0;
	unsigned int lwa_rx_threshold_high2 = 0;
	unsigned int lwa_rx_threshold_low2 = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	...
	md_ctrl = container_of(work, struct md_cd_ctrl, lwa_traffic_monitor_work);
	if ( md_ctrl == NULL ) {
		pr_info("[LWA monitor] %s: md_ctrl is NULL\n", __func__);
		return;
	}

	md = md_ctrl->modem;
	if ( md == NULL ) {
		pr_info("[LWA monitor] %s: md is NULL\n", __func__);
		return;
	}

	ctlb = ccmni_ctl_blk[0];
	if ( ctlb == NULL ) {
		pr_info("[LWA monitor] %s: ctlb is NULL\n", __func__);
		return;
	}

	ccmni = ctlb->ccmni_inst[0];
	if ( ccmni == NULL ) {
		pr_info("[LWA monitor] %s: ccmni is NULL\n", __func__);
		return;
	}

	dev = ccmni->dev;
	if ( dev == NULL ) {
		pr_info("[LWA monitor] %s: dev is NULL\n", __func__);
		return;
	}

	lwa_rx_threshold_high = md_ctrl->lwa_rx_threshold_high;
	lwa_rx_threshold_low = md_ctrl->lwa_rx_threshold_low;
	lwa_rx_threshold_high2 = md_ctrl->lwa_rx_threshold_high2;
	lwa_rx_threshold_low2 = md_ctrl->lwa_rx_threshold_low2;

	if ( port_ch_get_lwa_state() >= 1 ) {
		int current_Mbps = 0;
		int enable_cup_lock = LWA_GEAR_LEVEL0;
		byte_last = md_ctrl->lwa_last_rx_bytes;
		byte_now = dev->stats.rx_bytes;
		byte_total = byte_now - byte_last;

		
		md_ctrl->lwa_last_rx_bytes = dev->stats.rx_bytes;
		md_ctrl->lwa_rx_bytes = dev->stats.rx_bytes;
		
		current_Mbps = byte_total / LWA_BYTES_TO_MBPS;
		
		enable_cup_lock = md_ctrl->enable_cup_lock;

		if ( md_ctrl->lwa_enable_threshold_v2 == 1 ) {
			if ( enable_cup_lock ==  LWA_GEAR_LEVEL0 ) {
				
				if ( current_Mbps >= lwa_rx_threshold_high2 ) {
					enable_cup_lock = LWA_GEAR_LEVEL2;
				} else if ( current_Mbps >= lwa_rx_threshold_high ) {
					enable_cup_lock = LWA_GEAR_LEVEL1;
				}
			} else if ( enable_cup_lock ==  LWA_GEAR_LEVEL1 ) {
				
				
				if ( current_Mbps >= lwa_rx_threshold_high2 ) {
					enable_cup_lock = LWA_GEAR_LEVEL2;
				} else if ( current_Mbps < lwa_rx_threshold_low ) {
					enable_cup_lock = LWA_GEAR_LEVEL0;
				}
			} else if ( enable_cup_lock ==  LWA_GEAR_LEVEL2 ) {
				
				
				if ( current_Mbps < lwa_rx_threshold_low ) {
					enable_cup_lock = LWA_GEAR_LEVEL0;
				} else if ( current_Mbps < lwa_rx_threshold_low2 ) {
					enable_cup_lock = LWA_GEAR_LEVEL1;
				}
			}
		} else {
			if ( current_Mbps >= lwa_rx_threshold_high ) {
				enable_cup_lock = LWA_GEAR_LEVEL1;
			} else if ( current_Mbps < lwa_rx_threshold_low ) {
				enable_cup_lock = LWA_GEAR_LEVEL0;
			}
		}
		pr_info("[LWA monitor] %s: dev=[%s], byte_last=[%llu], byte_now=[%llu], byte_total=[%llu], current_Mbps=[%d], enable_cup_lock=[%d]\n", __func__, dev->name, byte_last, byte_now, byte_total, current_Mbps, enable_cup_lock);

		md_cd_lwa_enable_cup_lock(md, enable_cup_lock);

		mod_timer( &md_ctrl->lwa_traffic_monitor, jiffies + 1 * HZ );
	} else {
		
		md_cd_lwa_enable_cup_lock(md, 0);
	}
}

void md_cd_lwa_traffic_monitor_func(unsigned long data)
{
	struct ccci_modem *md = NULL;
	struct md_cd_ctrl *md_ctrl = NULL;

	if ( data == 0 ) {
		pr_info("[LWA monitor] %s: data is NULL\n", __func__);
		return;
	}

	md = (struct ccci_modem *)data;
	if ( md == NULL ) {
		pr_info("[LWA monitor] %s: md is NULL\n", __func__);
		return;
	}
	...
	schedule_work(&md_ctrl->lwa_traffic_monitor_work);
}

int md_cd_enable_lwa_traffic_monitor(struct ccci_modem *md)
{
	int r = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	ccmni_ctl_block_t *ctlb = NULL;
	...
	md_ctrl->lwa_last_rx_bytes = dev->stats.rx_bytes;
	md_ctrl->lwa_rx_bytes = dev->stats.rx_bytes;
	pr_info("[LWA monitor] %s: dev=[%s], lwa_last_rx_bytes=[%llu], lwa_rx_bytes=[%llu]\n", __func__, dev->name, md_ctrl->lwa_last_rx_bytes, md_ctrl->lwa_rx_bytes);

	mod_timer( &md_ctrl->lwa_traffic_monitor, jiffies + 1 * HZ );
	return r;
}
EXPORT_SYMBOL(md_cd_enable_lwa_traffic_monitor);

int md_cd_disable_lwa_traffic_monitor(struct ccci_modem *md)
{
	int r = 0;
	return r;
}
EXPORT_SYMBOL(md_cd_disable_lwa_traffic_monitor);
	...
	*rxbytes = 0;
	...
	while (1) {
		...
		if (atomic_cmpxchg(&md->wakeup_src, 1, 0) == 1)
			CCCI_ERROR_LOG(md->index, TAG, "CLDMA_MD wakeup source:(%d/%d)\n",
...
static int cldma_net_rx_push_thread(void *arg)
{
	...
	while (1) {
		if (skb_queue_empty(&queue->skb_list.skb_list)) {
			cldma_queue_broadcast_state(md, RX_FLUSH, IN, queue->index);
			count = 0;
		...
		for (i = 0; i < QUEUE_LEN(md_ctrl->rxq); i++) {
			if (L2RISAR0 & CLDMA_BM_INT_DONE & (1 << i)) {
				...
				cldma_write32(md_ctrl->cldma_ap_ao_base, CLDMA_AP_L2RIMSR0,
					      CLDMA_BM_ALL_QUEUE & (1 << i));
				cldma_read32(md_ctrl->cldma_ap_ao_base, CLDMA_AP_L2RIMSR0); 
...
static void md_cd_wdt_work(struct work_struct *work)
{
	...
	ccci_write32(md_ctrl->md_pll_base->md_busreg1, 0x94, 0xE7C5);
...
static int md_cd_start(struct ccci_modem *md)
{
	...
	if (md->config.setting & MD_SETTING_FIRST_BOOT) {
        ...
        memset_io_raw(md->mem_layout.smem_region_vir, 0, md->mem_layout.smem_region_size);
...
static void config_ap_runtime_data_v2_1(struct ccci_modem *md, struct ap_query_md_feature_v2_1 *ap_feature)
{
	...
	ap_feature->noncached_mpu_start_addr = md->smem_layout.ccci_exp_smem_base_phy
						- md->mem_layout.smem_offset_AP_to_MD;
	ap_feature->noncached_mpu_total_size = md->mem_layout.smem_region_size - md->smem_layout.ccci_lwa_smem_size
						- md->smem_layout.ccci_smart_logging_size
						+ md->mem_layout.md1_md3_smem_size;
	ap_feature->cached_mpu_start_addr = md->smem_layout.ccci_smart_logging_base_phy
						- md->mem_layout.smem_offset_AP_to_MD;
	ap_feature->cached_mpu_total_size =  md->smem_layout.ccci_smart_logging_size
						+ md->smem_layout.ccci_lwa_smem_size;
#else
	...
	ap_feature->noncached_mpu_total_size = md->mem_layout.smem_region_size - md->smem_layout.ccci_lwa_smem_size
						+ md->mem_layout.md1_md3_smem_size;
	ap_feature->cached_mpu_start_addr = md->smem_layout.ccci_lwa_smem_base_phy
						- md->mem_layout.smem_offset_AP_to_MD;
	ap_feature->cached_mpu_total_size = md->smem_layout.ccci_lwa_smem_size;
#endif
#else
#ifdef SMART_LOGGING_SUPPORT
	...
	ap_feature->noncached_mpu_start_addr = md->smem_layout.ccci_exp_smem_base_phy
						- md->mem_layout.smem_offset_AP_to_MD;
	ap_feature->noncached_mpu_total_size = md->mem_layout.smem_region_size
						- md->smem_layout.ccci_smart_logging_size
						+ md->mem_layout.md1_md3_smem_size;
	ap_feature->cached_mpu_start_addr = md->smem_layout.ccci_smart_logging_base_phy
						- md->mem_layout.smem_offset_AP_to_MD;
...
static int md_cd_send_runtime_data_v2(struct ccci_modem *md, unsigned int sbp_code)
{
	...
	if (md->multi_md_mpu_support) {
		ap_rt_data_v2_1 = (struct ap_query_md_feature_v2_1 *)(req->skb->data + sizeof(struct ccci_header));
	...
	ret = md->ops->send_request(md, 0, req, NULL);	
...
static int md_cd_send_runtime_data(struct ccci_modem *md, unsigned int sbp_code)
{
	...
	runtime->feature_6_val[2] = current_time_zone;	
	runtime->feature_6_val[3] = sys_tz.tz_dsttime;	
	...
	case CCCI_MESSAGE:
		...
		if (req) {
			...
			*(((u32 *) ccci_h) + 2) = CCCI_FORCE_ASSERT_CH;
			...
			return md->ops->send_request(md, 0, req, NULL);	
		}
		return -CCCI_ERR_ALLOCATE_MEMORY_FAIL;
	case CCIF_INTERRUPT:
...
static int md_cd_dump_info(struct ccci_modem *md, MODEM_DUMP_FLAG flag, void *buff, int length)
{
	...
	if (flag & DUMP_FLAG_CCIF) {
		...
		unsigned char ccif_sram[CCCC_SMEM_CCIF_DUMP_SIZE] = { 0 };
...
static ssize_t md_cd_filter_store(struct ccci_modem *md, const char *buf, size_t count)
{
	...
	if ( strlen(buf) > 18 ) {
		CCCI_ERROR_LOG(md->index, TAG, "command fail\n");
		return 0;
	}
...
static ssize_t md_lwa_state_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;

	count += snprintf(buf, 128, "md_lwa_state=%d\n", port_ch_get_lwa_state());
	return count;
}

static ssize_t md_lwa_state_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_status = 0;
	ret = kstrtoint(buf, 10, &lwa_status);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_status retrun fail: %d\n", __func__, ret);
		return 0;
	}

	pr_info("[LWA monitor] %s: lwa_status=[%d]\n", __func__, lwa_status);
	if ( lwa_status == LWA_STATE_ENABLE ) {
		port_ch_set_lwa_state(md, LWA_STATE_ENABLE);
	} else {
		port_ch_set_lwa_state(md, LWA_STATE_DISABLE);
	}

	return count;
}

static ssize_t md_lwa_monitor_enable_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;

	count += snprintf(buf, 128, "md_lwa_monitor_enable=%d\n", port_ch_get_lwa_monitor_enable());
	return count;
}

static ssize_t md_lwa_monitor_enable_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_enable = 0;
	ret = kstrtoint(buf, 10, &lwa_enable);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_enable retrun fail: %d\n", __func__, ret);
		return 0;
	}

	pr_info("[LWA monitor] %s: lwa_enable=[%d]\n", __func__, lwa_enable);
	if ( lwa_enable == 0) {
		port_ch_set_lwa_monitor_enable(md, 0);
	} else if ( lwa_enable == 1) {
		port_ch_set_lwa_monitor_enable(md, 1);
	}

	return count;
}

static ssize_t md_lwa_monitor_threshold_high_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;

	count += snprintf(buf, 128, "lwa_rx_threshold_high=%d\n", md_ctrl->lwa_rx_threshold_high);

	return count;
}

static ssize_t md_lwa_monitor_threshold_high_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_rx_threshold_high = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	ret = kstrtoint(buf, 10, &lwa_rx_threshold_high);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_rx_threshold_high retrun fail: %d\n", __func__, ret);
		return 0;
	}

	if ( lwa_rx_threshold_high > md_ctrl->lwa_rx_threshold_low) {
		md_ctrl->lwa_rx_threshold_high = lwa_rx_threshold_high;
	}
	pr_info("[LWA monitor] %s: lwa_rx_threshold_high=[%d]\n", __func__, lwa_rx_threshold_high);

	return count;
}

static ssize_t md_lwa_monitor_threshold_low_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;

	count += snprintf(buf, 128, "lwa_rx_threshold_low=%d\n", md_ctrl->lwa_rx_threshold_low);

	return count;
}

static ssize_t md_lwa_monitor_threshold_low_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_rx_threshold_low = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	ret = kstrtoint(buf, 10, &lwa_rx_threshold_low);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_rx_threshold_low retrun fail: %d\n", __func__, ret);
		return 0;
	}

	if ( lwa_rx_threshold_low < md_ctrl->lwa_rx_threshold_high) {
		md_ctrl->lwa_rx_threshold_low = lwa_rx_threshold_low;
	}
	pr_info("[LWA monitor] %s: lwa_rx_threshold_low=[%d]\n", __func__, lwa_rx_threshold_low);

	return count;
}

static ssize_t md_lwa_monitor_threshold_high2_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;

	count += snprintf(buf, 128, "lwa_rx_threshold_high2=%d\n", md_ctrl->lwa_rx_threshold_high2);

	return count;
}

static ssize_t md_lwa_monitor_threshold_high2_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_rx_threshold_high2 = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	ret = kstrtoint(buf, 10, &lwa_rx_threshold_high2);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_rx_threshold_high2 retrun fail: %d\n", __func__, ret);
		return 0;
	}

	if ( lwa_rx_threshold_high2 > md_ctrl->lwa_rx_threshold_low2) {
		md_ctrl->lwa_rx_threshold_high2 = lwa_rx_threshold_high2;
	}
	pr_info("[LWA monitor] %s: lwa_rx_threshold_high2=[%d]\n", __func__, lwa_rx_threshold_high2);

	return count;
}

static ssize_t md_lwa_monitor_threshold_low2_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;

	count += snprintf(buf, 128, "lwa_rx_threshold_low2=%d\n", md_ctrl->lwa_rx_threshold_low2);

	return count;
}

static ssize_t md_lwa_monitor_threshold_low2_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_rx_threshold_low2 = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	ret = kstrtoint(buf, 10, &lwa_rx_threshold_low2);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_rx_threshold_low2 retrun fail: %d\n", __func__, ret);
		return 0;
	}

	if ( lwa_rx_threshold_low2 < md_ctrl->lwa_rx_threshold_high2) {
		md_ctrl->lwa_rx_threshold_low2 = lwa_rx_threshold_low2;
	}
	pr_info("[LWA monitor] %s: lwa_rx_threshold_low2=[%d]\n", __func__, lwa_rx_threshold_low2);

	return count;
}

static ssize_t md_lwa_enable_threshold_v2_show(struct ccci_modem *md, char *buf)
{
	ssize_t count = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;

	count += snprintf(buf, 128, "lwa_enable_threshold_v2=%d\n", md_ctrl->lwa_enable_threshold_v2);
	return count;
}

static ssize_t md_lwa_enable_threshold_v2_store(struct ccci_modem *md, const char *buf, size_t count)
{
	int ret;
	unsigned int lwa_enable_threshold_v2 = 0;
	struct md_cd_ctrl *md_ctrl = NULL;
	md_ctrl = (struct md_cd_ctrl *)md->private_data;
	ret = kstrtoint(buf, 10, &lwa_enable_threshold_v2);
	if (ret < 0) {
		pr_err("[LWA monitor] %s: lwa_enable_threshold_v2 retrun fail: %d\n", __func__, ret);
		return 0;
	}

	md_ctrl->lwa_enable_threshold_v2 = lwa_enable_threshold_v2;

	pr_info("[LWA monitor] %s: lwa_rx_threshold_high2=[%d]\n", __func__, lwa_enable_threshold_v2);
...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
CCCI_MD_ATTR(NULL, lwa_state, 0660, md_lwa_state_show, md_lwa_state_store);

CCCI_MD_ATTR(NULL, lwa_monitor_enable, 0660, md_lwa_monitor_enable_show, md_lwa_monitor_enable_store);

CCCI_MD_ATTR(NULL, lwa_monitor_threshold_high, 0660, md_lwa_monitor_threshold_high_show, md_lwa_monitor_threshold_high_store);

CCCI_MD_ATTR(NULL, lwa_monitor_threshold_low, 0660, md_lwa_monitor_threshold_low_show, md_lwa_monitor_threshold_low_store);

CCCI_MD_ATTR(NULL, lwa_monitor_threshold_high2, 0660, md_lwa_monitor_threshold_high2_show, md_lwa_monitor_threshold_high2_store);

CCCI_MD_ATTR(NULL, lwa_monitor_threshold_low2, 0660, md_lwa_monitor_threshold_low2_show, md_lwa_monitor_threshold_low2_store);

CCCI_MD_ATTR(NULL, lwa_enable_threshold_v2, 0660, md_lwa_enable_threshold_v2_show, md_lwa_enable_threshold_v2_store);
...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
	ccci_md_attr_lwa_state.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_state.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_state.attr.name, ret);

	ccci_md_attr_lwa_monitor_enable.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_monitor_enable.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_monitor_enable.attr.name, ret);

	ccci_md_attr_lwa_monitor_threshold_high.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_monitor_threshold_high.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_monitor_threshold_high.attr.name, ret);

	ccci_md_attr_lwa_monitor_threshold_low.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_monitor_threshold_low.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_monitor_threshold_low.attr.name, ret);

	ccci_md_attr_lwa_monitor_threshold_high2.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_monitor_threshold_high2.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_monitor_threshold_high2.attr.name, ret);

	ccci_md_attr_lwa_monitor_threshold_low2.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_monitor_threshold_low2.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_monitor_threshold_low2.attr.name, ret);

	ccci_md_attr_lwa_enable_threshold_v2.modem = md;
	ret = sysfs_create_file(&md->kobj, &ccci_md_attr_lwa_enable_threshold_v2.attr);
	if(ret)
		CCCI_ERR_MSG(md->index, TAG, "fail to add sysfs node %s %d\n", ccci_md_attr_lwa_enable_threshold_v2.attr.name, ret);
...
#ifdef CONFIG_HTC_FEATURES_RIL_PCN0007_LWA
	init_timer(&md_ctrl->lwa_traffic_monitor);
	INIT_WORK(&md_ctrl->lwa_traffic_monitor_work, md_cd_lwa_traffic_check_work);
	md_ctrl->lwa_traffic_monitor.function = md_cd_lwa_traffic_monitor_func;
	md_ctrl->lwa_traffic_monitor.data = (unsigned long)md;
	md_ctrl->lwa_rx_threshold_high = LWA_HIGH_THRESHOLD;
	md_ctrl->lwa_rx_threshold_low = LWA_LOW_THRESHOLD;
	md_ctrl->lwa_rx_threshold_high2 = LWA_HIGH_THRESHOLD2;
	md_ctrl->lwa_rx_threshold_low2 = LWA_LOW_THRESHOLD2;
#ifdef LWA_ENABLE_THRESHOLD_V2
	md_ctrl->lwa_enable_threshold_v2= 1;
#endif
#endif
