...
static struct pinctrl_state *flashlight_all_low;
unsigned int gpio_hwen, gpio_enm, gpio_enf, gpio_irq;
#endif
...
struct device_node *node1 = pdev->dev.of_node;
	if(node1)
	{
		of_property_read_u32(node1, "flashlight_hwen", &gpio_hwen);
		of_property_read_u32(node1, "flashlight_enm", &gpio_enm);
		of_property_read_u32(node1, "flashlight_enf", &gpio_enf);
		of_property_read_u32(node1, "flashlight_irq", &gpio_irq);
		gpio_request(gpio_irq, "flt_irq");
		gpio_request(gpio_enf, "flt_enf");
		gpio_request(gpio_enm, "flt_enm");
		gpio_request(gpio_hwen, "flt_hwen");
	}
#endif
...
#endif
#if defined(CONFIG_SY7806_FLASHLIGHT)
	flashlight_all_low = pinctrl_lookup_state(flashlight_pinctrl, "init_all_low");
	if (IS_ERR(flashlight_all_low)) {
		ret = PTR_ERR(flashlight_all_low);
		logI("%s : init err, flashlight_all_low\n", __func__);
	}

			pinctrl_select_state(flashlight_pinctrl, flashlight_all_low);
#endif
	return ret;
}
...
int flashlight_gpio_set(int pin , int state)
{
	...
	case FLASHLIGHT_PIN_HWEN:
		gpio_set_value(gpio_hwen, state? 1 : 0);
		break;
	case MAIN_FLASHLIGHT_ENM_PIN:
		gpio_set_value(gpio_enm, state);
		break;
	case MAIN_FLASHLIGHT_ENF_PIN:
		gpio_set_value(gpio_enf, state);
		break;
#endif
	default:
			logI("%s : set err, pin(%d) state(%d)\n", __func__, pin, state);
...
static int setFlashDrv(int sensorDev, int strobeId)
{
	...
	if ((*ppF) != 0) {
		...
		printk("setFlashDrv ok %d", __LINE__);
	} else {
		printk("set function pointer not found!!");
		return -1;
	}
	return 0;
}
	...
	case FLASH_IOC_GET_MAIN2_PART_ID:
             ...
             case FLASH_IOC_RESUME_DISPLAY_BACKLIGHT:
		{
			...
			if (pF != 0) {
				...
				printk("\n [FLASH_IOC_SET_ONOFF] function pointer is wrong - \n");
			}
		}
		break;
...
#endif
	printk("[flashlight_init ] done! ~\n");
	return ret;
}

static void __exit flashlight_exit(void)
{
	logI("[flashlight_exit] start ~\n");
	platform_driver_unregister(&flashlight_platform_driver);
	
	
	logI("[flashlight_exit] done! ~\n");
}
