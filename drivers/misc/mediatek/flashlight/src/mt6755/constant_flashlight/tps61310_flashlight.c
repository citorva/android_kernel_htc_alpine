 ...
 *
 * Copyright (C) 2008-2016 HTC Corporation.
 *
...
static int tps61310_i2c_command(uint8_t address, uint8_t data)
{
	...
	if (ret < 0) {
		...
		if (this_tps61310->reset) {
			...
			gpio_set_value(this_tps61310->reset, 0);

			mdelay(10);
			gpio_set_value(this_tps61310->reset, 1);
...
static int flashlight_turn_off(void)
{
	...
	if (this_tps61310->power_save) {
		...
		if (status == 2 || (status >= 10 && status <=16)) {
			...
			gpio_set_value(this_tps61310->power_save, 0);
		} else if (status == FL_MODE_PRE_FLASH) {
			FLT_INFO_LOG("Enable power saving\n");
			gpio_set_value(this_tps61310->power_save, 1);
		}
	}
	if (this_tps61310->power_save_2) {
		...
		if (status == 2 || (status >= 10 && status <=16)) {
			FLT_INFO_LOG("Disable power saving\n");
			gpio_set_value(this_tps61310->power_save_2, 0);
		} else if (status == FL_MODE_PRE_FLASH) {
			FLT_INFO_LOG("Enable power saving\n");
			gpio_set_value(this_tps61310->power_save_2, 1);
		}
	}
...
int tps61310_flashlight_control(int mode)
{
	...
	if ( support_dual_flashlight ) {
			...
			break;
			case FL_MODE_FLASH:
				FLT_INFO_LOG("flash 750mA, count 1\n");
					...
					const struct i2c_device_id *id)
{
	...
	gpDMABuf_va = (u8 *)dma_alloc_coherent(&this_client->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, (dma_addr_t *)&gpDMABuf_pa, GFP_KERNEL);
	if(!gpDMABuf_va){
...
err_gpio_or_dev:
	dma_free_coherent(&this_client->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, gpDMABuf_va, (dma_addr_t)gpDMABuf_pa);
	gpDMABuf_va = NULL;
	gpDMABuf_pa = NULL;
err_dma_alloc_va:
	kfree(tps61310);
