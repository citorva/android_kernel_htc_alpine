...
#ifdef CONFIG_USB_C_SWITCH
static struct mutex xhci_typec_otg_power_lock;
#endif
...
static void mtk_enable_otg_mode(void)
{
	...
	#endif 
	bq25890_otg_en(0x01);
#if defined(CONFIG_MTK_BQ25896_BOOST_ILIM)
	pr_debug("%s bq25896 boost_ilim=%d\n", __func__, CONFIG_MTK_BQ25896_BOOST_ILIM);
	bq25890_set_boost_ilim(CONFIG_MTK_BQ25896_BOOST_ILIM);
#else
	pr_debug("%s bq25896 boost_ilim=3 (default)\n", __func__);
	bq25890_set_boost_ilim(0x03);	
#endif

#if defined(CONFIG_MTK_BQ25896_PFM_OTG_ENABLE)
	pr_debug("%s bq25896 PFM enabled\n", __func__);
	bq25890_set_pfm_otg_dis(0x0);
#else
	pr_debug("%s bq25896 PFM disabled\n", __func__);
	bq25890_set_pfm_otg_dis(0x1);
#endif
	#ifdef CONFIG_USB_XHCI_MTK_OTG_POWER_SAVING
...
static void otg_set_vbus_bq24296(int is_on)
{
	CABLE_INFO(" %s: is_on= %d\n", __func__, is_on);
	if (is_on == 1) {
		#ifdef CONFIG_USB_XHCI_MTK_OTG_POWER_SAVING
...
{
#ifdef CONFIG_MTK_BQ24296_SUPPORT
	otg_set_vbus_bq24296(0);
#endif
	mtk_disable_otg_mode();
	
	pr_debug("mtk_xhci_disPortPower, mtk_xhci(0x%p)\n", mtk_xhci);
	if (mtk_xhci)
...
void mtk_xhci_switch_init(void)
{
	...
	if (switch_dev_register(&dock_switch) < 0) {
		mtk_xhci_mtk_printk(K_DEBUG, "[Cable][%s] fail to register dock switch!\n", __func__);
		return;
	}
	ret = device_create_file(dock_switch.dev, &dev_attr_adc);
	if (ret != 0)
		mtk_xhci_mtk_printk(K_DEBUG, "[Cable][%s] dev_attr_adc failed\n", __func__);
	ret = device_create_file(dock_switch.dev, &dev_attr_USB_ID_status);
	if (ret != 0)
		mtk_xhci_mtk_printk(K_DEBUG, "[Cable][%s] dev_attr_USB_ID_status failed\n", __func__);
}
...
void mtk_xhci_mode_switch(struct work_struct *work)
{
	...
	int acce_type = ADC_NON_STABLE;
	static int curr_acce_type = 0;
	...
	if (mtk_idpin_cur_stat == IDPIN_OUT) {
		...
		if (acce_type == ADC_NON_STABLE) {
			schedule_delayed_work(&mtk_xhci_delaywork, ADC_DELAY);

			return;
		} else if (acce_type == DOCK_STATE_UNDEFINED) {
			printk("UnKown accessory Type\n");
			curr_acce_type = acce_type;
			htc_accessory_ineserted(curr_acce_type);
			goto done;
		}
		curr_acce_type = acce_type;
		htc_accessory_ineserted(curr_acce_type);
		if (curr_acce_type == DOCK_STATE_USB_HOST) {
		...
		if (is_load) {
			...
			#ifdef CONFIG_USB_XHCI_MTK_OTG_POWER_SAVING
			disable_ip_sleep_wakeup();
			slp_request_infra(false);
			#endif 
		}
		
		htc_accessory_removed(curr_acce_type);
		curr_acce_type = DOCK_STATE_UNDOCKED;
		CABLE_INFO(" %s(%d): current cable type= %d\n", __func__, __LINE__, htc_cable_detect_get_type());
		...
		while((value = mfg_check_white_accessory(curr_acce_type)) == -EAGAIN) {
			CABLE_INFO(" %s(%d): android_dev is not ready. Try again. value = %d\n", __func__, __LINE__, value);
			msleep(1000);
		}
	mtk_xhci_mtk_printk(K_ALET, "[USB][Cable] current mode is %s, ret(%d), switch(%d)\n",
...
static int xhci_reboot_notify(struct notifier_block *this, unsigned long code,
        void *unused)
{
	mtk_xhci_mtk_printk(K_DEBUG, "%s code: %ld\n", __func__, code);
...
static struct notifier_block xhci_reboot_notifier = {
	.notifier_call = xhci_reboot_notify,
};
...
int mtk_xhci_eint_iddig_init(void)
{
	...
	if (node) {
		...
		if (node) {
			...
			if (!retval) {
				...
				printk("[USB][CABLE] iddig gpio num = %d\n", mtk_idpin_irqnum);
			}
		}
	} else {
	...
	if (0)
		xhci_eint_iddig_isr(0,NULL);
#endif 

	retval = register_reboot_notifier(&xhci_reboot_notifier);
	if (retval != 0) {
		mtk_xhci_mtk_printk(K_DEBUG, "register_reboot_notifier fail, ret %d!!!\n", retval);
		return retval;
	}
...
static int typec_otg_enable(void *data)
{
	...
	mutex_lock(&xhci_typec_otg_power_lock);
	pr_info("%s  mtk_idpin_cur_stat %d\n", __func__, mtk_idpin_cur_stat);
	if (mtk_idpin_cur_stat == IDPIN_IN_HOST) {
		mutex_unlock(&xhci_typec_otg_power_lock);
		return ret;
	}
...
#ifdef CONFIG_USB_C_SWITCH
	mutex_init(&xhci_typec_otg_power_lock); 
...
void mtk_xhci_set(struct usb_hcd *hcd, struct xhci_hcd *xhci)
{
	...
	pr_debug("mtk_xhci_set mtk_xhci = 0x%p\n", xhci); 
	mtk_xhci = xhci;
}
...
void mtk_xhci_reset(struct xhci_hcd *xhci)
{
	...
	mtk_xhci = NULL;
	pr_debug("mtk_xhci_reset mtk_xhci = 0x%p\n", xhci); 
}
