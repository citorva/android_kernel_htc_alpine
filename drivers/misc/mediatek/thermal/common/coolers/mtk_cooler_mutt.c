...
/* minimum Tx throughput*/
static unsigned int min_tx_Tput;
...
static int mtk_cl_mutt_set_cur_state(struct thermal_cooling_device *cdev, unsigned long state)
{
	...
	/*Only active while UL Tput > min_Tput */
	if (mtk_thermal_get_sma_txtput() < min_tx_Tput)
		state = 0;
...
#endif
		seq_printf(m, "mutt_min_tx_Tput %d\n", min_tx_Tput);
				       ...
				       loff_t *data)
{
	...
	int amutt_target_temp, mutt_min_tx_Tput;
	int scan_count = 0;
	...
	scan_count = sscanf(desc, "%d %d %d %d %d %d %d %d %d %d %d",
			&klog_on, &mutt0_a, &mutt0_s, &mutt1_a, &mutt1_s, &mutt2_a, &mutt2_s,
			&mutt3_a, &mutt3_s, &amutt_target_temp , &mutt_min_tx_Tput);
...
#if 1 /*  This is for thermal policy to set the min_tx_Tput */
		if (scan_count > 2+MAX_NUM_INSTANCE_MTK_COOLER_MUTT*2)
			min_tx_Tput = mutt_min_tx_Tput;
#endif
