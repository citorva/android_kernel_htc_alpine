...
static unsigned long tx_sma_throughput;

unsigned long mtk_thermal_get_sma_txtput(void)
{
	return tx_sma_throughput;
}
EXPORT_SYMBOL(mtk_thermal_get_sma_txtput);

#define MIN(_a_, _b_) ((_a_) < (_b_) ? (_a_) : (_b_))
#define MAX_SMA_LEN	5
static unsigned int ma_len = MAX_SMA_LEN;
static unsigned int ma_counter;
static unsigned long ma[MAX_SMA_LEN];

static unsigned long  get_sma(unsigned long  latest_val)
{
	unsigned long ret = 0;
	int i = 0;
...
static int pa_cal_stats(unsigned long data)
{
	...
	tx_sma_throughput = get_sma(tx_throughput);
