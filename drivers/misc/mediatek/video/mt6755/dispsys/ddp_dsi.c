...
unsigned int clock_lane = 0;
unsigned int data_lane3 = 0;
unsigned int data_lane2 = 0;
unsigned int data_lane1 = 0;
unsigned int data_lane0 = 0;
...
static void lcm_set_reset_pin(uint32_t value)
{
	...
	if (value == 0)
		...
		disp_dts_gpio_select_state(DTS_GPIO_STATE_LCM_RST_IN);
#endif
#endif
}
...
long lcd_enp_bias_ioexp_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value == 0)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENP0);
	else if (value == 1)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENP1);
	else if (value == 2)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENP_IN);
#endif
	return ret;
}

long lcd_enn_bias_ioexp_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value == 0)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENN0);
	else if (value == 1)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENN1);
	else if (value == 2)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LCD_BIAS_ENN_IN);
#endif
	return ret;
}

long lm36274_hwen_ioexp_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LM36274_HWEN1);
	else
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_LM36274_HWEN0);
#endif
	return ret;
}

long tp_set_reset_ioexp_pin(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_TP_RST_OUT1);
	else
		ret = disp_dts_gpio_select_state(DTS_IOEXP_GPIO_STATE_TP_RST_OUT0);
#endif
	return ret;
}
...
long lcd_io_18_en_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_IO_18_EN1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCD_IO_18_EN0);
#endif
	return ret;
}

long lcdbl_en_setting(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCDBL_EN1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_LCDBL_EN0);
#endif
	return ret;
}
...
#else
#ifdef CONFIG_PINCTRL_TCA6418
	utils->set_ioexp_gpio_lcd_enp_bias = lcd_enp_bias_ioexp_setting;
	utils->set_ioexp_gpio_lcd_enn_bias = lcd_enn_bias_ioexp_setting;
	utils->set_ioexp_gpio_lm36274_hwen = lm36274_hwen_ioexp_setting;
	utils->set_ioexp_tp_reset_pin = tp_set_reset_ioexp_pin;
#else
	utils->set_gpio_lcd_enp_bias = lcd_enp_bias_setting;
	utils->set_gpio_lcd_enn_bias = lcd_enn_bias_setting;
#endif
	utils->set_gpio_lcd_io_18_en = lcd_io_18_en_setting;
	utils->set_gpio_lcdbl_en = lcdbl_en_setting;
#endif
#endif
...
int ddp_dsi_init(DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (MIPITX_IsEnabled(module, cmdq)) {
		...
		clock_lane = (INREG32(MIPI_TX_REG_BASE + 0x4));
		data_lane3 = (INREG32(MIPI_TX_REG_BASE + 0x14));
		data_lane2 = (INREG32(MIPI_TX_REG_BASE + 0x10));
		data_lane1 = (INREG32(MIPI_TX_REG_BASE + 0xc));
		data_lane0 = (INREG32(MIPI_TX_REG_BASE + 0x8));
