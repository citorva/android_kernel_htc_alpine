...
unsigned char get_camera_blk(void)
{
	return pgc->plcm->params->camera_blk;
}
EXPORT_SYMBOL(get_camera_blk);

#define BRI_SETTING_MIN 32
#define BRI_SETTING_DEF 142
#define BRI_SETTING_MAX 255

static unsigned int shrink_pwm(unsigned int val)
{
	...
	static unsigned int last_br = BRI_SETTING_DEF;

	if (pwm_min == 0 || pwm_default == 0 || pwm_max == 0 ||pwm_default == 0xff )
		return val;
	...
	if (shrink_br != last_br)
		pr_info("[DISP] brightness trans %d to %d\n", val, shrink_br);
	last_br = shrink_br;

	return shrink_br;
}

int primary_display_setbacklight(unsigned int level, int from_cali)
{
	int ret = 0;
	...
	if (pgc->state == DISP_SLEPT) {
		...
		if (!from_cali)
			level = htc_update_bklt_cali_data(level);
		primary_display_idlemgr_kick((char *)__func__, 0);
		if (primary_display_cmdq_enabled()) {
			if (primary_display_is_video_mode()) {
...
int primary_display_get_pwm(unsigned int *pdefault, unsigned int *pmin, unsigned int *pmax )
{
	LCM_PARAMS *lcm_param = disp_lcm_get_params(pgc->plcm);

	if(pmin)
		*pmin = lcm_param ? lcm_param->pwm_min : 0;
	if(pdefault)
		*pdefault = lcm_param ? lcm_param->pwm_default : 0;
	if(pmax)
		*pmax = lcm_param ? lcm_param->pwm_max : 0;

	return lcm_param ? 1 : 0;
}
#endif
