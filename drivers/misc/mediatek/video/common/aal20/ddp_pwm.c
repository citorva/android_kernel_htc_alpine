...
#if defined(CONFIG_MTK_CPU_PWM)
#define B8_TO_B10(x)  ((x+1)*4-1)
#define BRI_SETTING_MIN_B10 B8_TO_B10(30)
#define BRI_SETTING_DEF_B10 B8_TO_B10(142)
#define BRI_SETTING_MAX_B10 B8_TO_B10(255)

int disp_pwm_shrink(int bl_1024)
{
	unsigned int shrink_br = BRI_SETTING_DEF_B10;
	unsigned int pwm_min = 0;
	unsigned int pwm_default = 0;
	unsigned int pwm_max = 0;

	extern int primary_display_get_pwm(unsigned int *pdefault, unsigned int *pmin, unsigned int *pmax);
	primary_display_get_pwm(&pwm_default, &pwm_min, &pwm_max);
	...
	if (bl_1024 <= 0) {
		shrink_br = 0;
	} else if (bl_1024 > 0 && (bl_1024 < BRI_SETTING_MIN_B10)) {
		shrink_br = B8_TO_B10(pwm_min);
	} else if ((bl_1024 >= BRI_SETTING_MIN_B10) && (bl_1024 <= BRI_SETTING_DEF_B10)) {
		shrink_br = (bl_1024 - BRI_SETTING_MIN_B10) * (B8_TO_B10(pwm_default) - B8_TO_B10(pwm_min)) /
			(BRI_SETTING_DEF_B10 - BRI_SETTING_MIN_B10) + B8_TO_B10(pwm_min);
	} else if (bl_1024 > BRI_SETTING_DEF_B10 && bl_1024 <= BRI_SETTING_MAX_B10) {
		shrink_br = (bl_1024 - BRI_SETTING_DEF_B10) * (B8_TO_B10(pwm_max) - B8_TO_B10(pwm_default)) /
			(BRI_SETTING_MAX_B10 - BRI_SETTING_DEF_B10) + B8_TO_B10(pwm_default);
	} else if (bl_1024 > BRI_SETTING_MAX_B10)
		shrink_br = B8_TO_B10(pwm_max);

	PWM_MSG("shrink pwm bl_1024 from %d to %d",bl_1024,shrink_br);
...
{
	#if defined(CONFIG_MTK_CPU_PWM)
	level_1024 = disp_pwm_shrink(level_1024);
	#endif
	return level_1024;
}
...
static void disp_pwm_test_div(const char *cmd)
{
	...
	disp_pwm_set_backlight(DISP_PWM0, 256 + div);	
}
