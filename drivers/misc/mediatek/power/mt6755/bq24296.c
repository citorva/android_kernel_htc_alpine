...
kal_uint32 bq24296_get_power_ovp_state(void)
{
	kal_uint32 ret=0;
    kal_uint8 val=0;
...
//CON09------------------------------------------------------
kal_uint32 bq24296_get_power_ovp_state2(void)
{
	kal_uint32 ret=0;
    kal_uint8 val=0;

    ret=bq24296_read_interface(     (kal_uint8)(bq24296_CON9), 
                                    (&val),
                                    (kal_uint8)(CON9_CHRG_FAULT_MASK),
                                    (kal_uint8)(CON9_CHRG_FAULT_SHIFT)
                                    );
    return val;
}
...
int bq24296_dump_register_htc(char *buf, int size)
{
    int i=0, len = 0;
    for (i=0;i<bq24296_REG_NUM;i++)
    {
		...
		if( i == 0 )
			{
				len += scnprintf(buf+len, size-len,"batt_Vin_dpm:\t%d\n",
					((bq24296_reg[0] & 0x78) == 0x40) ? 4520 : 4200);
				len += scnprintf(buf+len, size-len,"batt_curr_iinlim:\t%d\n",
					(bq24296_reg[0] & 0x7) == 0x5 ? 1500 :
					(bq24296_reg[0] & 0x7) == 0x4 ? 1000 :
					(bq24296_reg[0] & 0x7) == 0x3 ? 900 : 500);
			}
        len += scnprintf(buf + len, size - len,
                "BQ24296_REG[0x%x]: 0x%X;\n", i, bq24296_reg[i]);
    }
    return len;
}
...
//module_init(bq24296_subsys_init);
