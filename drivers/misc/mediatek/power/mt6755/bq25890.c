...
static DEFINE_MUTEX(bq25890_config_access);
				    ...
				    unsigned char SHIFT)
{
	...
	mutex_lock(&bq25890_config_access);
	...
	mutex_unlock(&bq25890_config_access);
...
void bq25890_set_pfm_otg_dis(unsigned int val)
{
	unsigned int ret = 0;

	ret = bq25890_config_interface((unsigned char) (bq25890_CONA),
				       (unsigned char) (val),
				       (unsigned char) (CONA_PFM_OTG_DIS_MASK),
				       (unsigned char) (CONA_PFM_OTG_DIS_SHIFT)
	    );
}
...
int bq25890_dump_register_htc(char *buf, int size)
{
    int i=0, len = 0;
    for (i=0;i<bq25890_REG_NUM;i++)
    {
...
static DEVICE_ATTR(bq25890_access, 0664, show_bq25890_access, store_bq25890_access);	
