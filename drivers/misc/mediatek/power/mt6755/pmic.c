...
static ssize_t show_kick_long_press_reboot_timer(struct device *dev, struct device_attribute *attr, char *buf)
{
	int len = 0;
	len += snprintf(buf + len, PAGE_SIZE - len, "PMIC_RG_PWRKEY_RST_TD:%x\n", pmic_get_register_value(PMIC_RG_PWRKEY_RST_TD));
	len += snprintf(buf + len, PAGE_SIZE - len, "PMIC_RG_PWRKEY_RST_EN:%x\n", pmic_get_register_value(PMIC_RG_PWRKEY_RST_EN));
	len += snprintf(buf + len, PAGE_SIZE - len, "PMIC_RG_HOMEKEY_RST_EN:%x\n", pmic_get_register_value(PMIC_RG_HOMEKEY_RST_EN));
	return len;
}

static ssize_t store_kick_long_press_reboot_timer(struct device *dev, struct device_attribute *attr, const char *buf,
			      size_t size)
{
	pmic_set_register_value(PMIC_RG_STRUP_PWRKEY_COUNT_RESET, 0x1);
	return size;
}

static DEVICE_ATTR(kick_long_press_reboot_timer, 0664, show_kick_long_press_reboot_timer, store_kick_long_press_reboot_timer);
				...
				size_t size)
{
	...
	if(buf != NULL && size != 0)
	{
		enable = simple_strtoul(buf,&pvalue,10);
	}
	else{
		pr_err("[store_LDO_STATUS] buf is NULL or size is 0 \n");
		return size;
	}

	mreg=container_of(attr,struct mtk_regulator,en_att);
	if(mreg->isUsedable != 1){
		pr_err("[EM] Not Support Write Function\n");
		return size;
	}

	if (mreg->en_reg != 0) {
		pmic_set_register_value(mreg->en_reg, enable);
		add = pmu_flags_table[mreg->en_reg].offset;
		val = upmu_get_reg_value(add);
	}

	pr_info("regulator_enable(name=%s id=%d en_reg=%x vol_reg=%x) [%x]=0x%x\n", mreg->desc.name,
		mreg->desc.id, mreg->en_reg, mreg->vol_reg, add, val);
	pr_info("[EM] LDO_%s\n", mreg->desc.name);
	pr_err("%s:%s %s\n",__func__,mreg->desc.name,(enable)? "ON":"OFF");
				 ...
				 size_t size)
{
	...
	if(buf != NULL && size != 0)
	{
		...
		PMICLOG("[store_LDO_STATUS] buf is NULL or size is 0 \n");
		return size;
	}

	mreg=container_of(attr,struct mtk_regulator,en_att);

	if(!strcmp(mreg->desc.name, "VGPU")){
#if 1
		pr_err("[EM] Not Support BUCK_%s Write Function\n", mreg->desc.name);
#else
		
		pr_err("%s:BUCK_VGPU %s\n",__func__,(enable)? "ON":"OFF");
		if (enable == 1)
	1		pmic_config_interface(MT6351_PMIC_BUCK_VGPU_EN_ADDR, 0x1,\
				      MT6351_PMIC_BUCK_VGPU_EN_MASK,\
				      MT6351_PMIC_BUCK_VGPU_EN_SHIFT);	
	2		pmic_set_register_value(MT6351_PMIC_BUCK_VGPU_EN, 1);
	3		pmic_set_register_value(mreg->qi_en_reg, 1);
		else
	1		pmic_config_interface(MT6351_PMIC_BUCK_VGPU_EN_ADDR, 0x0,\
				      MT6351_PMIC_BUCK_VGPU_EN_MASK,\
				      MT6351_PMIC_BUCK_VGPU_EN_SHIFT);	
	2		pmic_set_register_value(MT6351_PMIC_BUCK_VGPU_EN, 0);
	3		pmic_set_register_value(mreg->qi_en_reg, 0);
#endif
	}
	else if(!strcmp(mreg->desc.name, "VMD1") ||
		!strcmp(mreg->desc.name, "VMODEM") ||
		!strcmp(mreg->desc.name, "VSRAM_MD")){
		pr_err("%s:BUCK_<VMD1/VMODEM/VSRAM_MD> %s\n",__func__, (enable)? "ON":"OFF");
		if(enable == 0){
...
#define BAT_HV_THD   (POWER_INT0_VOLT*4096/5400)	
#define BAT_LV_1_THD (POWER_INT1_VOLT*4096/5400)	
#define BAT_LV_2_THD (POWER_INT2_VOLT*4096/5400)	
...
#if defined(CONFIG_MTK_SMART_BATTERY)
#define BAT_OC_H_THD   \
(65535-((300400*POWER_BAT_OC_CURRENT_H/1000)/batt_meter_cust_data.car_tune_value))	
#define BAT_OC_L_THD   \
(65535-((300400*POWER_BAT_OC_CURRENT_L/1000)/batt_meter_cust_data.car_tune_value))	

#define BAT_OC_H_THD_RE   \
(65535-((300400*POWER_BAT_OC_CURRENT_H_RE/1000)/batt_meter_cust_data.car_tune_value))	
#define BAT_OC_L_THD_RE   \
(65535-((300400*POWER_BAT_OC_CURRENT_L_RE/1000)/batt_meter_cust_data.car_tune_value))	
#else
...
{				
	...
	if (g_battery_percent_stop == 1) {
		...
		pr_err
			...
			pr_err("battery_percent_callback == NULL\n");
			}
#endif
	}
}
...
int get_rac_val(void)
{
	...
	do {
		...
		if ((curr_2 - curr_1) >= 700 && (curr_2 - curr_1) <= 1200
			...
			rac_cal = ((volt_1 - volt_2) * 1000) / (curr_2 - curr_1);	
...
static int pmic_mt_probe(struct platform_device *dev)
{
	...
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_kick_long_press_reboot_timer);
