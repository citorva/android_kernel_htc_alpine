...
extern int htc_battery_get_AICL_status(void);
#endif
...
static kal_uint32 charging_hw_init(void *data)
{
...
#ifdef HTC_ENABLE_AICL
	if(htc_battery_get_AICL_status())
		bq24296_set_vindpm(0x4); 
	else bq24296_set_vindpm(0x8);
#else
	bq24296_set_vindpm(0x8); 
#endif
	bq24296_set_reg_rst(0x0);
	bq24296_set_wdt_rst(0x1); 
	bq24296_set_sys_min(0x5); 
	bq24296_set_iprechg(0x2); 
	bq24296_set_iterm(0x0); 

	if (s_first) { 
#if defined(HIGH_BATTERY_VOLTAGE_SUPPORT)
		bq24296_set_vreg(0x38); 
#else
	...
	bq24296_set_batlowv(0x1); 
	bq24296_set_vrechg(0x0); 
	bq24296_set_en_term(0x0); 
	bq24296_set_watchdog(0x1); 
	bq24296_set_en_timer(0x0); 
	bq24296_set_int_mask(0x0); 
...
static kal_uint32 charging_htc_set_vdm(void *data)
{
	...
	bq24296_set_vindpm(register_value);
...
{
	kal_uint8 val = 0;
	val = bq24296_get_system_status();
	if( val & 0x8) *(kal_uint32 *)data = KAL_TRUE;
	else *(kal_uint32 *)data = KAL_FALSE;
	return STATUS_OK;
}

static kal_uint32 charging_htc_get_idpm_state(void *data)
{
	return STATUS_OK;
}
...
static kal_uint32 charging_dump_register_htc(void *data)
 {
	...
	len = bq24296_dump_register_htc(buf, size);
	pReg_dump->size = len;
	return STATUS_OK;
 }
...
static kal_uint32 charging_hiz_en_ctrl(void *data)
{
    kal_uint32 status = STATUS_OK;
	kal_uint32 enable = *(kal_uint32*)(data);
...
static kal_uint32 charging_set_cv_voltage(void *data)
{
	...
	if(BATTERY_VOLT_04_200000_V == cv_value)
		cv_value = 4208000;
	if(BATTERY_VOLT_04_100000_V == cv_value)
		cv_value = 4080000;
...
static kal_uint32 charging_get_power_ovp_state(void *data)
{
	kal_uint32 status = STATUS_OK;
	if(bq24296_get_power_ovp_state() == 1)
		*(kal_bool *)data = KAL_TRUE;
	else
		*(kal_bool *)data = KAL_FALSE;
	return status;	
...
static kal_uint32 charging_get_power_ovp_state2(void *data)
{
	kal_uint32 status = STATUS_OK;
	if(bq24296_get_power_ovp_state2() == 0x1)
		*(kal_bool *)data = KAL_TRUE;
	else
		*(kal_bool *)data = KAL_FALSE;
	return status;	
...
static kal_uint32 charging_set_batfet_state(void *data)
{
	kal_uint32 value =  *(kal_uint32*)(data);
	bq24296_set_batfet_disable(value);
	return STATUS_OK;
}
