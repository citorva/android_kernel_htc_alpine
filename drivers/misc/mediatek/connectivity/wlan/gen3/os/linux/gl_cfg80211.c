...
int mtk_cfg80211_get_link_statistics(struct wiphy *wiphy, struct net_device *ndev, u8 *mac, struct station_info *sinfo)
{
	...
	if (prDevStats) {
		...
		kalMemZero(&rQueryStaStatistics, sizeof(rQueryStaStatistics));
		COPY_MAC_ADDR(rQueryStaStatistics.aucMacAddr, arBssid);
		rQueryStaStatistics.ucLlsReadClear = FALSE;	
...
INT_32 mtk_cfg80211_process_str_cmd(P_GLUE_INFO_T prGlueInfo, PUINT_8 cmd, INT_32 len)
{
	...
	UINT_32 u4SetInfoLen = 0;
#endif
	if (strnicmp(cmd, "SETALWAYSSCANSTATE ", 19) == 0) {
#if 0
		rStatus = kalIoctl(prGlueInfo,
					...
					&u4SetInfoLen);
#endif
	} else if (strnicmp(cmd, "set-lwa-filter-coefficient ", 27) == 0) {
		struct _CMD_LWA_FILTER_COEFFICIENT lwaFilterCoeff = {0};
		int coefficient = 0;
		int result = 0;
		char coToInt[16] = {0};

		lwaFilterCoeff.bssIndex = 0;
		kalStrnCpy(coToInt, cmd + 27, len - 27);
		result = kalkStrtos32(coToInt, 0, &coefficient);
		if (result)
			DBGLOG(INIT, INFO, "fileter coefficient parse error: %d\n", coefficient);

		lwaFilterCoeff.filterCoefficient = coefficient;
		prGlueInfo->prAdapter->rWifiVar.rScanInfo.filterCoefficient = coefficient;
		wlanSendSetQueryCmd(prGlueInfo->prAdapter,
					CMD_ID_LWA_FILTER_COEFFICIENT,
					TRUE,
					FALSE,
					FALSE,
					NULL,
					NULL,
					sizeof(struct _CMD_LWA_FILTER_COEFFICIENT),
					(PUINT_8) &lwaFilterCoeff,
					NULL,
					0);
		rStatus = WLAN_STATUS_SUCCESS;
	} else if (strnicmp(cmd, "lwa_shm_init 0", 14) == 0) {
		lwaInit(1);
		rStatus = WLAN_STATUS_SUCCESS;
	} else if (strnicmp(cmd, "lwa_shm_init 1", 14) == 0) {
		lwaInit(0);
		rStatus = WLAN_STATUS_SUCCESS;
	} else
		return -EOPNOTSUPP;
...
int mtk_cfg80211_testmode_cmd(IN struct wiphy *wiphy, IN struct wireless_dev *wdev, IN void *data, IN int len)
{
	...
	if (prParams) {
		...
		case TESTMODE_CMD_ID_STR_CMD:
			...
			break;
		case TESTMODE_CMD_ID_LWA_CONNECTION:
			setLwaConnection(((struct _LWA_CONNECTION *)data)->value);
			i4Status = 0;
			break;
		default:
			i4Status = -EINVAL;
			      ...
			      IN struct net_device *ndev, IN struct cfg80211_sched_scan_request *request)
{
	...
	for (i = 0; i < request->n_match_sets; i++) {
		if (request->match_sets == NULL || &(request->match_sets[i]) == NULL) {
			...
			prSchedScanRequest->acRssiThresold[i] = 0;
		}
	}
...
#define CHN_DIRTY_WEIGHT_UPPERBOUND 4
...
#endif
	{
		...
		PreferChannels[0].ucChannel = 0;	
		PreferChannels[1].ucChannel = 0;	
