...
extern int proc_lwa_smem_debug_init(void);
#endif
...
static void createWirelessDevice(void)
{
	...
	prWiphy->max_scan_ssids = GL_CFG80211_SCAN_SSID_MAX_NUM; 
	prWiphy->max_scan_ie_len = 512;
	prWiphy->max_sched_scan_ssids     = CFG_SCAN_SSID_MATCH_MAX_NUM;
...
#define KAL_WLAN_REMOVE_TIMEOUT_MSEC			3000
...
#ifdef FW_CFG_SUPPORT
	DBGLOG(INIT, TRACE, "Before cfgRemoveProcEntry()!\n");
	cfgRemoveProcEntry();
#endif
#ifdef WLAN_INCLUDE_PROC
	DBGLOG(INIT, TRACE, "Before procRemoveProcfs()!\n");
	procRemoveProcfs();
#endif 
...
#endif
	DBGLOG(INIT, TRACE, "Before flush_delayed_work()!\n");
	...
	DBGLOG(INIT, TRACE, "wlanNetUnregister()!\n");
	wlanNetUnregister(prDev->ieee80211_ptr);

	
	DBGLOG(INIT, TRACE, "wlanNetDestroy!\n");
	...
	DBGLOG(INIT, TRACE, "Remove wlan End!\n");
...
VOID nicConfigProcSetCamCfgWrite(BOOLEAN enabled)
{
	...
	if (ucBssIndex >= BSS_INFO_NUM)
		return;
	arPowerSaveMode[ucBssIndex].ucBssIndex = ucBssIndex;
