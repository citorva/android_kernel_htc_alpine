...
static VOID kalRemoveOneBssFromCfg80211(
	struct wiphy *wiphy, UINT_8 ucChnl, ENUM_BAND_T eBand)
{
	struct ieee80211_channel *prChannel = NULL;
	struct cfg80211_bss *prBss = NULL;
	struct ieee80211_channel *prIeeeChnls = NULL;
	UINT_8 j = 0;
	UINT_8 ucChnlSize = 0;
	...
	switch (eBand) {
	case BAND_2G4:
		prIeeeChnls = mtk_band_2ghz.channels;
		ucChnlSize = mtk_band_2ghz.n_channels;
		ieeeBand = IEEE80211_BAND_2GHZ;
		break;
	case BAND_5G:
		prIeeeChnls = mtk_band_2ghz.channels;
		ucChnlSize = mtk_band_5ghz.n_channels;
		ieeeBand = IEEE80211_BAND_5GHZ;
		break;
	default:
		DBGLOG(OID, WARN, "Wrong band %d in channel list\n", eBand);
		return;
	}
	for (j = 0; j < ucChnlSize; j++, prIeeeChnls++) {
		if (prIeeeChnls->hw_value != ucChnl)
			continue;
		if (prIeeeChnls->flags & IEEE80211_CHAN_DISABLED) {
			DBGLOG(OID, INFO, "Channel %d was disabled\n", ucChnl);
			break;
		}
		prChannel = ieee80211_get_channel(wiphy, ieee80211_channel_to_frequency(ucChnl, ieeeBand));
		if (!prChannel) {
			DBGLOG(OID, WARN, "ieee80211 get channel failed, chnl %d, band %d\n", ucChnl, ieeeBand);
			break;
		}
		j = 255; /* Currently, j will be not used in for loop, reuse here */
		do {
			prBss = cfg80211_get_bss(wiphy, prChannel, NULL, NULL, 0,
					WLAN_CAPABILITY_ESS, WLAN_CAPABILITY_ESS);
			if (!prBss)
				break;
			cfg80211_unlink_bss(wiphy, prBss);
		} while (--j > 0); /* Avoid dead loop */
		DBGLOG(OID, INFO, "total %d BSSes were removed on channel %d\n", 255-j, ucChnl);
		break;
	}
}

VOID kalClearScanCacheByBand(P_ADAPTER_T prAdapter, ENUM_BAND_T eBand)
{
	RF_CHANNEL_INFO_T aucChannelList[mtk_band_2ghz.n_channels + mtk_band_5ghz.n_channels];
	UINT_8 i;
	UINT_8 ucNumOfChannel = 0;
	struct wiphy *prWiphy = NULL;

	switch (eBand) {
	case BAND_NULL:
		ucNumOfChannel = mtk_band_2ghz.n_channels + mtk_band_5ghz.n_channels;
		break;
	case BAND_2G4:
		ucNumOfChannel = mtk_band_2ghz.n_channels;
		break;
	case BAND_5G:
		ucNumOfChannel = mtk_band_5ghz.n_channels;
		break;
	default:
		DBGLOG(OID, ERROR, "Wrong band %d\n", eBand);
		return;
	}
	prWiphy = priv_to_wiphy(prAdapter->prGlueInfo);

	rlmDomainGetChnlList(prAdapter, eBand, FALSE, ucNumOfChannel, &ucNumOfChannel, aucChannelList);
	for (i = 0; i < ucNumOfChannel; i++)
		kalRemoveOneBssFromCfg80211(prWiphy, aucChannelList[i].ucChannelNum, aucChannelList[i].eBand);
}
