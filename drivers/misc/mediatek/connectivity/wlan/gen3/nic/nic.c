...
/* prAdapter->rWlanInfo.ePowerSaveMode.ucPsProfile = (UINT_8)ePwrMode; */
	...
	if (fgForceSetCAM && (ucBssIndex == prAdapter->prAisBssInfo->ucBssIndex))
		return WLAN_STATUS_SUCCESS;
...
VOID nicInitSystemService(IN P_ADAPTER_T prAdapter)
{
	...
	DBGLOG(INIT, INFO, "lwa timer, before cnmTimerInitialized\n");
	cnmTimerInitialize(prAdapter);
	DBGLOG(INIT, INFO, "lwa timer, after cnmTimerInitialized\n");
...
VOID nicUninitSystemService(IN P_ADAPTER_T prAdapter)
{
	...
	/* Timer Destruction */
	DBGLOG(INIT, INFO, "lwa timer, before cnmTimerUNInitialized\n");
