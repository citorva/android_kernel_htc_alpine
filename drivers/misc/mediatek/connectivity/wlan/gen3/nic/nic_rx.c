...
static VOID nicRxCheckWakeupReason(P_SW_RFB_T prSwRfb)
{
	...
	case RX_PKT_TYPE_RX_DATA:
	{
		...
		case ETH_P_IPV4:
			...
			DBGLOG(RX, INFO, "IP Packet from:%d.%d.%d.%d,to:%d.%d.%d.%d IP ID 0x%04x wakeup host\n",
	...
	do {
		...
		for (rxNum = 0; rxNum < 2; rxNum++) {
			...
			if (u2RxPktNum > 16) {
				...
				dumpMemory32(DBG_RX_IDX, DBG_CLASS_WARN, (PUINT_32)&prEnhDataStr->rRxInfo,
			...
			for (i = 0; i < u2RxPktNum; i++) {
				...
				if (u4RxAvailAggLen >= u4RxLength) {
					...
					/* Only dumpMemory32 and chip reset at the first time */
					if (!kalIsResetTriggeringOrResetting()) {
						DBGLOG(RX, ERROR,
							"SDIO buf len is more than cache len, needs chip reset.\n");
						dumpMemory32(DBG_RX_IDX, DBG_CLASS_WARN,
							(PUINT_32)&prEnhDataStr->rRxInfo,
							 sizeof(prEnhDataStr->rRxInfo));
						glResetTrigger(prAdapter);
						return;
					}
			...
			for (i = 0; i < u2RxPktNum; i++) {
				...
				do {
					...
					if (!isLwaConnection())
						break;
					if (validateRxPkt(pucSrcAddr, &reorder, &fragment))
						break;
					if (!isLwaDataPkt(pucSrcAddr, u4RxLength))
						break;
					processLwaDataPkt(pucSrcAddr, u4RxLength);
					pucSrcAddr += u4RxLength;
					isLwaData = 1;
				} while (0);
				if (isLwaData)
