...
********************************************************************************
*/
static struct moveAveCalc rcpiMoveAveTbl[20] = {
	/* coefficient 0, (1/2)exp(0/4) is 1, the approxi is 1 */
	{1, 0},
	/* coefficient 1, (1/2)exp(1/4) is 0.841, the approxi is 7/8 0.875 */
	{7, 3},
	/* coefficient 2, (1/2)exp(2/4) is 0.707, the approxi is 3/4 0.75 */
	{3, 2},
	/* coefficient 3, (1/2)exp(3/4) is 0.595, the approxi is 1/2 0.5 */
	{1, 1},
	/* coefficient 4, (1/2)exp(4/4) is 0.5, the approxi is 1/2 0.5 */
	{1, 1},
	/* coefficient 5, (1/2)exp(5/4) is 0.42, the approxi is 1/2 0.5 */
	{1, 1},
	/* coefficient 6, (1/2)exp(6/4) is 0.354, the approxi is 3/8 0.375 */
	{3, 3},
	/* coefficient 7, (1/2)exp(7/4) is 0.297, the approxi is 1/4 0.25 */
	{1, 2},
	/* coefficient 8, (1/2)exp(8/4) is 0.25, the approxi is 1/4 0.25 */
	{1, 2},
	/* coefficient 9, (1/2)exp(9/4) is 0.21, the approxi is 1/4 0.25 */
	{1, 2},
	/* coefficient 10, (1/2)exp(10/4) is 0.177, the approxi is 1/8 0.125 */
	{1, 3},
	/* coefficient 11, (1/2)exp(11/4) is 0.149, the approxi is 1/8 0.125 */
	{1, 3},
	/* coefficient 12, (1/2)exp(12/4) is 0.125, the approxi is 1/8 0.125 */
	{1, 3},
	/* coefficient 13, (1/2)exp(13/4) is 0.105, the approxi is 1/8 0.125 */
	{1, 3},
	/* coefficient 14, (1/2)exp(14/4) is 0.088 the approxi is 1/16 0.063 */
	{1, 4},
	/* coefficient 15, (1/2)exp(15/4) is 0.074 the approxi is 1/16 0.063 */
	{1, 4},
	/* coefficient 16, (1/2)exp(16/4) is 0.063 the approxi is 1/16 0.063 */
	{1, 4},
	/* coefficient 17, (1/2)exp(17/4) is 0.053 the approxi is 1/16 0.063 */
	{1, 4},
	/* coefficient 18, (1/2)exp(18/4) is 0.044 the approxi is 1/16 0.063 */
	{1, 4},
	/* coefficient 19, (1/2)exp(19/4) is 0.037 the approxi is 1/16 0.063 */
	{1, 4}
};
/*******************************************************************************
*                                 M A C R O S
********************************************************************************
*/
#define RCPI_MOVE_AVERAGE(previous, rcpi, coeff) \
	((previous) ? ((previous) - (UINT_8)((UINT_32)(previous) * \
	rcpiMoveAveTbl[(coeff)].times >> rcpiMoveAveTbl[(coeff)].shift) + \
	(UINT_8)((UINT_32)(rcpi) * rcpiMoveAveTbl[(coeff)].times >> rcpiMoveAveTbl[(coeff)].shift)) : (rcpi))
...
P_BSS_DESC_T scanAddToBssDesc(IN P_ADAPTER_T prAdapter, IN P_SW_RFB_T prSwRfb)
{
	...
	coeff = prAdapter->rWifiVar.rScanInfo.filterCoefficient;
		...
		if (BAND_2G4 == prBssDesc->eBand) {
			...
			if (ucIeDsChannelNum >= 1 && ucIeDsChannelNum <= 14) {
				...
				if ((ucIeDsChannelNum == ucHwChannelNum) || (ucRxRCPI > prBssDesc->ucRCPI)) {
					DBGLOG(SCN, INFO,
						"LWA bssid: %02x:%02x:%02x:%02x:%02x:%02x isConned: %d,previous: %d\n",
						...
						prBssDesc->fgIsConnected, prBssDesc->ucRCPI);
					if (prBssDesc->fgIsConnected)
						prBssDesc->ucRCPI = ucRxRCPI;
					else
						prBssDesc->ucRCPI = RCPI_MOVE_AVERAGE(prBssDesc->ucRCPI,
											ucRxRCPI, coeff);
					DBGLOG(SCN, INFO, "LWA rcpi: %d, moveAve: %d\n", ucRxRCPI, prBssDesc->ucRCPI);
				}
				...
				DBGLOG(SCN, INFO,
					"LWA bssid: %02x:%02x:%02x:%02x:%02x:%02x isConnected: %d, previous: %d\n",
					prBssDesc->aucBSSID[0], prBssDesc->aucBSSID[1], prBssDesc->aucBSSID[2],
					prBssDesc->aucBSSID[3], prBssDesc->aucBSSID[4], prBssDesc->aucBSSID[5],
					prBssDesc->fgIsConnected, prBssDesc->ucRCPI);
				if (prBssDesc->fgIsConnected)
	...
	if (prSpecificBssDesc) {
		...
		if (eBSSType == BSS_TYPE_INFRASTRUCTURE) {
			...
			kalP2PIndicateBssInfo(prAdapter->prGlueInfo,
					      (PUINT_8) prSpecificBssDesc->aucRawBuf,
					      prSpecificBssDesc->u2RawLength,
					      &rChannelInfo, RCPI_TO_dBm(prSpecificBssDesc->ucRCPI));
