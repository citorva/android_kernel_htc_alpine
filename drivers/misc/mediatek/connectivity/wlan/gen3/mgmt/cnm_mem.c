...
VOID cnmMemFree(IN P_ADAPTER_T prAdapter, IN PVOID pvMemory)
{
	...
	if (prBufInfo->aucAllocatedBlockNum[u4BlockIndex] == 0) {
		/* add debug info before ASSERT */
		DBGLOG(CNM, ERROR, "pvMemory=%ld, &prAdapter->rMsgBufInfo=%ld, &prAdapter->aucMsgBuf[0]=%ld\n",
			(ULONG)pvMemory, (ULONG)&prAdapter->rMsgBufInfo, (ULONG)&prAdapter->aucMsgBuf[0]);

		DBGLOG(CNM, ERROR, "&prAdapter->rMgtBufInfo=%ld, prAdapter->pucMgtBufCached=%ld\n",
			(ULONG)&prAdapter->rMgtBufInfo, (ULONG)prAdapter->pucMgtBufCached);

		DBGLOG(CNM, ERROR, "prBufInfo->pucBuf=%ld, u4BlockIndex=%u\n",
			(ULONG)prBufInfo->pucBuf, u4BlockIndex);

		DBGLOG(CNM, ERROR, "aucAllocatedBlockNum[u4BlockIndex]=%d, prBufInfo->rFreeBlocksBitmap=%u\n",
			prBufInfo->aucAllocatedBlockNum[u4BlockIndex], prBufInfo->rFreeBlocksBitmap);

		DBGLOG(CNM, ERROR, "prBufInfo->aucAllocatedBlockNum[0-32]\n");
		for (i = 0; i < 32; i++)
