...

	{
		...
		if (prCmdKey->ucTxKey) {
			...
			if (fgAddTxBcKey || !prStaRec) {
				...
				prBssInfo->ucBMCWlanIndex = prCmdKey->ucWlanIndex;	
		...
		if (prCmdKey->ucWlanIndex < WTBL_SIZE) {
			...
			if (prCmdKey->ucTxKey) {
				if (prStaRec) {
					...
					prWtblCfg[entry].ucMUARIdx = 0x00;	
				} else {
#if 0
					if (prCmdKey->ucIsAuthenticator) {
			 ...
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	if (u4SetBand == QCA_SETBAND_5G) {
		band = BAND_5G;
		scanRemoveBssDescByBandAndNetwork(prAdapter, BAND_2G4, ucBssIdx);
		
		
	} else if (u4SetBand == QCA_SETBAND_2G) {
		band = BAND_2G4;
		scanRemoveBssDescByBandAndNetwork(prAdapter, BAND_5G, ucBssIdx);
		
	} else {
		band = BAND_NULL;
	}

	prAdapter->aePreferBand[ucBssIdx] = band;
