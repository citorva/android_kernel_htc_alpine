...
wlanoidSetConnect(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	DBGLOG(OID, LOUD, "ssid %s, bssid %pM, conn policy %d, disc reason %d\n",
			...
			IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	...
	*pu4SetInfoLen = sizeof(PARAM_CUSTOM_WMM_PS_TEST_STRUCT_T);
	...
	rStatus = wlanSendSetQueryCmd(prAdapter, CMD_ID_SET_WMM_PS_TEST_PARMS, TRUE, FALSE, TRUE, NULL,	
...
batchSetCmd(IN P_ADAPTER_T prAdapter, IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4WritenLen)
{
	...
	*pu4WritenLen = 0;
	...
	if (!kalStrnCmp(head, BATCHING_SET, kalStrLen(BATCHING_SET))) {
		...
		p = head + kalStrLen(PARAM_CHANNEL) + 2;	
	    ...
	    OUT PVOID pvQueryBuffer, IN UINT_32 u4QueryBufferLen, OUT PUINT_32 pu4QueryInfoLen)
{
	...
	*pu4QueryInfoLen = sizeof(EVENT_BATCH_RESULT_T);
	...
	rCmdBatchReq.ucMScan = prEventBatchResult->ucScanCount;	
...
WLAN_STATUS
wlanoidSetBand(IN P_ADAPTER_T prAdapter,
			 IN PVOID pvSetBuffer, IN UINT_32 u4SetBufferLen, OUT PUINT_32 pu4SetInfoLen)
{
	UINT_32 u4SetBand;
	ENUM_BAND_T band;
	...
	u4SetBand = *(PUINT_32)pvSetBuffer;

	if (u4SetBand == QCA_SETBAND_5G) {
		band = BAND_5G;
		scanRemoveBssDescByBandAndNetwork(prAdapter, BAND_2G4, NETWORK_TYPE_AIS_INDEX);
	} else if (u4SetBand == QCA_SETBAND_2G) {
		band = BAND_2G4;
		scanRemoveBssDescByBandAndNetwork(prAdapter, BAND_5G, NETWORK_TYPE_AIS_INDEX);
	} else {
		band = BAND_NULL;
	}

	prAdapter->aeSetBand[NETWORK_TYPE_AIS_INDEX] = band;

	DBGLOG(REQ, INFO, "wlanoidSetBand, band= %d\n", band);
