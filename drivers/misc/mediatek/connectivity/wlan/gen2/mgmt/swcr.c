...
SWCR_MOD_MAP_ENTRY_T g_arSwCrAllMaps[] = {
	{SWCR_MAP_NUM(g_arRlmArSwCrMap), g_arRlmArSwCrMap},	
	{0, NULL}
};
#endif

UINT_32 g_au4SwCr[SWCR_CR_NUM];	

UINT_32 g_u4mDNSRXFilter = 0;	
		...
		case SWCTRL_QM_INFO:
			{
				...
				case 2:
					g_au4SwCr[1] = prQM->arTxQueue[ucOpt1].u4NumElem;	
		       ...
		       IN BOOLEAN fgIsBurstEnd, IN BOOLEAN ucPacketType, IN BOOLEAN ucPsSessionID, IN BOOLEAN fgSetEOSP)
{
	...
	prMsduInfo->ucPsSessionID = ucPsSessionID ;
...
VOID testPsCmdCategory0(P_ADAPTER_T prAdapter, UINT_8 ucCate, UINT_8 ucAction, UINT_8 ucOpt0, UINT_8 ucOpt1)
{
	...
	if (ucRead == SWCR_WRITE) {
		...
		case TEST_PS_SEND_NULL:
			{
				...
				testPsSendQoSNullFrame(prAdapter, prStaRec, (UINT_8) (g_au4SwCr[1] & 0xFF),	
						       ucOpt0, (BOOLEAN) ((g_au4SwCr[1] >> 8) & 0xFF),	
						       (BOOLEAN) ((g_au4SwCr[1] >> 16) & 0xFF),	
						       (BOOLEAN) ((g_au4SwCr[1] >> 24) & 0xFF),	
						       (UINT_8) ((g_au4SwCr[2]) & 0xFF), 
						       FALSE	
				    );
			}
			break;
		...
		case TEST_PS_UPDATE_BEACON:
			bssUpdateBeaconContent(prAdapter, ucOpt0 );
			break;
