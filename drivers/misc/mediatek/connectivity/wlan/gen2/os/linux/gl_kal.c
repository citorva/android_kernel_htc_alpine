...
{
	KAL_WAKE_LOCK_TIMEOUT(prGlueInfo->prAdapter, &(prGlueInfo->rAhbIsrWakeLock), (HZ / 10));	
}
...
kalIndicateStatusAndComplete(IN P_GLUE_INFO_T prGlueInfo, IN WLAN_STATUS eStatus, IN PVOID pvBuf, IN UINT_32 u4BufLen)
{
	...
	case WLAN_STATUS_MEDIA_CONNECT:
		...
		do {
			...
			DBGLOG(AIS, LOUD, " %s netif_carrier_on [ssid:%s %pM ]\n",
				   ...
				   OUT PVOID prGenUse)
{
	...
	*pfgIsPAL = FALSE;
	...
	}  else if (u2EtherTypeLen == ETH_P_1X || u2EtherTypeLen == ETH_P_PRE_1X) {	
