...
int htc_reboot_reason_update_by_cmd(const char* cmd) {
	...
	if (htc_reboot_info.initialized) {
		if (cmd == NULL) {
			...
			} else if(HTC_REBOOT_CMD_CMP("dm-verity device corrupted", cmd)) {
				ret = htc_set_reboot_reason(HTC_REBOOT_INFO_LOGGING, cmd);
			} else {
...
static int htc_reboot_info_initialize(void) {
	...
	htc_reboot_info.htc_reboot_info_p = (htc_reboot_info_struct*) ioremap_nocache(htc_reboot_info.phys, sizeof(htc_reboot_info_struct));
	...
	htc_reboot_info.last_htc_reboot_info_p = (htc_reboot_info_struct*) ioremap_nocache(htc_reboot_info.phys + sizeof(htc_reboot_info_struct), sizeof(htc_reboot_info_struct));
	...
	if (!htc_is_valid_reboot_info()) {
		memset_io(htc_reboot_info.htc_reboot_info_p, 0, sizeof(htc_reboot_info_struct));
	}

	memset_io(htc_reboot_info.htc_reboot_info_p, 0, sizeof(htc_reboot_info_struct));
	htc_reboot_info.last_htc_reboot_info_p->reboot_string[HTC_REBOOT_INFO_REBOOT_STRING_SIZE-1] = 0;

	
	htc_reboot_info.htc_reboot_info_p->reboot_reason = HTC_REBOOT_INFO_RAMDUMP;
	strncpy(htc_reboot_info.htc_reboot_info_p->reboot_string, "UNKNOWN",
...
int htc_set_reboot_reason(unsigned int reboot_reason, const unsigned char* reboot_string) {
	...
	if (htc_reboot_info.initialized && htc_reboot_info.htc_reboot_info_p) {
		...
		if (reboot_reason == HTC_REBOOT_INFO_RAMDUMP) {
			is_ramdump_set = 1;
		}

		pr_crit("%s: reboot_reason: 0x%.8X\n", __func__, htc_reboot_info.htc_reboot_info_p->reboot_reason);
		pr_crit("%s: reboot_string: %s\n", __func__, htc_reboot_info.htc_reboot_info_p->reboot_string);
		return 0;
	}
	return -1;
}
...
int htc_set_reboot_reason_ramdump_by_panic(const unsigned char* msg)
{
	...
	if (msg != NULL) {
		if (0 == strcmp(msg, "Powerkey Hard Reset"))
