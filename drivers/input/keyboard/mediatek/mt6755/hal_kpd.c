...
ssize_t long_press_reboot_function_setting_print(char *buf, ssize_t bufLen)
{
	int len = 0;
	len += snprintf(buf + len, bufLen - len, "PMIC_RG_PWRKEY_RST_TD:%x\n", pmic_get_register_value(PMIC_RG_PWRKEY_RST_TD));
	len += snprintf(buf + len, bufLen - len, "PMIC_RG_PWRKEY_RST_EN:%x\n", pmic_get_register_value(PMIC_RG_PWRKEY_RST_EN));
	len += snprintf(buf + len, bufLen - len, "PMIC_RG_HOMEKEY_RST_EN:%x\n", pmic_get_register_value(PMIC_RG_HOMEKEY_RST_EN));
	return len;
}

void do_kick_long_press_reboot_timer(void)
{
#if 0 // way_1: write 0x02B6 bit 10 to 1 and then 0
	if(pmic_get_register_value(PMIC_RG_PWRKEY_RST_EN)==1 &&
		pmic_get_register_value(PMIC_RG_HOMEKEY_RST_EN)==1){

		pmic_set_register_value(PMIC_RG_PWRKEY_RST_EN, 0x00);
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x00);
		/* For reliable hardware operation, we delay 100ms. */
		mdelay(100);
		pmic_set_register_value(PMIC_RG_PWRKEY_RST_EN, 0x01);
		pmic_set_register_value(PMIC_RG_HOMEKEY_RST_EN, 0x00);
		mdelay(100);
	}
#else // way_2: wirte 0x002E bit 0 to 1
