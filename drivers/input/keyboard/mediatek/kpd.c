...
#define KPD_NAME	"mediatek,mt6755-keypad"
...
unsigned int key_up;
unsigned int key_col0;
unsigned int key_col1;
...
static ssize_t long_press_reboot_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return long_press_reboot_function_setting_print(buf, PAGE_SIZE);
}

static ssize_t kick_long_press_reboot_timer(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	do_kick_long_press_reboot_timer();
	return count;
}

static DEVICE_ATTR(kick_long_press_reboot_timer,      0664, long_press_reboot_show,   kick_long_press_reboot_timer);

int htc_long_press_reboot_attr(struct device *dev)
{
	int err = 0;

	if (!dev)
		return -EINVAL;
	err = device_create_file(dev, &dev_attr_kick_long_press_reboot_timer);

	return err;
}

static int kpd_key_up_init(struct platform_device *pdev)
{
	struct pinctrl *kpd_pinctrl;
	struct pinctrl_state *kpd_keyup_default;
	int ret = 0;

	struct device_node *node1 = pdev->dev.of_node;
	if(node1)
	{
		of_property_read_u32(node1, "key_up", &key_up);

		gpio_request(key_up, "key_volup");

		of_property_read_u32(node1, "key_col0", &key_col0);
		of_property_read_u32(node1, "key_col1", &key_col1);
	}
	kpd_pinctrl = devm_pinctrl_get(&pdev->dev);
	if(IS_ERR(kpd_pinctrl))
	{
		ret = PTR_ERR(kpd_pinctrl);
		kpd_err("Can't get keypad pinctrl.\n");
		return ret;
	}
	kpd_keyup_default = pinctrl_lookup_state(kpd_pinctrl, "key_up_pins_default");
	if(IS_ERR(kpd_keyup_default))
	{
		ret = PTR_ERR(kpd_keyup_default);
		kpd_err("Can't find keypad key_up state.\n");
		return ret;
	}

	ret = pinctrl_select_state(kpd_pinctrl, kpd_keyup_default);
	if(ret < 0)
	{
		kpd_err("keypad setting state failed!\n");
	}
	return ret;
}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	err = kpd_key_up_init(pdev);
	if(err < 0)
	{
		kpd_err("keypad fail to init key_up pin!\n");
		return err;
	}
	...
	r = htc_long_press_reboot_attr(kpd_dev.this_device);

	if (r) {
		kpd_info("create htc longPressToReset attr failed (%d)\n", r);
	}
