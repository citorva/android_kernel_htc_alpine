...
static void gpio_keys_gpio_work_func(struct work_struct *work)
{
	...
	if ((gpio_get_value(bdata->button->gpio) ? 1 : 0) ^ bdata->button->active_low)
...
#if defined(MTK_PLATFORM)
		if ((gpio_get_value(bdata->button->gpio)? 1 : 0) ^ button->active_low)
...
gpio_keys_get_devtree_pdata(struct device *dev)
{
	...
	for_each_child_of_node(node, pp) {
		...
		if (of_property_read_u32(pp, "gpios", &gpio)) {
			error = -EINVAL;
			dev_err(dev, "Failed to get property: gpios, error: %d\n", error);
			kfree(pdata);
...
err_pinctrl:
	if (ddata->key_pinctrl) {
		set_state = pinctrl_lookup_state(ddata->key_pinctrl, "tlmm_gpio_key_suspend");
		if (IS_ERR(set_state))
