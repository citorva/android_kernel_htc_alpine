...
static int himax_tpd_rst_gpio_number = 82;
static int himax_tpd_int_gpio_number = 116;

static int self_test_inter_flag = 0;
static unsigned int tamper_flag = 0;

static uint8_t g_cap_fw_file_ver_maj = 0x00;
static uint8_t g_cap_fw_file_ver_min = 0x00;
...
static int i2c_himax_write(struct i2c_client *client, uint8_t command, uint8_t *buf, uint8_t len, uint8_t toRetry)
{
	int rc=0,retry=0;
...
static int i2c_himax_master_write(struct i2c_client *client, uint8_t *buf, uint8_t len, uint8_t toRetry)
{
	int rc=0, retry=0;
	u8 *pWriteData = gpDMABuf_va;
...
static void himax_read_CAP_info(struct i2c_client *client, bool b_show);

static int himax_cap_fw_update(struct firmware *vfw)
{
	...
	if (vfw->data[0] == 'C' && vfw->data[1] == 'S') {
		...
		if (index != NULL) {
			...
			if (*(index + strlen(name)) == '_') {
				...
				g_cap_fw_file_ver_maj = file_ver_maj;
				g_cap_fw_file_ver_min = file_ver_min;
				...
				if (file_ver_maj != ts->vendor_fw_ver_H ||
				    file_ver_min != ts->vendor_fw_ver_L ||
				    file_cfg_ver != ts->vendor_config_ver) {
				    I("[FW] FW version difference. Need Update\n");
...
doFirmwareUpdate_Retry:
	...
	himax_read_CAP_info(private_ts->client, true);
...
static int updateFirmware(void *arg)
{
	...
	if (ret == 0)
		ret = himax_cap_fw_update((struct firmware *)ts->fw);
...
{
	himax_tp_notifier.fwupdate = himax_cap_fw_update;
...
static void himax_read_CAP_info(struct i2c_client *client,bool b_show)
{
	...
	if (count++ <= 1)
	{
		I("sensor_id=%x, fw_ver=%02X,%02X,CFG=%02X, fw_file_ver=%02X,%02X,CFG=%02X.\n",private_ts->vendor_sensor_id,
			private_ts->vendor_fw_ver_H,private_ts->vendor_fw_ver_L, private_ts->vendor_config_ver,
			g_cap_fw_file_ver_maj, g_cap_fw_file_ver_min, g_cap_fw_file_cfg_ver);
		I("fw_update_complete=%s.\n", fw_update_complete ? "True":"False");
	}
...
bypass_checksum_failed_packet:
	...
	if (buf[HX_TOUCH_INFO_POINT_CNT] == 0xff
		...
		if (!((vkey_previous ^ vkey_status) & BIT(0)) &&
			!((vkey_previous ^ vkey_status) & BIT(1))) {
			I("All Finger leave\n");
			return;
		}
...
static int touch_event_handler(void *ptr)
{
	...
	do
	{
		...
		if (atomic_read(&private_ts->suspend_mode)&&(!FAKE_POWER_KEY_SEND)&&(private_ts->SMWP_enable)) {
			...
			himax_int_enable_cap(private_ts->client->irq,1,false);
			continue;
		}
#endif
...
int himax_ts_register_interrupt_cap(struct i2c_client *client)
{
	...
	if (node) {
		...
		himax_touch_irq_cap = irq_of_parse_and_map(node, 0);

		I("hxtp:himax_touch_irq_cap=%ud \n",himax_touch_irq_cap);

		client->irq = himax_touch_irq_cap;
		ts->client->irq = himax_touch_irq_cap;
	...
	if(ts->pdata->gpio_irq > 0 && himax_int_gpio_read_cap(ts->pdata->gpio_irq)==0)
	{
		for (i=0;i<50;i++)
		{
			ret = i2c_himax_read(ts->client, 0x86, buf, 128,DEFAULT_RETRY_CNT);
...
#if 0	
#ifdef CONFIG_OF_TOUCH
				ret = request_irq(client->irq, tpd_eint_interrupt_handler, IRQF_TRIGGER_FALLING, HIMAX852xes_NAME_CAP, NULL);
				if(ret > 0){
				    ret = -1;
...
#if 0	
#ifdef CONFIG_OF_TOUCH
				ret = request_irq(client->irq, tpd_eint_interrupt_handler, IRQF_TRIGGER_LOW, HIMAX852xes_NAME_CAP, NULL);
				if(ret > 0){
				    ret = -1;
				E("tpd request_irq IRQ LINE NOT AVAILABLE\n");
				}
...
if (0) {	
		touch_thread = kthread_run(touch_event_handler, 0, CAP_DEVICE);
...
static DEVICE_ATTR(attn, (S_IRUGO), attn_show, NULL);
...
static DEVICE_ATTR(self_test, (S_IWUSR | S_IRUGO), himax_chip_self_test_function, NULL);
#endif
...
#ifdef HX_TP_SYS_SELF_TEST
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_self_test.attr);
	if (ret) {
		E("sysfs_create_file dev_attr_self_test failed\n");
		return ret;
	}
#endif
...
#ifdef HX_TP_SYS_SELF_TEST
	sysfs_remove_file(android_touch_kobj, &dev_attr_self_test.attr);
#endif
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	if (of_property_read_u32(dt, "himax,irq-gpio", &data) == 0) {
		pdata->gpio_irq = (data);
		if (!gpio_is_valid(pdata->gpio_irq)) {
			I(" DT:gpio_irq value is not valid\n");
		} else
			himax_tpd_int_gpio_number = pdata->gpio_irq;
	}
	else {
		pdata->gpio_irq = -1;
	}

	if (of_property_read_u32(dt, "himax,rst-gpio", &data) == 0) {
		pdata->gpio_reset = (data);
		if (!gpio_is_valid(pdata->gpio_reset)) {
			I(" DT:gpio_reset value is not valid\n");
		} else
			himax_tpd_rst_gpio_number = pdata->gpio_reset;
	}
	else {
		pdata->gpio_reset = -1;
	}
...
static int himax_pinctrl_init(struct device *dev)
{
	...
	gpio_state_active
		= pinctrl_lookup_state(ts_pinctrl, "pmx_ts_power_on");
	if (IS_ERR_OR_NULL(gpio_state_active)) {
		I("Can not get ts default pinstate\n");
		ret = PTR_ERR(gpio_state_active);
		ts_pinctrl = NULL;
		return ret;
	}
	...
	gpio_reset_active
		= pinctrl_lookup_state(ts_pinctrl, "pmx_ts_reset_active");
	...
	gpio_reset_suspend
		= pinctrl_lookup_state(ts_pinctrl, "pmx_ts_reset_suspend");
...
static int himax_pinctrl_select(struct device *dev, int on)
{
	...
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(ts_pinctrl, pins_state);
		if (ret) {
...
static void himax_rst_gpio_set_cap(int pinnum, uint8_t value)
{
	...
	pins_state = value ? gpio_reset_suspend
		     : gpio_reset_active;
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(ts_pinctrl, pins_state);
		if (ret) {
			E("can not set %s pins\n",
			  value ? "pmx_ts_reset_suspend" : "pmx_ts_reset_active");
		}
	} else
		E("not a valid '%s' pinstate\n",
		  value ? "pmx_ts_reset_suspend" : "pmx_ts_reset_active");
...
static int himax_gpio_power_config_cap(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	if (pdata->gpio_3v3_en > 0 && gpio_is_valid(pdata->gpio_3v3_en)) {
	...
	himax_rst_gpio_set_cap(pdata->gpio_reset, GPIO_OUT_ONE);

	msleep(20);

	himax_rst_gpio_set_cap(pdata->gpio_reset, GPIO_OUT_ZERO);
...
of_get_himax85xx_platform_data(&client->dev);
	...
	himax_read_CAP_info(client, true);
#ifdef HX_AUTO_UPDATE_FW
...
void himax852xes_suspend_cap(struct device *dev)
{
	...
	himax_read_CAP_info(hx_i2c_client_point, false); 
...
#if 0
	if (ts->cap_glove_mode_status == 1) {
		hx_sensitivity_setup(ts, CS_SENS_HIGH);
	} else if (ts->cap_glove_mode_status == 0)
		hx_sensitivity_setup(ts, CS_SENS_DEFAULT);
#ifdef CONFIG_AK8789_HALLSENSOR
	if(ts->cap_hall_s_pole_status == HALL_NEAR)
		himax_int_enable_cap(ts->client->irq, 0, true);
#endif
#endif
