...
#define FTS_PACKET_LENGTH	 8
#define FTS_PARAM_PACKET_LENGTH	 64
...
static unsigned char CTPM_FW[] = {
		...
		#include "FT8716_FW_Reference_V0F_D04_20161010_app.i"
...
{
	{0x54,FTS_MAX_POINTS_10,AUTO_CLB_NONEED,2, 2, 0x54, 0x2c, 20, 2000}, 	
...
int hidi2c_to_stdi2c(struct i2c_client * client)
{
	...
	ret = fts_i2c_write(client, auc_i2c_write_buf, 3);
	if(ret < 0)
	{
		E("i2c write addr: 0x%x failed!\n", auc_i2c_write_buf[0]);
	}
	msleep(10);
...
static int in_flash = 0;
int update_flash_progress(int percentage)
{
	if(in_flash)
...
int fts_ctpm_fw_upgrade_ReadVendorID(struct i2c_client *client, u8 *ucPVendorID)
{
	...
	*ucPVendorID = 0;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			E("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			continue;
		}
	}
...
#endif
	if (ret<0 || chip_id != FTS_CHIP_ID)
	{
		E("Fail to read chip_id: %d\n", chip_id);
		chip_id = FTS_CHIP_ID;
		I("Force to set chip_id: 0x%x\n", chip_id);
	}
...
int fts_6x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	packet_buf = kzalloc(FTS_PACKET_LENGTH + 8, GFP_KERNEL);
	if(packet_buf == NULL)
	{
		E("fail to kzalloc memory!\n");
		return 0;
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			E("[FTS] Step 3: GET CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
		}
	}
	if (i >= FTS_UPGRADE_LOOP)
	...
	if (reg_val[0] != bt_ecc) 
	{
		E("[FTS]--ecc error! FW=%02x bt_ecc=%02x\n",
					reg_val[0],
					bt_ecc);
		return -EIO;
	}
...
int fts_6x06_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		{
			E("[FTS] Step 3: CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0], reg_val[1]);
		}
	}
	if (i > FTS_UPGRADE_LOOP)
		return -EIO;
...
int fts_5x36_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++)
	{
		...
		{
			E("[FTS] Step 3: CTPM ID FAILD,ID1 = 0x%x,ID2 = 0x%x\n", reg_val[0], reg_val[1]);
			continue;
		}
...
int fts_5822_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth)
{
	...
	for (i = 0; i < 100; i++) 
	{
		...
		reg_val[0] = reg_val[1] = 0x00;
		fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
		E("[FTS]--reg_val[0]=%02x reg_val[0]=%02x\n", reg_val[0], reg_val[1]);
...
int  fts_8716_writepram(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
	...
	u32 temp;
	u32 lenght;
	u8 packet_buf[FTS_PARAM_PACKET_LENGTH + 6];
	u8 auc_i2c_write_buf[10];
	u8 bt_ecc;
	int ret;

	TPD_DEBUG("fts_8716_writepram dw_lenth= %d\n",dw_lenth);
	if(dw_lenth > 0x10000 || dw_lenth ==0)
	{		
		return -EIO;
	}
	...
	for (i = 0; i < 20; i++) 
	{
		...
		if(fts_write_reg(client, 0xfc, FTS_UPGRADE_AA) < 0)
		{
			E("fts_write_reg write register failed!\n");
		}
		msleep(fts_updateinfo_curr.delay_aa);
		if(fts_write_reg(client, 0xfc, FTS_UPGRADE_55) < 0)
		{
			E("fts_write_reg write register failed!\n");
		}
		msleep(200);
		...
		auc_i2c_write_buf[0] = FTS_UPGRADE_55;
		ret = fts_i2c_write(client, auc_i2c_write_buf, 1);
		if(ret < 0)
		{
			TPD_DEBUG("[FTS] failed writing  0x55 ! \n");
			continue;
		...
		ret = fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2);
		if(ret < 0)
		{
			E("i2c read id failed!\n");
		}
		...
		{
			E("[FTS] Step 3: READ CTPM ID FAIL,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
	...
	temp = 0;
	packet_number = (dw_lenth) / FTS_PARAM_PACKET_LENGTH;
	packet_buf[0] = 0xae;
	packet_buf[1] = 0x00;

	for (j = 0; j < packet_number; j++) 
	{
		temp = j * FTS_PARAM_PACKET_LENGTH;
		packet_buf[2] = (u8) (temp >> 8);
		packet_buf[3] = (u8) temp;
		lenght = FTS_PARAM_PACKET_LENGTH;
		packet_buf[4] = (u8) (lenght >> 8);
		packet_buf[5] = (u8) lenght;

		for (i = 0; i < FTS_PARAM_PACKET_LENGTH; i++)
		{
			packet_buf[6 + i] = pbt_buf[j * FTS_PARAM_PACKET_LENGTH + i];
			bt_ecc ^= packet_buf[6 + i];
		}
		ret = fts_i2c_write(client, packet_buf, FTS_PARAM_PACKET_LENGTH + 6);
		if(ret < 0)
		{
			E("fts_i2c_write packet_buf failed, ret: %d!\n", ret);
			return ret;
		}
	}

	if ((dw_lenth) % FTS_PARAM_PACKET_LENGTH > 0)
	{
		temp = packet_number * FTS_PARAM_PACKET_LENGTH;
		packet_buf[2] = (u8) (temp >> 8);
		packet_buf[3] = (u8) temp;
		temp = (dw_lenth) % FTS_PARAM_PACKET_LENGTH;
		packet_buf[4] = (u8) (temp >> 8);
		packet_buf[5] = (u8) temp;

		for (i = 0; i < temp; i++) 
		{
			packet_buf[6 + i] = pbt_buf[packet_number * FTS_PARAM_PACKET_LENGTH + i];
			bt_ecc ^= packet_buf[6 + i];
		}		
		ret = fts_i2c_write(client, packet_buf, temp + 6);
		if(ret < 0)
		{
			E("fts_i2c_write packet_buf failed!\n");
		}
	}
	update_flash_progress(20);
	
	
	TPD_DEBUG("Step 6: read out checksum\n");
	auc_i2c_write_buf[0] = 0xcc;
	msleep(2);
	ret = fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 1);
	if(ret < 0)
	{
		E("Read out checksum failed!\n");
	}

	if (reg_val[0] != bt_ecc) 
	{
		E("[FTS]checksum_ecc error! FW=%02x bt_ecc=%02x\n",reg_val[0],bt_ecc);
		return -EIO;
	}
	printk(KERN_WARNING "checksum %X %X \n",reg_val[0],bt_ecc);
	msleep(100);	
...
int  fts_8716_ctpm_fw_upgrade(struct i2c_client * client, u8* pbt_buf, u32 dw_lenth)
{
    ...
    printk("xljadd log2 %s\n",__func__);

    i_ret = hidi2c_to_stdi2c(client);

	packet_buf = devm_kzalloc(&client->dev, FTS_PACKET_LENGTH + 8, GFP_KERNEL);
	if(packet_buf == NULL)
	{
		E("fail to kzalloc memory!\n");
		return 0;
	...
	TPD_DEBUG("fts_8716_ctpm_fw_upgrade dw_lenth= %d\n",dw_lenth);
	auc_i2c_write_buf[0] = 0x05;
	reg_val_id[0] = 0x00;

	if(fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val_id, 1) < 0)
	{
		E("i2c access error!\n");
		return -1;
	}
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if((ret = fts_i2c_read(client, auc_i2c_write_buf, 4, reg_val, 2)) < 0)
		{
			E("i2c access error!\n");
			return ret;
		}
		...
		if ((reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			E("[FTS] Step 3: READ FAIL CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",
				reg_val[0], reg_val[1]);
			continue;
		}
	}	
	...
	{		
		...
		cmd[1] = reg_val_id[0];
		cmd[2] = 0x00;	
		if((ret = fts_i2c_write(client, cmd, 3)) < 0)
		{
			E("i2c access error.\n");
			return ret;
		}
	...
	
	{
		cmd[0] = 0x09;
		cmd[1] = 0x0B;
		if((ret=fts_i2c_write(client, cmd, 2)) < 0)
		{
			E("i2c access error.\n");
			return ret;
		}
	...
	auc_i2c_write_buf[0] = 0x61;
	if((ret = fts_i2c_write(client, auc_i2c_write_buf, 1)) < 0)
	{
		E("i2c access error!\n");
		return ret;
	}
	...
	for(i = 0;i < 15;i++)
	{
		auc_i2c_write_buf[0] = 0x6a;
		reg_val[0] = reg_val[1] = 0x00;
		if((ret = fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2)) < 0)
		{
			E("i2c access error.\n");
			return ret;
		}
		...
		if(0xF0==reg_val[0] && 0xAA==reg_val[1])
		{
			I("fts_i2c_read:0x6a, i:%d\n", i);
			break;
		}
		msleep(50);
	}
	update_flash_progress(30);
	...
	TPD_DEBUG("dw_length: %d, packet_number: %d\n", dw_lenth, packet_number);
	...
	for (j = 0; j < packet_number; j++) {
		...
		ret = fts_i2c_write(client, packet_buf, FTS_PACKET_LENGTH + 6);
		if(ret < 0)
		{
			E("fts_i2c_write packet_buf failed, packet_number:%d,j: %d\n", packet_number, j);
		}
		...
		for(i = 0;i < 30;i++)
		{
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
			ret = fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
			if(ret < 0)
			{
				E("Error: fail to read auc_i2c_write_buf: 0x6a!\n");
			}
			I("i:%d, reg_val: %d %d\n", i, reg_val[0], reg_val[1]);
			if ((j + 0x22+0x1000) == (((reg_val[0]) << 8) | reg_val[1]))
			{
				I("break, j:%d, i:%d, reg_val: %d %d\n", j, i, reg_val[0], reg_val[1]);
				break;
			}
			msleep(1);
	...
	TPD_DEBUG("write firmware(FW) to ctpm flash reach here!\n");
	...
	TPD_DEBUG("write firmware(FW) to ctpm flash finished!\n");

	msleep(50);
	update_flash_progress(80);
	...
	auc_i2c_write_buf[4] = (u8)(temp >> 8);
	auc_i2c_write_buf[5] = (u8)(temp);
	ret = fts_i2c_write(client, auc_i2c_write_buf, 6);
	if(ret < 0)
	{
		E("i2c access error.\n");
		return ret;
	...
	if (reg_val[0] != bt_ecc) 
	{
		E("ecc_check_error! FW=%02x bt_ecc=%02x\n", reg_val[0],bt_ecc);
		return -EIO;
	}
	W("checksum %X %X \n",reg_val[0],bt_ecc);
	update_flash_progress(90);
	...
	TPD_DEBUG("reach here: ret = %d!\n", ret);
	msleep(200);

	TPD_DEBUG("Step 8: reset TP end\n");
	return 0;
}
...
int fts_ctpm_fw_upgrade_with_sys_fs(struct i2c_client *client, unsigned char *pbt_buf, int fwsize)
{
	...
	if (fwsize <= 0) 
	{
		E("%s ERROR:Get firmware size failed\n",__func__);
		return -EIO;
	}
	...
	{
		E("%s() - ERROR: request_firmware failed\n",__func__);
		kfree(pbt_buf);
		return -EIO;
	}	
	#endif
	...
	{
		ret = fts_5x06_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	...
	{
		ret = fts_5x36_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x06))
	{
		ret = fts_6x06_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x36))
	{
		ret = fts_6x36_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x64))
	{
		ret = fts_6336GU_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x54))
	{
		ret = fts_5x46_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x58))
	{
		ret =  fts_5822_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x59))
	{
		ret =  fts_5x26_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x86))
	{
	   	 
		ret = fts_8606_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));
	
		if (ret != 0)
		{
			E("%s:fts_8606_writepram failed. err.\n",__func__);
			return -EIO;
		}	
		
		ret =  fts_8606_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x87))
	{
		
    	ret = fts_8716_writepram(client, aucFW_PRAM_BOOT, sizeof(aucFW_PRAM_BOOT));
	
		if (ret != 0)
		{
			E("%s:fts_8716_writepram failed. err.\n",__func__);
			return -EIO;
		}	
		
		ret =  fts_8716_ctpm_fw_upgrade(client, pbt_buf, fwsize);
		if(ret < 0)
		{
			E("fts_8716_ctpm_fw_upgrade failed!\n");
		}
		I("focal_upgrade finished!\n");
		update_flash_progress(100);
	}
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}

	if (ret != 0)
		E("%s() - ERROR:[FTS] upgrade failed..\n",__func__);
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) 
	{
		...
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			...
			E("[FTS] Step 3 fail: CTPM ID,ID1 = 0x%x,ID2 = 0x%x, 0x%x, 0x%x: \n",
				reg_val[0], reg_val[1], fts_updateinfo_curr.upgrade_id_1, fts_updateinfo_curr.upgrade_id_2);
		}
#endif	
	}
...
static unsigned char ft8716_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	for(i = 0;i < FTS_UPGRADE_LOOP; i++) 
	{
		auc_i2c_write_buf[2] = 0x11;
		auc_i2c_write_buf[3] = 0x0c;
		i_ret = fts_i2c_write(client, auc_i2c_write_buf, 4);
		if (i_ret < 0)
		{
			DBG( "[FTS] Step 4: read vendor id from flash error when i2c write, i_ret = %d\n", i_ret);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	DBG("[FTS] uc_tp_fm_ver = 0x%x, uc_tp_vendor_id = 0x%x\n", uc_tp_fm_ver, uc_tp_vendor_id);
	...
	if((uc_tp_vendor_id != FTS_Vendor_1_ID) && (uc_tp_vendor_id != FTS_Vendor_2_ID))
	{
		...
		uc_boot_vendor_id = ft8716_ctpm_VidFWid_get_from_boot(client);
		...
		{
			DBG("[FTS] FW .i unmatched,stop upgrade\n");
			return -EIO;
		}
	}
#endif

    if(uc_tp_fm_ver>0x20)
    	uc_tp_fm_ver=0;

#ifdef	FORCE_UPGRADE
    
    uc_tp_fm_ver = FTS_REG_FW_CUR_VER;
#endif

	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
	DBG("[FTS] uc_host_fm_ver = 0x%x\n",uc_host_fm_ver);
	
	if ((uc_tp_fm_ver == FTS_REG_FW_CUR_VER) || (uc_tp_fm_ver < uc_host_fm_ver))
	...
	
	{
		...
		if (i_ret == 0)
		{
			msleep (300);
			uc_host_fm_ver = fts_ctpm_get_i_file_ver ();
			DBG("[FTS] upgrade to new version 0x%x\n", uc_host_fm_ver);
			DBG("[FTS] upgrade to new version 0x%x\n", uc_host_fm_ver);
		}
		else
		{
			pr_err("[FTS] upgrade failed ret=%d.\n", i_ret);
			E("[FTS] upgrade failed ret=%d.\n", i_ret);
			return -EIO;
		}
	}
	return 0;
