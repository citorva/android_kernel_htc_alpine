...
struct heatmap_param {
	bool scan_start;
	enum scan_data_type_list data_type; 
	int num_element;
};
#endif
...
enum htc_mfg_item {
	eHTC_MFG_NA,
	eHTC_MFG_MUTUAL_CAP_DIFF_DATA_ID_2,
	eHTC_MFG_SELF_CAP_DIFF_DATA_ID_5,
	eHTC_MFG_CM_PANEL,
	eHTC_MFG_CP_PANEL,
};

struct htc_cyttsp5_mfg_data{
	union {
		u8   u8_mutual_cap_data[4096]; 
		u8   u8_mutual_cap_Tx30_Rx17x2[30][17*2]; 
		u8   u8_mutual_cap_Tx28_Rx16x2[28][16*2]; 
	};
	u8   u8_self_cap_Tx[256];
	u8   u8_self_cap_Rx[256];
};
...
struct cyttsp5_device_access_data {
	...
	struct dentry *android_touch_dentry;
	struct htc_cyttsp5_mfg_data mfg_data;
};
...
const char htc_string_new_line[] = "\n";
const char htc_string_space[] = " ";
const char htc_string_empty[] = "";
static char htc_string_self_cap[128] = {};

static const char *htc_get_char_end_string(struct device *dev, int in_idx, int length, int hdr_sz, int new_line_pos)
{
	
	
	int index = in_idx + 1;
	const char* ret = htc_string_space;
	...
	if(index == hdr_sz && hdr_sz != 0) {
		ret = htc_string_new_line;
	}
	else if( (index > hdr_sz) &&\
		(((index-hdr_sz)%new_line_pos) == 0) ) {
		ret = htc_string_new_line;
	}
	else if(index == length) {
		ret = htc_string_new_line;
	}
	else if(in_idx == length) {
		ret = htc_string_new_line;
	}
...
static const char *htc_get_self_cap_string(struct device *dev, int in_idx, int length,
	int new_line_pos, enum htc_mfg_item item_id)
{
	
	
	int sum;
	u8 u8_data1, u8_data2;
	s16 s16_data;
	u8* buf;
	...
	index = in_idx + 1;
	buf = htc_string_self_cap;
	ret = htc_string_empty;

	
	if( (index % new_line_pos) == 0) {
		self_cap_index = index/new_line_pos - 1;
		u8_data1 = dad->mfg_data.u8_self_cap_Rx[self_cap_index*2];
		u8_data2 = dad->mfg_data.u8_self_cap_Rx[self_cap_index*2 + 1];
		s16_data = (u8_data2<<8) | u8_data1;
		memset(buf, 0, sizeof(htc_string_self_cap));
		scnprintf(buf, sizeof(htc_string_self_cap), "    %4d", s16_data);
		ret = htc_string_self_cap;
#ifdef DEBUG_HTC_MFG_SELF_CAP
		pr_info("%s(Rx): index %d new_line_pos %d u8_self_cap_Rx[%d] & [%d]\n",
			__func__, index, new_line_pos, self_cap_index*2, self_cap_index*2 + 1);
#endif
	}
		...
		sum = 0;
		memset(buf, 0, sizeof(htc_string_self_cap));
		for(self_cap_index = 0; self_cap_index < new_line_pos ; self_cap_index += 2) {
			u8_data1 = dad->mfg_data.u8_self_cap_Tx[self_cap_index];
			u8_data2 = dad->mfg_data.u8_self_cap_Tx[self_cap_index + 1];
			s16_data = (u8_data2<<8) | u8_data1;
			sum += scnprintf(buf + sum, sizeof(htc_string_self_cap) - sum, "%4d ", s16_data);
#ifdef DEBUG_HTC_MFG_SELF_CAP
			pr_info("%s(Tx): in_idx %d new_line_pos %d u8_self_cap_Tx[%d] %#x & [%d] %#x sum %d\n",
				__func__, in_idx, new_line_pos,
				self_cap_index, u8_data1,
				self_cap_index + 1, u8_data2,
				sum);
#endif
		}
		ret = htc_string_self_cap;
...
static void htc_transfer_from_tx_1st_to_rx_1st(struct device *dev,
	u8 *in_data, int data_sz, enum htc_mfg_item item_id)
{
	int x, y;
	int tx_num, rx_num;
	...
	tx_num = dad->si->sensing_conf_data.tx_num;
	rx_num = dad->si->sensing_conf_data.rx_num;

	if( item_id == eHTC_MFG_MUTUAL_CAP_DIFF_DATA_ID_2 &&
		dad && dad->mfg_data.u8_mutual_cap_data && data_sz == (tx_num * rx_num * 2) ) {
		memset(dad->mfg_data.u8_mutual_cap_data, 0, sizeof(dad->mfg_data.u8_mutual_cap_data));

		if( tx_num == 17 && rx_num == 30) {
			for(x = 0; x < tx_num; x++) {
				for(y = 0; y < rx_num ; y++ ) {
					dad->mfg_data.u8_mutual_cap_Tx30_Rx17x2[y][x*2] = *in_data;
					in_data++;
					dad->mfg_data.u8_mutual_cap_Tx30_Rx17x2[y][x*2 + 1] = *in_data;
					in_data++;
				}
			}
		}
		else if( tx_num == 16 && rx_num == 28) {
			for(x = 0; x < tx_num; x++) {
				for(y = 0; y < rx_num ; y++ ) {
					dad->mfg_data.u8_mutual_cap_Tx28_Rx16x2[y][x*2] = *in_data;
					in_data++;
					dad->mfg_data.u8_mutual_cap_Tx28_Rx16x2[y][x*2 + 1] = *in_data;
					in_data++;
				}
			}
		}
		else {
			pr_err("%s: please check dad, dad->mfg_data and data_sz %d, tx_num %d rx_num %d !!!",
					__func__, data_sz, tx_num, rx_num);
		}

	}
	else if( item_id == eHTC_MFG_CM_PANEL &&
			 dad && dad->mfg_data.u8_mutual_cap_data && data_sz == ((tx_num * rx_num * 2) + 2) ) {
		memset(dad->mfg_data.u8_mutual_cap_data, 0, sizeof(dad->mfg_data.u8_mutual_cap_data));
...
static int htc_panel_scan_print(struct device *dev, int status, u8 *in_buf, int length,
		u8 *out_buf, size_t out_buf_size, int hdr_sz, int new_line_pos, enum htc_mfg_item item_id)
{
	int i;
	s16 s16_val;
	int index;
	...
	index = 0;
	dad = cyttsp5_get_device_access_data(dev);
	in_data = dad->mfg_data.u8_mutual_cap_data; 
	data_sz = length - hdr_sz;

	tx_num = dad->si->sensing_conf_data.tx_num;
	rx_num = dad->si->sensing_conf_data.rx_num;


	if( item_id == eHTC_MFG_CM_PANEL &&
		data_sz == ((tx_num * rx_num * 2) + 2) ) {
		
		data_sz = (tx_num * rx_num * 2);
	}
	...
	for (i = 0; i < hdr_sz; i++) {
		index += scnprintf(&out_buf[index], out_buf_size - index,
				"%02X%s", in_buf[i], htc_get_char_end_string(dev, i, length, hdr_sz, new_line_pos));
	}
	...
	for (i = 0; i < data_sz; i += 2) {
		s16_val = (in_data[i + 1] << 8) | in_data[i];
		index += scnprintf(&out_buf[index], out_buf_size - index,
				"%4d%s%s", s16_val,
				htc_get_self_cap_string(dev, i + 1, data_sz, new_line_pos, item_id),
				htc_get_char_end_string(dev, i + 1, data_sz, 0, new_line_pos));
	}
	...
	index += scnprintf(&out_buf[index], out_buf_size - index,
			"%s%s",
			htc_get_self_cap_string(dev, data_sz, data_sz, new_line_pos, item_id),
			"\n");
...
static int htc_prepare_print_buffer(struct device *dev, int status, u8 *in_buf, int length,
		u8 *out_buf, size_t out_buf_size, int hdr_sz, enum htc_mfg_item item_id)
{
...
#if 0
	pr_info("%s: length %d out_buf_size %zu hdr_sz %d nl_pos %d",\
		__func__, length, out_buf_size, hdr_sz, new_line_pos);
#endif

	
	if( item_id == eHTC_MFG_MUTUAL_CAP_DIFF_DATA_ID_2 &&
		data_sz == (tx_num * rx_num * 2) ) {
		new_line_pos = (tx_num * 2);
		htc_transfer_from_tx_1st_to_rx_1st(dev, in_data, data_sz, item_id);
		return htc_panel_scan_print(dev, status, in_buf, length,
			out_buf, out_buf_size, hdr_sz, new_line_pos, item_id);
	}

	
	if( item_id == eHTC_MFG_SELF_CAP_DIFF_DATA_ID_5 &&
		data_sz == ((tx_num + rx_num) * 2) ) {
		new_line_pos = tx_num;
		
		for(i = 0; i < (tx_num * 2) ; i++) {
			dad->mfg_data.u8_self_cap_Tx[i] = *in_data;
			in_data++;
#ifdef DEBUG_HTC_MFG_SELF_CAP
			pr_info("%s: Tx[%d] = %#x", __func__, i, dad->mfg_data.u8_self_cap_Tx[i]);
#endif
		}
		
		for(i = 0; i < (rx_num * 2) ; i++) {
			dad->mfg_data.u8_self_cap_Rx[i] = *in_data;
			in_data++;
#ifdef DEBUG_HTC_MFG_SELF_CAP
			pr_info("%s: Rx[%d] = %#x", __func__, i, dad->mfg_data.u8_self_cap_Rx[i]);
#endif
		}
	}
	...
	if( item_id == eHTC_MFG_CP_PANEL &&
		data_sz == ((tx_num + rx_num) * 2 * 2) ) {
		new_line_pos = tx_num;
		
		for(i = 0; i < (tx_num * 2 * 2) ; i++) {
			dad->mfg_data.u8_self_cap_Tx[i] = *in_data;
			in_data++;
#ifdef DEBUG_HTC_MFG_SELF_CAP
			pr_info("%s: Tx[%d] = %#x", __func__, i, dad->mfg_data.u8_self_cap_Tx[i]);
	...
	struct cmcp_data *cmcp_info)
{
	...
	dev_dbg(dev, "%s tx_num=%d", __func__, tx_num); 
	dev_dbg(dev, "%s rx_num=%d", __func__, rx_num); 
		...
		size_t count, loff_t *ppos)
{
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on execute panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error on retrieve panel scan r=%d\n",
				__func__, rc);
		goto resume_scan;
	}
...
put_pm_runtime:
	...
	if(dad->panel_scan_data_id == 0x02) {
		data->pr_buf_len = htc_prepare_print_buffer(dev, status, &dad->ic_buf[5],
				length, data->pr_buf, sizeof(data->pr_buf), 5, eHTC_MFG_MUTUAL_CAP_DIFF_DATA_ID_2);
	}
	else if(dad->panel_scan_data_id == 0x05) {
		data->pr_buf_len = htc_prepare_print_buffer(dev, status, &dad->ic_buf[5],
				length, data->pr_buf, sizeof(data->pr_buf), 5, eHTC_MFG_SELF_CAP_DIFF_DATA_ID_5);
	}
	else {
		data->pr_buf_len = prepare_print_buffer(status, &dad->ic_buf[5],
				length, data->pr_buf, sizeof(data->pr_buf));
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (max_read >= CY_MAX_PRBUF_SIZE) {
		...
		htc_dev_err(dev, "%s: Will get %d element\n", __func__,
				dad->heatmap.num_element);
	}
...
static int cyttsp5_setup_sysfs(struct device *dev)
{
	...
	dad->android_touch_dentry = debugfs_create_symlink("android_touch", NULL, dev_name(dev));
	if (IS_ERR_OR_NULL(dad->android_touch_dentry)) {
		htc_dev_err(dev, "%s: Error, could not create android_touch symlink\n",
				__func__);
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("panel_scan", 0600,
		...
		htc_dev_err(dev, "%s: Error, could not create panel_scan\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("get_idac", 0600,
		...
		htc_dev_err(dev, "%s: Error, could not create get_idac\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("auto_shorts", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create auto_shorts\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("opens", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create opens\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("calibrate", 0600,
		...
		htc_dev_err(dev, "%s: Error, could not create calibrate\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("baseline", 0600,
		...
		htc_dev_err(dev, "%s: Error, could not create baseline\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cm_panel", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create cm_panel\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cp_panel", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create cp_panel\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cm_button", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create cm_button\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(debugfs_create_file("cp_button", 0400,
		...
		htc_dev_err(dev, "%s: Error, could not create cp_button\n",
				__func__);
		goto unregister_base_dir;
	}
	...
	if (IS_ERR_OR_NULL(dad->cmcp_results_debugfs)) {
		htc_dev_err(dev, "%s: Error, could not create cmcp_results\n",
				__func__);
	...
	if (IS_ERR_OR_NULL(dad->tthe_get_panel_data_debugfs)) {
		htc_dev_err(dev, "%s: Error, could not create get_panel_data\n",
				__func__);
...
static void cyttsp5_device_access_release(struct device *dev, void *data)
{
	...
	if (dad->sysfs_nodes_created) {
		...
		debugfs_remove_recursive(dad->base_dentry);
		debugfs_remove(dad->android_touch_dentry);
#ifdef TTHE_TUNER_SUPPORT
