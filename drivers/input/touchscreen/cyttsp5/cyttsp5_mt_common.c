...
static const char htc_cyttsp5_event_id_string[CY_EV_LIFTOFF+1][16] = {
	"NA",	
	"DOWN",	
	"MOVE",	
	"UP",	
		...
		struct cyttsp5_touch *tch, int num_cur_tch)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		if (t < md->t_min || t > md->t_max) {
			htc_dev_err(dev, "%s: tch=%d -> bad trk_id=%d max_id=%d\n",
...
cyttsp5_get_mt_touches_pr_tch:
		...
		if( tch->abs[CY_TCH_E] == CY_EV_TOUCHDOWN ||
			...
			htc_tamper_fs_print(dev, "Finger #%d(%d) %s => X:%d, Y:%d, Z:%d, N:%d",
				i,
				tch->hdr[CY_TCH_NUM],
				htc_cyttsp5_event_id_string[tch->abs[CY_TCH_E]],
				tch->abs[CY_TCH_X],
				tch->abs[CY_TCH_Y],
				tch->abs[CY_TCH_P],
				tch->hdr[CY_TCH_NOISE]
			);
		}
#endif
	}
...
static int cyttsp5_xy_worker(struct cyttsp5_mt_data *md)
{
	...
	if (num_cur_tch > max_tch) {
		htc_dev_err(dev, "%s: Num touch err detected (n=%d)\n",
...
static int cyttsp5_mt_attention(struct device *dev)
{
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: xy_worker error r=%d\n", __func__, rc);
...
static int cyttsp5_mt_resume_attention(struct device *dev)
{
	...
	mutex_lock(&md->mt_lock);
	if(md->si) {
		md->num_prv_rec = md->si->sensing_conf_data.max_tch;
		cyttsp5_mt_lift_all(md);
	}
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	if (rc < 0)
		htc_dev_err(dev, "%s: Error, failed register input device r=%d\n",
			__func__, rc);
	else
...
int cyttsp5_mt_probe(struct device *dev)
{
	...
	if (!md->input) {
		htc_dev_err(dev, "%s: Error, failed to allocate input device\n",
			__func__);
		rc = -ENODEV;
