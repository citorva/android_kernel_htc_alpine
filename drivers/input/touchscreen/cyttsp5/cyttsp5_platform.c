		...
		struct device *dev)
{
	...
	return 0;
#else
	int rst_gpio_ioexp = pdata->rst_gpio_ioexp; 
	...
	if(rst_gpio > 0) {
		...
		if(rst_gpio_ioexp > 0) {
#if 1
			gpio_set_value(rst_gpio_ioexp, 1);
			msleep(40);
			gpio_set_value(rst_gpio_ioexp, 0);
			msleep(80);
			gpio_set_value(rst_gpio_ioexp, 1);
			msleep(40);
#else
		if( pdata->pinctrl != NULL &&
			pdata->pinsctrl_rst_gpio_ioexp_active != NULL &&
			pdata->pinsctrl_rst_gpio_ioexp_inactive != NULL ) {
			pinctrl_select_state(pdata->pinctrl, pdata->pinsctrl_rst_gpio_ioexp_inactive);
			msleep(200);
			pinctrl_select_state(pdata->pinctrl, pdata->pinsctrl_rst_gpio_ioexp_active);
			msleep(20);
			pinctrl_select_state(pdata->pinctrl, pdata->pinsctrl_rst_gpio_ioexp_inactive);
			msleep(400);
		}
...
#endif
			dev_info(dev,
				"%s: RESET CYTTSP gpio_ioexp %d rc %d\n", __func__,
				pdata->rst_gpio_ioexp, rc);
		}
	}
		...
		int on, struct device *dev)
{
	...
	if (on) {
		...
		if(rst_gpio_ioexp > 0) {
			if(pdata->pinctrl != NULL && pdata->pinsctrl_rst_gpio_ioexp_inactive != NULL) {
				pinctrl_select_state(pdata->pinctrl, pdata->pinsctrl_rst_gpio_ioexp_inactive);
	...
	dev_info(dev, "%s: INIT CYTTSP RST gpio=%d gpio_exp=%d and IRQ gpio=%d rc=%d TP_1V8_EN_gpio=%d TP_I2C_SEL_CPU_gpio=%d\n",
		__func__, rst_gpio, rst_gpio_ioexp, irq_gpio, rc, TP_1V8_EN_gpio, TP_I2C_SEL_CPU_gpio);
