		...
		struct cyttsp5_hid_cmd *hid_cmd)
{
	...
	cmd_length = 2 
		+ 2    
		+ (hid_cmd->report_id >= 0XF ? 1 : 0)   
		+ (hid_cmd->has_data_register ? 2 : 0)	
...
error:
	...
	*cmd_state = 0;
	...
	dev_info(cd->dev, "%s: rc %d, opcode %d, timeout_ms %d",
		__func__, rc, hid_cmd->opcode, timeout_ms); 
exit:
	return rc;
}
...
retry_set_power:
	...
	pr_info("%s: power_state %d ++", __func__, power_state); 
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	if (cd->response_buf[HID_OUTPUT_RESPONSE_REPORT_OFFSET]
		...
		htc_dev_err(cd->dev, "%s: HID output response, wrong report_id 0x%x\n",
...
error:
	...
	cd->hid_cmd_state = 0;
	mutex_unlock(&cd->system_lock);
	dev_info(cd->dev, "%s: command_code = %d, timeout_ms = %d",
		__func__, hid_output->command_code, timeout_ms); 
exit:
	return rc;
}
...
struct cyttsp5_sysinfo *_cyttsp5_request_sysinfo(struct device *dev)
{
	...
	pr_err("%s: sysinfo is not ready", __func__); 
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = sprintf(buf,
		...
		"Reset GPIO IOEXP", pdata->core_pdata->rst_gpio_ioexp, 
		"TP_1V8_EN_gpio", pdata->core_pdata->TP_1V8_EN_gpio, 
		"TP_I2C_SEL_CPU_gpio", pdata->core_pdata->TP_I2C_SEL_CPU_gpio, 
...
static int cyttsp5_setup_irq_gpio(struct cyttsp5_core_data *cd)
{
	...
	dev_info(dev, "%s: irq_gpio = 0x%x", __func__, cd->cpdata->irq_gpio); 
