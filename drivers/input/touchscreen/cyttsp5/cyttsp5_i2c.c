...
#ifdef USE_I2C_DMA
#define MELFAS_I2C_MASTER_CLOCK 400 
	...
	const struct i2c_device_id *i2c_id)
{
	...
	if(dev) {
		...
		if(platform_data) {
			...
			if(core_pdata) {
				if(core_pdata->rst_gpio_ioexp > 0) {
					...
					core_pdata->pinsctrl_rst_gpio_ioexp_active = pinctrl_lookup_state(core_pdata->pinctrl, "rst_gpio_ioexp_active");
					if (IS_ERR(core_pdata->pinsctrl_rst_gpio_ioexp_active)) {
						core_pdata->pinsctrl_rst_gpio_ioexp_active = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_rst_gpio_ioexp_active", __func__);
					}

					core_pdata->pinsctrl_rst_gpio_ioexp_inactive = pinctrl_lookup_state(core_pdata->pinctrl, "rst_gpio_ioexp_inactive");
					if (IS_ERR(core_pdata->pinsctrl_rst_gpio_ioexp_inactive)) {
						core_pdata->pinsctrl_rst_gpio_ioexp_inactive = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_rst_gpio_ioexp_inactive", __func__);
					}

					core_pdata->pinsctrl_TP_1V8_EN_HIGH = pinctrl_lookup_state(core_pdata->pinctrl, "TP_1V8_EN_HIGH");
					if (IS_ERR(core_pdata->pinsctrl_TP_1V8_EN_HIGH)) {
						core_pdata->pinsctrl_TP_1V8_EN_HIGH = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_1V8_EN_HIGH", __func__);
					}

					core_pdata->pinsctrl_TP_1V8_EN_LOW = pinctrl_lookup_state(core_pdata->pinctrl, "TP_1V8_EN_LOW");
					if (IS_ERR(core_pdata->pinsctrl_TP_1V8_EN_LOW)) {
						core_pdata->pinsctrl_TP_1V8_EN_LOW = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_1V8_EN_LOW", __func__);
					}

					core_pdata->pinsctrl_TP_I2C_SEL_CPU = pinctrl_lookup_state(core_pdata->pinctrl, "TP_I2C_SEL_CPU");
					if (IS_ERR(core_pdata->pinsctrl_TP_I2C_SEL_CPU)) {
						core_pdata->pinsctrl_TP_I2C_SEL_CPU = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_I2C_SEL_CPU", __func__);
					}

					core_pdata->pinsctrl_TP_I2C_SEL_SHUB = pinctrl_lookup_state(core_pdata->pinctrl, "TP_I2C_SEL_SHUB");
					if (IS_ERR(core_pdata->pinsctrl_TP_I2C_SEL_SHUB)) {
						core_pdata->pinsctrl_TP_I2C_SEL_SHUB = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_I2C_SEL_SHUB", __func__);
					}

					core_pdata->pinsctrl_TP_I2C_SDA = pinctrl_lookup_state(core_pdata->pinctrl, "TP_I2C_SDA");
					if (IS_ERR(core_pdata->pinsctrl_TP_I2C_SDA)) {
						core_pdata->pinsctrl_TP_I2C_SDA = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_I2C_SDA", __func__);
					}
					else {
						pinctrl_select_state(core_pdata->pinctrl, core_pdata->pinsctrl_TP_I2C_SDA);
					}

					core_pdata->pinsctrl_TP_I2C_SCL = pinctrl_lookup_state(core_pdata->pinctrl, "TP_I2C_SCL");
					if (IS_ERR(core_pdata->pinsctrl_TP_I2C_SCL)) {
						core_pdata->pinsctrl_TP_I2C_SCL = (struct pinctrl_state *)NULL;
						rc = -EIO;
						pr_err("%s: no pinsctrl_TP_I2C_SCL", __func__);
					}
					else {
						pinctrl_select_state(core_pdata->pinctrl, core_pdata->pinsctrl_TP_I2C_SCL);
					}
				...
				core_pdata->htc_lcm_id = htc_get_lcm_id();
	...
	rc = regulator_set_voltage(tpd->reg, 2800000, 2800000);	
	if (rc) {
