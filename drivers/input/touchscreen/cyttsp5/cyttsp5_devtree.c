		...
		struct device_node *core_node)
{
	...
	pdata->rst_gpio = -1;
	pdata->rst_gpio_ioexp = -1; 
	rc = of_property_read_u32(core_node, "cy,rst_gpio", &value);
	if (!rc) {
		pdata->rst_gpio = value;
	}
	else {
		pr_debug("%s: no rst_gpio, try to use rst_gpio_ioexp", __func__);
		pdata->rst_gpio_ioexp = of_get_named_gpio(core_node, "cy,rst_gpio_ioexp", 0);
		if (pdata->rst_gpio_ioexp < 0) {
			pdata->rst_gpio_ioexp = -1;
			rc = -EIO;
			pr_err("%s: no rst_gpio_ioexp", __func__);
			goto fail;
		}
	}
	...
	printk("%s: irq_gpio:%d rst_gpio:%d rst_gpio_ioexp:%d \n" 
		...
		pdata->irq_gpio, pdata->rst_gpio, pdata->rst_gpio_ioexp, 
		pdata->TP_1V8_EN_gpio, pdata->TP_I2C_SEL_CPU_gpio, 
