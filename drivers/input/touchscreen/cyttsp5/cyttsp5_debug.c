		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (value != 0 && value != 1) {
		htc_dev_err(dev, "%s: Invalid value %lu\n", __func__, value);
		return size;
	}
...
static int cyttsp5_debug_probe(struct device *dev, void **data)
{
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create int_count\n",
				__func__);
	...
	if (rc) {
		htc_dev_err(dev, "%s: Error, could not create formated_output\n",
				__func__);
	...
	*data = dd;
	...
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, could not subscribe attention cb\n",
				__func__);
...
static void cyttsp5_debug_release(struct device *dev, void *data)
{
	...
	rc = cmd->unsubscribe_attention(dev, CY_ATTEN_IRQ, CYTTSP5_DEBUG_NAME,
		cyttsp5_debug_attention, CY_MODE_OPERATIONAL);
	if (rc < 0) {
		htc_dev_err(dev, "%s: Error, could not un-subscribe attention\n",
				__func__);
