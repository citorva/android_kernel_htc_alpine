...
#define CY_DRIVER_DATE "20150715"	
...
enum htc_touch_i2c_select{
	eTP_I2C_SEL_CPU = 0,
	eTP_I2C_SEL_SHUB = 1,
};

enum htc_diag_command{
	eHTC_DIAG_PANEL_SCAN,
	eHTC_DIAG_CM_CP_PANEL,
};

enum htc_touch_power_state{
	eTP_POWER_ON_DONE = 0,
	eTP_POWER_OFF_DONE = 1<<0,
	eTP_POWER_ON_DOING = 1<<1,
	eTP_POWER_OFF_DOING = 1<<2,
};
...
struct cyttsp5_core_platform_data {
	...
	int rst_gpio_ioexp;
	int TP_1V8_EN_gpio;
	int TP_I2C_SEL_CPU_gpio; 
	struct pinctrl *pinctrl;
	struct pinctrl_state *pinsctrl_rst_gpio_ioexp_active;
	struct pinctrl_state *pinsctrl_rst_gpio_ioexp_inactive;
	struct pinctrl_state *pinsctrl_TP_1V8_EN_HIGH;
	struct pinctrl_state *pinsctrl_TP_1V8_EN_LOW;
	struct pinctrl_state *pinsctrl_TP_I2C_SEL_CPU;
	struct pinctrl_state *pinsctrl_TP_I2C_SEL_SHUB;
	struct pinctrl_state *pinsctrl_TP_I2C_SDA;
	struct pinctrl_state *pinsctrl_TP_I2C_SCL;
	struct kobject *android_touch_kobj;
	int htc_tamper_sf;
	enum htc_diag_command diag_cmd;
	int htc_lcm_id;
	enum htc_touch_i2c_select tp_i2c_select;
	int force_fw_upgrade_from_core;
	int fw_corrupt_detected;
	atomic_t loader_fw_upgrading;
	atomic_t tp_power_state; 
};
