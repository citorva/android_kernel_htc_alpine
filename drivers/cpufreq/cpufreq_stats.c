...
static void __cpufreq_stats_free_table(struct cpufreq_policy *policy)
{
	...
	#ifdef NO_RESET
	cluster_id = (policy->cpu < 4) ? 0 : (policy->cpu < 8) ? 1 : 2;
		...
		struct cpufreq_frequency_table *table, int count)
{
	...
	#ifdef NO_RESET
	cluster_id = (policy->cpu < 4) ? 0 : + (policy->cpu < 8) ? 1 : 2;
	stat = (cpufreq_stats_table_bk[cluster_id] != NULL) ? cpufreq_stats_table_bk[cluster_id] : kzalloc(sizeof(*stat), GFP_KERNEL);
	#else
	...
	#ifdef NO_RESET
	stat->time_in_state = (time_in_state_bk[cluster_id] != NULL) ? time_in_state_bk[cluster_id] : kzalloc(alloc_size, GFP_KERNEL);
	#else
