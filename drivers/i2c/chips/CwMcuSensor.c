...
struct cwmcu_data {
	...
	u8 rv_calibration;
	u8 prev_rv_accuracy;
	struct delayed_work i2c_resume;
};
...
static int mcu_pinctrl_init_gpio(struct cwmcu_data *mcu_data)
{
	...
	gpio_request(8,"sensorhub_INT_8");
	gpio_request(3,"sensorhub_cpu2mcu_3");
	gpio_request(4,"sensorhub_boot0_4");
	gpio_request(5,"sensorhub_STATUS_5");
	gpio_request(6,"sensorhub_HUB_RST_6");
...
#else   
 if ((MCU_IN_BOOTLOADER()) || (mt_get_gpio_in(mcu_data->gpio_reset) == 0)) {
	return MCU2CPU_STATUS_GPIO_LEVEL_SHUB;
}
			  ...
			  u8 reg_addr, const void *data, u8 len, bool mutex_lock_i2c)
{
	...
	if (mcu_data->is_block_i2c) {
		...
		I("%s: gpio_reset = %d\n", __func__,
		mt_get_gpio_in(mcu_data->gpio_reset));
	#endif 
#else
			gpio_direction_input(mcu_data->gpio_reset);
                        I("%s: gpio_reset = %d\n", __func__,
...
static ssize_t read_light_kadc(struct cwmcu_data *mcu_data, char *buf)
{
	u8 data[4] = {0};
	u16 light_gadc;
	u16 light_kadc;
...
static ssize_t read_light_polling(struct cwmcu_data *mcu_data, char *buf)
{
	u8 data[REPORT_EVENT_COMMON_LEN] = {0};
	u8 data_polling_enable;
	u8 light_enable;
...
static void do_i2c_resume(struct work_struct *work)
{
	char read_buf[60];
    struct cwmcu_data *mcu_data = container_of((struct delayed_work *)work, struct cwmcu_data, i2c_resume);
	read_light_kadc(mcu_data,read_buf);
	I("semico %s reset kadc read_buf:%s\n", __func__, read_buf);
	read_light_polling(mcu_data,read_buf);
	I("semico %s reset poll read_buf:%s\n", __func__, read_buf);
}
...
static void cwmcu_irq_work(struct work_struct *work)
{
	...
	if (INT_st1 & CW_MCU_INT_BIT_LIGHT) {
		u8 data[REPORT_EVENT_COMMON_LEN] = {0};
		s16 data_buff[REPORT_EVENT_COMMON_LEN] = {0};
		static u8 last_data[REPORT_EVENT_COMMON_LEN] = {0};
		u8 i2c_err = 0;

		if (mcu_data->enabled_list & (1<<light)) {
			CWMCU_i2c_read_power(mcu_data, CWSTM32_READ_Light, data, sizeof(data));
			if (I2C_ERR_MAGIC == data[0])
			{
				i2c_err = I2C_ERR_MAGIC;
				memcpy(data, last_data, sizeof(u8)*REPORT_EVENT_COMMON_LEN);
				I("semico %s reset data[0]:%x, data[1]:%x, data[2]:%x\n", __func__, data[0], data[1], data[2]);
			...
			if (data[0] < 10 || data[0] == 0xFF) {
				...
				cw_send_event(mcu_data, CW_LIGHT, data_buff, 0);
				memcpy(last_data, data, sizeof(u8)*REPORT_EVENT_COMMON_LEN);
				I(
		...
		if (I2C_ERR_MAGIC == i2c_err)
		{
			i2c_err = 0;
			schedule_delayed_work(&mcu_data->i2c_resume, msecs_to_jiffies(500));
		}
	}
...
prop = of_find_property(dt, "mcu,Cpu_wake_mcu-gpio", NULL);
 ...
 prop = of_find_property(dt, "mcu,Chip_mode-gpio", NULL);
		 if (prop) {
			 ...
			 E("%s:mcu,Chip_mode-gpio not found", __func__);
#else   
				       ...
				       const struct i2c_device_id *id)
{
  ...
  gpio_direction_output(mcu_data->gpio_chip_mode, 0);
	...
	INIT_DELAYED_WORK(&mcu_data->i2c_resume, do_i2c_resume);
...
static int CWMCU_i2c_remove(struct i2c_client *client)
{
  ...
  gpio_set_value(mcu_data->gpio_wake_mcu, 1);
	...
	cancel_delayed_work_sync(&mcu_data->i2c_resume);
