...
static int lsm6ds3_irq_registration(struct lsm6ds3_data *cdata, irq_handler_t handler,
		     unsigned long flags)
{
	...
	node = of_find_compatible_node(NULL, NULL, "mediatek, GSENSOR-eint");
	if(node){

		D("[GSN][lsm6ds3] %s:irq node is ok!", __func__);
		of_property_read_u32_array(node , "debounce", ints, ARRAY_SIZE(ints));
		gpio_set_debounce(ints[0], ints[1]);

		D("ints[0] = %d, ints[1] = %d!!\n", ints[0], ints[1]);

		cdata->irq = irq_of_parse_and_map(node, 0);
		if (!cdata->irq) {
			...
			pr_err("[GSN][lsm6ds3 error] %s: request_irq can not find eint device node!.\n", __func__);
			ret = -1;
	}
	ret = cdata->irq;
	D("[GSN][lsm6ds3] %s: irq:%d, debounce:%d-%d: \n", __func__, cdata->irq, ints[0], ints[1]);
	return ret;
...
static void LSM6DS3_dumpReg(struct lsm6ds3_sensor_data *sdata)
{
	int i=0;
	u8 addr = 0x0C;
	u8 regdata=0;
	...
	for(i=0; i<35 ; i++)
	{
		...
		sdata->cdata->tf->read(sdata->cdata, addr, 1, &regdata, false);
		D("Reg addr=%x regdata=%x\n",addr,regdata);
		addr++;
	}
...
#ifdef IS_MTK_PLATFORM
err = lsm6ds3_irq_registration(cdata, lsm6ds3_save_timestamp,
			IRQF_TRIGGER_HIGH);
#endif
	...
	if (!sdata->cdata->sensors[LSM6DS3_ACCEL].enabled) {
		if (enable) {
			err = lsm6ds3_write_data_with_mask(sdata->cdata,
				...
				lsm6ds3_odr_table.odr_avl[1].value, true);
			if (err < 0)
				return err;
		} else {
			err = lsm6ds3_write_data_with_mask(sdata->cdata,
				...
				size_t count)
{
	...
	struct lsm6ds3_sensor_data *sdata = dev_get_drvdata(dev);
    unsigned int set_polling_rate = 0;
	...
	if (!(err < 0)) {
		set_polling_rate =  polling_rate - (polling_rate/10);
		sdata->poll_interval = set_polling_rate;
		sdata->ktime = ktime_set(0, MS_TO_NS(set_polling_rate));
	}
	D("[GSN][lsm6ds3] set_polling_rate %d, polling_rate %d\n", set_polling_rate,polling_rate);
