...
#define D(x...) printk(KERN_DEBUG "[COMP][AKM09916] " x)
#define I(x...) printk(KERN_INFO "[COMP][AKM09916] " x)
#define E(x...) printk(KERN_ERR "[COMP][AKM09916] " x)
	...
	int32_t *rbuf)
{
	...
	if (ready & MAG_DATA_READY) {
		input_report_abs(akm->input, AKM_EVABS_MAG_X, 0);
	...
	if (ready & FUSION_DATA_READY) {
		...
		input_report_abs(akm->input, AKM_EVABS_ORI_Y, 0);
		...
		input_report_abs(akm->input, AKM_EVABS_GEORV_X, 1);
	...
	if (ready & MAGUC_DATA_READY) {
		...
		input_report_abs(akm->input, AKM_EVABS_MUC_X, 0);
...
#define __BIN_ATTR_AKM(name_, mode_, size_, private_, read_, write_) \
	{ \
...
#define __BIN_ATTR_NULL_AKM \
	{ \
		.attr   = { .name = NULL }, \
	}

static struct bin_attribute akm_compass_bin_attributes[] = {
	__BIN_ATTR_AKM(accel, 0220, 6, NULL,
	...
	struct i2c_client *client)
{
	...
	akm->sense_conf[0] = akm->sense_conf[1] = akm->sense_conf[2] = 128;
...
static int akm09916_compass_parse_dt(struct device *dev, struct akm09916_platform_data *pdata)
{
	...
	prop = of_find_property(dt, "compass_akm09916,layout", NULL);
	if (prop) {
		of_property_read_u32(dt, "compass_akm09916,layout", &buf);
		pdata->layout = buf;
		I("%s: layout = %d\n", __func__, pdata->layout);
	} else
		I("%s: compass_akm09916,layout not found", __func__);
...
int akm_compass_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		...
		err = akm09916_compass_parse_dt(&client->dev, pdata);
		if (err) {
			dev_err(&client->dev,
...
#else
#define akm09916_match_table NULL
#endif
