...
#define MODIFY_FOR_PINCRL_GPIO
#define MODIFY_FOR_PINCRL_LINUX_GPIO


#ifdef IS_MTK_PLATFORM
#ifndef MODIFY_FOR_PINCRL_LINUX_GPIO
...
#if COMPATIBLE_LS_TABLE
extern int of_machine_hwid(void);
#endif



#define ST_SENSORHUB_PINCTRL
#ifdef ST_SENSORHUB_PINCTRL
typedef struct sensorhub_gpio_config {
	struct pinctrl *pinctrl;
	struct pinctrl_state *pins_default;
	struct pinctrl_state *pins_cpuTomcu_high;
	struct pinctrl_state *pins_cpuTomcu_low;
	struct pinctrl_state *pins_boot0_high;
	struct pinctrl_state *pins_boot0_low;

	struct pinctrl_state *pins_mcu_status_high;
	struct pinctrl_state *pins_mcu_status_low;
	struct pinctrl_state *pins_reset_high;
	struct pinctrl_state *pins_reset_low;
	struct pinctrl_state *pins_interrupt_high;
	struct pinctrl_state *pins_interrupt_low;
	struct pinctrl_state *pins_rst_input_high;
	struct pinctrl_state *pins_rst_input_low;
	struct pinctrl_state *pins_sr_1v8_sel_high;
	struct pinctrl_state *pins_sr_1v8_sel_low;


}sensorhub_gpio_config;

sensorhub_gpio_config g_hubpinctrl_gpio;

int mcu_audio_gpio_output(u8 v_gpio_group, u8 v_gpio_ping, bool v_hilo) {
	u8 data[2] = {0};
	int error = 0;
	const char *bootmode;
	int wait_probe_cnt = 0;

	bootmode = htc_get_bootmode();
	I("%s: bootmode = %s, s_mcu_state = %d", __func__, bootmode, s_mcu_state);
	if (!strncmp(bootmode, "charger", sizeof("charger"))) {
		E("%s: can't set gpio for audio on offmode charging", __func__);
		return -EPERM;
	}

	while(s_mcu_state != MCU_STATE_SHUB_RUN &&
		wait_probe_cnt++ < 20) {
		I("%s: wait driver probe finish, mcu_state %d", __func__, s_mcu_state);
		msleep(200);
	}

	if(s_mcu_data == NULL) {
		E("%s: driver probe fail, skip to set audio gpio!!!", __func__);
		return -EPERM;
	}

	if (v_gpio_group > 6) {
		E("%s: first parameter gpio group which should be between 0~6(0:groupA; 1:groupB; 2:groupC...6:groubG )\n", __func__);
		return -EINVAL;
	}
	switch(v_gpio_ping) {
	case 1:
	...
	case 5:
		...
		break;
	default:
		E("%s: 2nd parameter is gpio ping which should be between 0~15, please check SHUB gpio table\n", __func__);
		return -EINVAL;
	}
	...
	error = CWMCU_i2c_write_power(s_mcu_data,
				CW_I2C_AUD_GPIO_SETTING,data, 2);
...
EXPORT_SYMBOL_GPL(mcu_audio_gpio_output);

static int mcu_pinctrl_init_gpio(struct cwmcu_data *mcu_data)
{
    struct i2c_client *client = mcu_data->client;
    int ret;

    I("htc mcu_pinctrl_init_gpio");

      
	g_hubpinctrl_gpio.pinctrl = devm_pinctrl_get(&client->dev);
	if (IS_ERR(g_hubpinctrl_gpio.pinctrl)) {
		g_hubpinctrl_gpio.pinctrl = (struct pinctrl *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pinctrl);
		E("Cannot find htc mcu pinctrl!\n");
	}

	g_hubpinctrl_gpio.pins_default= pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "pin_default");
	if (IS_ERR(g_hubpinctrl_gpio.pins_default)) {
		g_hubpinctrl_gpio.pins_default = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_default);
		E("Cannot find htc mcu pinctrl default!\n");
	}

	g_hubpinctrl_gpio.pins_cpuTomcu_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "pin_cfg_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_cpuTomcu_high)) {
		g_hubpinctrl_gpio.pins_cpuTomcu_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_cpuTomcu_high);
		E("Cannot find htc mcu pinctrl pins_cpuTomcu_high!\n");

	}

	g_hubpinctrl_gpio.pins_cpuTomcu_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "pin_cfg_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_cpuTomcu_low)) {
		g_hubpinctrl_gpio.pins_cpuTomcu_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_cpuTomcu_low);
		E("Cannot find htc mcu pinctrl pins_cpuTomcu_low!\n");

	}

	g_hubpinctrl_gpio.pins_boot0_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "Chip_mode-gpio_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_boot0_high)) {
		g_hubpinctrl_gpio.pins_boot0_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_boot0_high);
		E("Cannot find htc mcu pinctrl pins_boot0_high!\n");

	}

	g_hubpinctrl_gpio.pins_boot0_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "Chip_mode-gpio_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_boot0_low)) {
		g_hubpinctrl_gpio.pins_boot0_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_boot0_low);
		E("Cannot find htc mcu pinctrl pins_boot0_low!\n");
	}


	g_hubpinctrl_gpio.pins_mcu_status_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "mcu_status-gpio_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_mcu_status_high)) {
		g_hubpinctrl_gpio.pins_mcu_status_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_mcu_status_high);
		E("Cannot find htc mcu pinctrl pins_mcu_status_high!\n");

	}

	g_hubpinctrl_gpio.pins_mcu_status_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "mcu_status-gpio_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_mcu_status_low)) {
		g_hubpinctrl_gpio.pins_mcu_status_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_mcu_status_low);
		E("Cannot find htc mcu pinctrl pins_mcu_status_low!\n");

	}

	g_hubpinctrl_gpio.pins_reset_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "Reset-gpio_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_reset_high)) {
		g_hubpinctrl_gpio.pins_reset_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_reset_high);
		E("Cannot find htc mcu pinctrl pins_reset_high!\n");

	}

	g_hubpinctrl_gpio.pins_reset_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "Reset-gpio_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_reset_low)) {
		g_hubpinctrl_gpio.pins_reset_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_reset_low);
		E("Cannot find htc mcu pinctrl pins_reset_low!\n");

	}

	g_hubpinctrl_gpio.pins_interrupt_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "intr-gpio_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_interrupt_high)) {
		g_hubpinctrl_gpio.pins_interrupt_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_interrupt_high);
		E("Cannot find htc mcu pinctrl pins_interrupt_high!\n");

	}

	g_hubpinctrl_gpio.pins_interrupt_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "intr-gpio_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_interrupt_low)) {
		g_hubpinctrl_gpio.pins_interrupt_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_interrupt_low);
		E("Cannot find htc mcu pinctrl pins_interrupt_low!\n");

	}

	g_hubpinctrl_gpio.pins_rst_input_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "shub_rst_input_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_rst_input_high)) {
		g_hubpinctrl_gpio.pins_rst_input_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_rst_input_high);
		E("Cannot find htc mcu pinctrl pins_rst_input_high!\n");
	}

	g_hubpinctrl_gpio.pins_rst_input_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "shub_rst_input_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_rst_input_low)) {
		g_hubpinctrl_gpio.pins_rst_input_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_rst_input_low);
		E("Cannot find htc mcu pinctrl pins_rst_input_low!\n");
	}

	g_hubpinctrl_gpio.pins_sr_1v8_sel_high = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "sr_1v8-gpio_high");
    if (IS_ERR(g_hubpinctrl_gpio.pins_sr_1v8_sel_high)) {
		g_hubpinctrl_gpio.pins_sr_1v8_sel_high = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_sr_1v8_sel_high);
		E("Cannot find htc mcu pinctrl pins_sr_1v8_sel_high!\n");
	}

	g_hubpinctrl_gpio.pins_sr_1v8_sel_low = pinctrl_lookup_state(g_hubpinctrl_gpio.pinctrl, "sr_1v8-gpio_low");
    if (IS_ERR(g_hubpinctrl_gpio.pins_sr_1v8_sel_low)) {
		g_hubpinctrl_gpio.pins_sr_1v8_sel_low = (struct pinctrl_state *)NULL;
		ret = PTR_ERR(g_hubpinctrl_gpio.pins_sr_1v8_sel_low);
		E("Cannot find htc mcu pinctrl pins_sr_1v8_sel_low!\n");
	}

	if(g_hubpinctrl_gpio.pinctrl != NULL) {
		if(g_hubpinctrl_gpio.pins_cpuTomcu_low != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_cpuTomcu_low);
		}
		if(g_hubpinctrl_gpio.pins_boot0_low != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_boot0_low);
		}
		if(g_hubpinctrl_gpio.pins_mcu_status_low != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_mcu_status_low);
		}
		if(g_hubpinctrl_gpio.pins_reset_high != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_reset_high);
		}
		if(g_hubpinctrl_gpio.pins_interrupt_low != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_interrupt_low);
		}
		if(g_hubpinctrl_gpio.pins_sr_1v8_sel_high != NULL) {
			pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_sr_1v8_sel_high);
		}
	}

	if(mcu_data != NULL) {
		if(mcu_data->gpio_mcu_irq > 0) {
			gpio_request(mcu_data->gpio_mcu_irq, "sensorhub_INT");
		}
		if(mcu_data->gpio_wake_mcu > 0) {
			gpio_request(mcu_data->gpio_wake_mcu, "sensorhub_cpu2mcu");
		}
		if(mcu_data->gpio_chip_mode > 0) {
			gpio_request(mcu_data->gpio_chip_mode, "sensorhub_boot0");
			gpio_direction_output(mcu_data->gpio_chip_mode, 0);
		}
		if(mcu_data->gpio_mcu_status > 0) {
			gpio_request(mcu_data->gpio_mcu_status, "sensorhub_STATUS");
		}
		if(mcu_data->gpio_reset > 0) {
			gpio_request(mcu_data->gpio_reset, "sensorhub_HUB_RST");
			gpio_direction_output(mcu_data->gpio_reset, 1);
		}

		if(mcu_data->gpio_sr_1v8 > 0) {
			gpio_request(mcu_data->gpio_sr_1v8, "sensorhub_1v8");
			gpio_direction_output(mcu_data->gpio_sr_1v8, 1);
		}
	}
...
#if 0
static int hubpinctl_rst_input_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_rst_input_high);
		if(ret < 0)
		{
			E("Config pins_rst_input_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_rst_input_low);
		if(ret < 0)
		{
			E("Config pins_rst_input_low failed!\n");
		}
	}
	return ret;
}
...
static int hubpinctl_cpuTomcu_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_cpuTomcu_high);
		if(ret < 0)
		{
			E("Config pins_cpuTomcu_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_cpuTomcu_low);
		if(ret < 0)
		{
			E("Config pins_cpuTomcu_low failed!\n");
		}
	}
	return ret;
}
...
static int hubpinctl_mcu_status_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_mcu_status_high);
		if(ret < 0)
		{
			E("Config pins_mcu_status_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_mcu_status_low);
		if(ret < 0)
		{
			E("Config pins_mcu_status_low failed!\n");
		}
	}
	return ret;
}
...
static int hubpinctl_mcu_interrupt_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_interrupt_high);
		if(ret < 0)
		{
			E("Config pins_interrupt_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_interrupt_low);
		if(ret < 0)
		{
			E("Config pins_interrupt_low failed!\n");
		}
	}
	return ret;
}
...
#if 0
static int hubpinctl_boot0_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_boot0_high);
		if(ret < 0)
		{
			E("Config pins_boot0_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_boot0_low);
		if(ret < 0)
		{
			E("Config pins_boot0_low failed!\n");
		}
	}
	return ret;
}

static int hubpinctl_rst_set(int level)
{
	int ret = 0;
	if(level)
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_reset_high);
		if(ret < 0)
		{
			E("Config pins_reset_high failed!\n");
		}
	}
	else
	{
		ret = pinctrl_select_state(g_hubpinctrl_gpio.pinctrl, g_hubpinctrl_gpio.pins_reset_low);
		if(ret < 0)
		{
			E("Config pins_reset_low failed!\n");
		}
	}
	return ret;
}
...
{
#ifdef IS_MTK_PLATFORM
#ifdef	MODIFY_FOR_PINCRL_LINUX_GPIO
	if ((MCU_IN_BOOTLOADER()) || (gpio_get_value(mcu_data->gpio_reset) == 0)) {
...
#else   
	if ((MCU_IN_BOOTLOADER()) || (mt_get_gpio_out(mcu_data->gpio_reset) == 0)) {
		return MCU2CPU_STATUS_GPIO_LEVEL_SHUB;
	}

	
	if (true) {
		return mt_get_gpio_in(mcu_data->gpio_mcu_status);
	}
	else {
#endif 
#else
...
#else   
	if (!mt_get_gpio_out(gpio))
		mt_set_gpio_out(gpio, 1);
	mt_set_gpio_out(gpio, 0);
#endif 
#else
	if (!gpio_get_value(gpio))
		gpio_set_value(gpio, 1);
...
static void mcu_chip_mode_set(struct cwmcu_data *mcu_data, int onoff)
{
	...
	if(mcu_data->gpio_chip_mode > 0) {
		...
		gpio_set_value(mcu_data->gpio_chip_mode,onoff);
	}
#else   
...
static int cwmcu_set_touch_solution(struct cwmcu_data *mcu_data)
{
	if (g_touch_solution == TOUCH_SOLUTION_MAXIM1187X ||
	  g_touch_solution == TOUCH_SOLUTION_SYNAPTICS3351||g_touch_solution == TOUCH_SOLUTION_HIMAX8527X) {
    ...
    uint8_t *data, int length)
{
	...
	if (ret < 0){
		E("Dma I2C client %#x Read Error: 0x%04X, %d byte(s), err-code: %d", client->addr, addr, length, ret);
		mutex_unlock(&dma_i2c_lock);
		return ret;
	}
    ...
    const uint8_t *data, int length)
{
	...
	if (ret < 0){
		E("Dma I2C client %#x Write Error: 0x%04X, %d byte(s), err-code: %d", client->addr, addr, length, ret);
		mutex_unlock(&dma_i2c_lock);
		return ret;
	}
	mutex_unlock(&dma_i2c_lock);
...
#endif
void mcu_set_display_state(u8 on_off)
{
    int ret;

    u8 display_state[2] = {0};
    display_state[0] = (u8)on_off;
    display_state[1] = (u8)htc_get_lcm_id();
    ret = CWMCU_i2c_write_block_power(s_mcu_data, CW_I2C_REG_DISPLAY_STATE, (u8*)display_state, sizeof(display_state));
    I("%s(%d): display_state:0x%x, display panel id = %x, ret:%d\n", __func__, __LINE__, display_state[0],display_state[1] , ret);
}
EXPORT_SYMBOL(mcu_set_display_state);
...
static bool reset_hub(struct cwmcu_data *mcu_data, bool force_reset)
{
	...
	gpio_direction_output(mcu_data->gpio_reset, 0);
        ...
        I("%s: gpio_reset = %d\n", __func__,
			mt_get_gpio_out(mcu_data->gpio_reset));
	#endif 
#else
		gpio_direction_output(mcu_data->gpio_reset, 0);
                I("%s: gpio_reset = %d\n", __func__,
...
int touch_power(u8 status)
{
	...
	I("[TP] touch_power: status = %d\n", status);
	if(status != 0xff)
	{
		ret = CWMCU_i2c_write_power(s_mcu_data, TOUCH_POWER_REGISTER, &status, 1);
		if(!ret)
			E("[TP] %s: touch power write error\n", __func__);
	}
			  ...
			  const char *buf, size_t count)
{
	...
	atomic_inc(&mcu_data->critical_sect);

	str_buf = kstrndup(buf, count, GFP_KERNEL);
	if (str_buf == NULL) {
		E("%s: cannot allocate buffer\n", __func__);
		atomic_dec(&mcu_data->critical_sect);
		return -ENOMEM;
	}
	running = str_buf;
		...
		*handle_time_base = timestamp;
		...
		if (data[0] == CW_ORIENTATION) {
			...
			cw_send_event_special(mcu_data, CW_ROTATIONVECTOR,
			   ...
			   && ((data[0] == CW_MAGNETIC) || (data[0] == CW_ROTATIONVECTOR))) {
			int rc;
			u8 accuracy;
			u16 bias_event[REPORT_EVENT_COMMON_LEN] = {0};
			...
			bias_event[0] = accuracy;
			bias_event[1] = (accuracy << 8) | ((!accuracy) ? 1 : 0);
			...
			if (mcu_data->prev_rv_accuracy != accuracy) {
				I(
				  ...
				  ", bias_event[0] = 0x%x"
				  ", bias_event[1] = 0x%x, bias_event[2] = 0x%x\n"
				  , *event_count, *i, data[0]
				  , accuracy, bias_event[0], bias_event[1], bias_event[2]);
			}

			cw_send_event_special(mcu_data, data[0],
					      data_event,
					      bias_event,
					      timestamp_event +
					      *handle_time_base);
...
static int cwmcu_suspend(struct device *dev)
{
	...
	for (i = 0; (mcu_data->power_on_counter != 0) &&
		    (mt_get_gpio_out(mcu_data->gpio_wake_mcu) != 1) &&
		    (i < ACTIVE_RETRY_TIMES); i++)
		usleep_range(10, 20);
		...
		if(mcu_data->enabled_list & (1<<proximity)) {
			...
			if(data[0] < 2) {
                ...
                I("irq_hndl: ps int val: %d, adc: 0x%X, madc: 0x%X,"
                  " atk_thd: 0x%X, pre_thd: 0x%X, p_pk: %d, ps_k: %d\n",
            ...
            D("irq_hndl: ps int val: %d, adc: 0x%X, madc: 0x%X,"
              " atk_thd: 0x%X, pre_thd: 0x%X, p_pk: %d, ps_k: %d (msg only)\n",
                    data[0], ps_adc, ps_min_adc, ps_autok_thd,
                    g_ps_autok_thd, data[7], mcu_data->ps_calibrated);
		}
...
static int mcu_parse_dt(struct device *dev, struct cwmcu_data *pdata)
{
	...
	if (prop) {
		...
		I("%s:mcu,mcu_status-gpio = %d\n", __func__, pdata->gpio_mcu_status);
	} else
		E("%s:mcu,mcu_status-gpio not found", __func__);
	...
	if (prop) {
		...
		E("%s:mcu,Chip_mode-gpio not found, try to use ioexp", __func__);
		pdata->gpio_chip_mode = of_get_named_gpio(dt, "mcu,Chip_mode-gpio_ioexp", 0);
	}
	I("%s:mcu,Chip_mode-gpio = %d\n", __func__, pdata->gpio_chip_mode);

	prop = of_find_property(dt, "mcu,SR_1V8-gpio", NULL);
	if (prop) {
		of_property_read_u32(dt, "mcu,SR_1V8-gpio", &buf);
		pdata->gpio_sr_1v8 = buf;
	} else
		E("%s:mcu,SR_1V8-gpio not found", __func__);

#else   
	pdata->gpio_mcu_status = GPIO_MCU_STATUS;

	pdata->gpio_wake_mcu = GPIO_MCU_CPU_WAKE;

	pdata->gpio_mcu_irq = GPIO_MCU_INTR;

	pdata->gpio_reset = GPIO_MCU_RESET;

	pdata->gpio_chip_mode = GPIO_MCU_CHIP_MODE;

	D("%s: End, gpio_mcu_status:%lu, gpio_wake_mcu:%lu, gpio_mcu_irq:%lu, gpio_reset:%lu, gpio_chip_mode:%lu\n", __func__, pdata->gpio_mcu_status, pdata->gpio_wake_mcu, pdata->gpio_mcu_irq, pdata->gpio_reset, pdata->gpio_chip_mode);
    ...
    pdata->gpio_sr_1v8 = of_get_named_gpio(dt, "mcu,SR_1V8-gpio", 0);
    if (!gpio_is_valid(pdata->gpio_sr_1v8))
        E("DT:gpio_sr_1v8 value is not valid\n");
    else
        D("DT:gpio_sr_1v8=%d\n", pdata->gpio_sr_1v8);
#endif
...
#if COMPATIBLE_LS_TABLE
	prop = of_find_property(dt, "mcu,pcb-id", NULL);
	if (prop) {
		of_property_read_u32(dt, "mcu,pcb-id", &pcb_id_flags);
		if(1==pcb_id_flags) {
			pcb_id = of_machine_hwid();
			I("%s: of_machine_hwid = 0x%x", __func__, pcb_id);
		}
	}

	if(pcb_id >= 0x80) {
        prop = of_find_property(dt, "mcu,als_levels_2", NULL);
        if (prop) {
                of_property_read_u32_array(dt, "mcu,als_levels_2", adc_table, 10);
...
#if 0
static ssize_t als_table_flag_show(struct device *dev,
                                 struct device_attribute *attr,
                                 char *buf)
{
    return snprintf(buf, PAGE_SIZE, "%d\n", g_als_levevls_2);
...
static struct device_attribute attributes[] = {
	...
	__ATTR(nvram_cali_data, 0664, get_nvram_cali_data, set_nvram_cali_data),
#ifdef SHUB_LOGGING_SUPPORT
	...
	__ATTR(rv_cali, 0664, rv_cali_show, rv_cali_set),
...
static void cwmcu_one_shot(struct work_struct *work)
{
	...
	u8 clear_intr;
#ifdef CONFIG_AK8789_HALLSENSOR
	D("w_hall_inform_mcu is %d",mcu_data->w_hall_inform_mcu);
#endif
	D("w_activated_i2c is %d,w_re_init is %d, w_kick_start_mcu is %d, w_mcu_state_change is %d, w_flush_fifo is %d, w_clear_fifo is %d, w_batch_read is %d\n",
...
static int mcu_i2c_rx(struct i2c_client *client, u8 *buf, int len)
{
	...
	client->ext_flag = (client->ext_flag & I2C_MASK_FLAG);
	if(len > 8) {
...
static int mcu_i2c_tx_cmd(struct i2c_client *client, u8 cmd, u8 *buf, int len)
{
    int ret;
...
static int mcu_i2c_rx_cmd(struct i2c_client *client, u8 cmd, u8 *buf, int len)
{
    int ret;
    u8 i2c_data[1];
...
static void mcu_bl_enter_leave(uint8_t enter)
{
    ...
    if (enter) {
        ...
        chip_mode_target = MCU_CHIP_MODE_BOOTLOADER;
    }
    else {
        I("%s(%d) : leave\n", __func__, __LINE__);
        mcu_state_enter_unknown(s_mcu_data);
        mcu_chip_mode_set(s_mcu_data, MCU_CHIP_MODE_APPLICATION);
        chip_mode_target = MCU_CHIP_MODE_APPLICATION;
    }
#ifdef IS_MTK_PLATFORM
	#ifdef	MODIFY_FOR_PINCRL_LINUX_GPIO
		if(s_mcu_data->gpio_chip_mode > 0) {
			
			
			chip_mode_rel = gpio_get_value(s_mcu_data->gpio_chip_mode);
			if (chip_mode_target != chip_mode_rel)
				E("%s, set chip_mode failed, target =%d",__func__,chip_mode_target);
			else
				I("%s, set chip_mode success, target =%d",__func__,chip_mode_target);
		}
	#else   
		I("%s[%d]: gpio_chip_mode value = %d\n", __func__, __LINE__,
		 mt_get_gpio_out(s_mcu_data->gpio_chip_mode));
    ...
    if (MCU_IN_SHUB()) {
        ...
        while (count > 0) {
            ...
            if (atomic_read(&s_mcu_data->critical_sect)) {
                I("%s: Stop reading log data because critical_sect > 0\n", __func__);
                msleep(5);
            }
				       ...
				       const struct i2c_device_id *id)
{
	...
	I("%s++: Do not call iio_push_to_buffers when !pseudo_irq_enable\n",
	  __func__);
#if 0
    mt_set_gpio_dir(GPIO_MHL_I2C_BUS_POWER, 1);
    mt_set_gpio_mode(GPIO_MHL_I2C_BUS_POWER, 0);
    mt_set_gpio_out(GPIO_MHL_I2C_BUS_POWER, 1);
    ...
    gpDMABuf_va = (u8 *)dma_alloc_coherent(&client->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, (dma_addr_t *)&gpDMABuf_pa, GFP_KERNEL);
  ...
  gpDMABuf_va1 = (u8 *)dma_alloc_coherent(&client->dev, GTP_DMA_MAX_TRANSACTION_LENGTH, (dma_addr_t *)&gpDMABuf_pa1, GFP_KERNEL);
...
#ifdef	 MODIFY_FOR_PINCRL_LINUX_GPIO
	mcu_pinctrl_init_gpio(mcu_data);
#endif 
	...
	gpio_direction_output(mcu_data->gpio_wake_mcu, 1);
		...
		if(mcu_data->gpio_chip_mode > 0) {
			...
			gpio_set_value(mcu_data->gpio_chip_mode,0);
		}
	#else   
		mt_set_gpio_dir(mcu_data->gpio_chip_mode, 1);
		mt_set_gpio_mode(mcu_data->gpio_chip_mode, 0);
	...
	if (!strcmp(htc_get_bootmode(), "charger")) {
		I("%s: disabled when offmode_charging\n", __func__);
#if 0
		mt_set_gpio_out(GPIO_MHL_I2C_BUS_POWER, 0);
#endif
		error = -1;
		goto error_free_dev;
	}
	...
	if (error)  {
		E("%s: create_sysfs_interfaces(ATTR files) failed\n", __func__);
		goto err_free_mem;
	}
	...
	if (error < 0)
		E("[CWMCU] could not enable irq as wakeup source (for QCT platform) %d\n", error);
