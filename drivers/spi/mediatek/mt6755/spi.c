...
struct mt_spi_t *fp_ms = NULL;
void fp_enable_clk(void)
{
    if(fp_ms == NULL){
        printk("%s, [HTC][FP] fp_ms is NULL !", __func__);
        return;
    }
	enable_clk(fp_ms);
}
EXPORT_SYMBOL(fp_enable_clk);

void fp_disable_clk(void)
{
    if(fp_ms == NULL){
        printk("%s, [HTC][FP] fp_ms is NULL !", __func__);
        return;
...
static void enable_clk(struct mt_spi_t *ms)
{
	...
	pr_debug("%s, run enable_clock(MT_CG_PERI_SPI0)\n",__func__);
#else
	clk_enable(ms->clk_main);
	pr_debug("%s, run clk_enable(ms->clk_main)\n",__func__); 
#endif
#endif
...
static void disable_clk(struct mt_spi_t *ms)
{
	...
	pr_debug("%s, run disable_clk(ms->clk_main)\n",__func__); 
#endif
#endif
...
static irqreturn_t mt_spi_interrupt(int irq, void *dev_id)
{
	...
	spi_rec_time("irqs");
	printk("[fp-ree-spi][Ree spi irq handler] %s ++\n", __func__);
	...
	if (unlikely(!xfer)) {
		pr_err("[fp-ree-spi][Ree SPI IRQ service] null pointer error, reg_val=0x%x\n", reg_val);
...
static int __init mt_spi_probe(struct platform_device *pdev)
{
    ...
    if(master->bus_num == 0){
      printk("[fp-ree-spi] NOT register irq handler for spi bus 0\n");
    }
    else {
#ifdef CONFIG_OF
    ...
    if(master->bus_num == 0){
	    fp_ms = ms;
        pr_debug("%s, [HTC][FP] get the SPI0 master, master->bus_num=%d\n", __func__, master->bus_num);
    }
