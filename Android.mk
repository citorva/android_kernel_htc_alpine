...
KERNEL_ROOT_DIR := $(PWD)
KERNEL_MODULES_OUT_HTC := $(TARGET_OUT)/lib/modules
...
ifneq ($(KERNEL_CONFIG_MODULES),)
	...
	vendor/mocana/scripts/keydar_build_kernel_modules.sh -v -M -c $(KERNEL_CROSS_COMPILE) -s `pwd`/vendor/mocana/src/mss -k 3.18 -K $(KERNEL_OUT) -e `pwd`/vendor/mocana/src/ecryptfs-mocana -a `pwd`/vendor/mocana/src/crypto-api-template -D `pwd`/$(KERNEL_MODULES_OUT_HTC)

ifeq ($(MOCANA_FIPS_MODULE), true)
	vendor/mocana/scripts/keydar_build_kernel_modules.sh -v -M -c $(KERNEL_CROSS_COMPILE) -s `pwd`/vendor/mocana/src/mss -k 3.18 -K $(KERNEL_OUT) -e `pwd`/vendor/mocana/src/ecryptfs-mocana -r `pwd`/vendor/mocana/src/dm-crypt-mocana -a `pwd`/vendor/mocana/src/crypto-api-template -D `pwd`/$(KERNEL_MODULES_OUT_HTC)
endif
endif
