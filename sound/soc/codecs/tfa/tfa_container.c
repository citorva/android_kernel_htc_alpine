...
nxpTfaFileDsc_t *tfacont_getfiledata(int dev_idx, int prof_idx, enum nxpTfaHeaderType type)
{
	...
	if(!prof) {
		pr_err("tfacont_getfiledata() prof is NULL, dev_idx %d,  prof_idx %d\n", dev_idx, prof_idx);
		return NULL;
	}
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx)
{
	...
	if (!previous_prof) {
		pr_err("tfaContWriteProfile() previous_prof is NULL \n");
		return Tfa98xx_Error_Bad_Parameter;
	}
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		...
		if (((tfa98xx_dev_family(dev_idx) == 2) && (TFA_GET_BF(dev_idx, MANSTATE) == 9)) ||
...
nxpTfaLiveDataList_t *tfaContGet1stLiveDataList(nxpTfaContainer_t *cont)
{
	...
	if(!dev) {
		pr_err("tfaContGet1stLiveDataList() dev is NULL");
		return NULL;
	}
