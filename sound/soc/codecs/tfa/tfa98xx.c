...
#define TFA98XX_DMA_BUFFER_BYTE_COUNT 4096
#define I2C_FIFO_SIZE (8) 

#define TFA98XX_MAX_LOAD_FW_RETRY_COUNT 30
static int tfa98xx_load_fw_retry_count = 0;

struct pinctrl *pinctrlaud_tfa98xx; 

struct tfa98xx_gpio_attr {
	const char *name;
	bool gpio_prepare;
	struct pinctrl_state *gpioctrl;
};

static struct tfa98xx_gpio_attr aud_tfa98xx_gpios[] = {
	{
		.name = "tfa98xx",
		.gpio_prepare = false,
		.gpioctrl = NULL,
...
static int tfa98xx_mixer_profile = -1; 
				     ...
				     loff_t *ppos)
{
	...
	n += snprintf(&str[n], sizeof(str)-1-n, "tfa98xx_devices[0]\t= %p\n",  tfa98xx_devices[0]); 
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if(profile < 0) {
		pr_warn("tfa98xx_get_vstep() profile id is less than 0\n");
		return -1;
	}
			     ...
			     struct snd_ctl_elem_value *ucontrol)
{
	...
	if(profile < 0) {
		pr_warn("tfa98xx_set_vstep() profile id is less than 0\n");
		return -1;
	}
	vstep = tfa98xx_prof_vsteps[profile];
			       ...
			       struct snd_ctl_elem_value *ucontrol)
{
	...
	get_profile_from_list(profile_name, new_profile); 
				...
				int len, unsigned char value[])
{
	...
	if (tfa98xx_devices[handle] && tfa98xx_devices[handle]->i2c) {
		...
		msgs[0].timing = msgs[1].timing = tfa98xx->i2c_speed;
		msgs[0].ext_flag = msgs[1].ext_flag = 0;
		if (msgs[1].len > I2C_FIFO_SIZE) {
			if (msgs[1].len > TFA98XX_DMA_BUFFER_BYTE_COUNT)
				pr_err("%s data longer than DMA buffer! len = %d (dma buf %d)\n", __func__, msgs[1].len, TFA98XX_DMA_BUFFER_BYTE_COUNT);
			else {
				msgs[1].ext_flag = I2C_DMA_FLAG;
				msgs[1].buf = (unsigned char *)(uintptr_t)tfa98xx->dma_buf_pa;
			}
		}
#endif
		...
		if (err != ARRAY_SIZE(msgs)) {
			...
			if (msgs[1].ext_flag == I2C_DMA_FLAG)
				memcpy(value, tfa98xx->dma_buf,  msgs[1].len);
		}
#endif
		if (tfa98xx_kmsg_regs)
				...
				const unsigned char data[])
{
	...
	if (tfa98xx_devices[handle]) {
		tfa98xx = tfa98xx_devices[handle];
#ifdef CONFIG_MTK_I2C_EXTENSION
		if (len > TFA98XX_DMA_BUFFER_BYTE_COUNT) {
			pr_err("%s data longer than DMA buffer! len = %d (dma buf %d)\n", __func__, len, TFA98XX_DMA_BUFFER_BYTE_COUNT);
			return Tfa98xx_Error_Fail;
		}

		tfa98xx->i2c->ext_flag = I2C_DMA_FLAG;
		memcpy(tfa98xx->dma_buf, data, len);
retry:
		ret = i2c_master_send(tfa98xx->i2c, (unsigned char *)(uintptr_t)tfa98xx->dma_buf_pa, len);
		if (ret < 0) {
			pr_warn("i2c error, retries left: %d\n", retries);
			if (retries) {
				retries--;
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (!cont) {
		...
		tfa98xx_load_fw_retry_count++;
		if (tfa98xx_load_fw_retry_count < TFA98XX_MAX_LOAD_FW_RETRY_COUNT) {
			ret = tfa98xx_load_container(tfa98xx);
			pr_info("tfa98xx_container_loaded: Retry to load fw: %d, retry: %d !\n", ret, tfa98xx_load_fw_retry_count);
		}
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (tfa98xx_mixer_profile == -1) {
		...
		pr_err("tfa9891 bypass mode and power %s\n", (mute)? "down":"up");
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	if ((strcmp(htc_get_bootmode(), "recovery") == 0)
		...
		|| (strcmp(htc_get_bootmode(), "charger") == 0)
		|| (strcmp(htc_get_bootmode(), "download") == 0)) {
		pr_info("%s: boot mode is %s, skip load firmware", __func__, htc_get_bootmode());
	} else {
		ret = tfa98xx_load_container(tfa98xx);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	if (prop) {
		of_property_read_u32(np, "reset-gpio", &tfa98xx->reset_gpio);
		pr_info("%s: reset_gpio = %d\n", __func__, tfa98xx->reset_gpio);
	} else {
		pr_warn("%s: reset-gpio not found\n", __func__);
		tfa98xx->reset_gpio = -1;
	}

	prop = of_find_property(np, "power-gpio", NULL);
	if (prop) {
		of_property_read_u32(np, "power-gpio", &tfa98xx->power_gpio);
		pr_info("%s: power_gpio = %d\n", __func__, tfa98xx->power_gpio);
	} else {
		pr_warn("%s: power-gpio not found\n", __func__);
		tfa98xx->power_gpio = -1;
	}

	prop = of_find_property(np, "irq-gpio", NULL);
	if (prop) {
		of_property_read_u32(np, "irq-gpio", &tfa98xx->irq_gpio);
		pr_info("%s: irq_gpio = %d\n", __func__, tfa98xx->irq_gpio);
	} else {
		pr_warn("%s: irq-gpio not found\n", __func__);
		tfa98xx->irq_gpio = -1;
	}

	prop = of_find_property(np, "i2c-speed", NULL);
	if (prop) {
		of_property_read_u32(np, "i2c-speed", &tfa98xx->i2c_speed);
		pr_info("%s: i2c_speed = %d\n", __func__, tfa98xx->i2c_speed);
	} else {
		pr_warn("%s: i2c-speed not found\n", __func__);
		tfa98xx->i2c_speed = 0;
	}
#else
...

	{
		...
		pinctrlaud_tfa98xx = devm_pinctrl_get(dev);
		if (IS_ERR(pinctrlaud_tfa98xx)) {
			ret = PTR_ERR(pinctrlaud_tfa98xx);
			pr_err("Cannot find pinctrlaud_tfa98xx!\n");
			return 0;
		}

		for (i = 0; i < ARRAY_SIZE(aud_tfa98xx_gpios); i++) {
			aud_tfa98xx_gpios[i].gpioctrl = pinctrl_lookup_state(pinctrlaud_tfa98xx, aud_tfa98xx_gpios[i].name);
			if (IS_ERR(aud_tfa98xx_gpios[i].gpioctrl)) {
				ret = PTR_ERR(aud_tfa98xx_gpios[i].gpioctrl);
				pr_err("%s pinctrl_lookup_state %s fail %d\n", __func__, aud_tfa98xx_gpios[i].name, ret);
			} else {
				pinctrl_select_state(pinctrlaud_tfa98xx, aud_tfa98xx_gpios[i].gpioctrl);
				aud_tfa98xx_gpios[i].gpio_prepare = true;
			}
		}
	}
...
#ifdef CONFIG_MTK_I2C_EXTENSION
	if (count + 1 > TFA98XX_DMA_BUFFER_BYTE_COUNT) {
		pr_err("%s data longer than DMA buffer! count + 1 = %zd (dma buf %d)\n", __func__, (count  + 1), TFA98XX_DMA_BUFFER_BYTE_COUNT);
		return -ENOMEM;
	}

	tfa98xx->i2c->ext_flag = I2C_DMA_FLAG;
	tfa98xx->dma_buf[0] = tfa98xx->reg;
	memcpy(&tfa98xx->dma_buf[1], buf, count);
retry:
	ret = i2c_master_send(tfa98xx->i2c, (unsigned char *)(uintptr_t)tfa98xx->dma_buf_pa, count + 1);
	if (ret < 0) {
		pr_warn("i2c error, retries left: %d\n", retries);
		if (retries) {
			retries--;
...
retry:
	...
	if (ret < 0) {
		...
		return ret;
		 ...
		 if (msgs[1].ext_flag == I2C_DMA_FLAG)
			memcpy(buf, tfa98xx->dma_buf,  msgs[1].len);
	}
#endif
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (gpio_is_valid(tfa98xx->power_gpio)) {
		ret = devm_gpio_request_one(&i2c->dev, tfa98xx->power_gpio,
			GPIOF_OUT_INIT_HIGH, "TFA98XX_POWER");
		if (ret)
			goto err;
	}
...
#ifdef CONFIG_MTK_I2C_EXTENSION
	tfa98xx->i2c->ext_flag = 0;
	tfa98xx->i2c->timing = tfa98xx->i2c_speed;

	#ifdef CONFIG_64BIT
	tfa98xx->dma_buf =
		(char *)dma_alloc_coherent(&i2c->dev, TFA98XX_DMA_BUFFER_BYTE_COUNT,
					(dma_addr_t *) &tfa98xx->dma_buf_pa,
					GFP_KERNEL);
	#else
	tfa98xx->dma_buf =
		(char *)dma_alloc_coherent(NULL, TFA98XX_DMA_BUFFER_BYTE_COUNT,
					(dma_addr_t *) &tfa98xx->dma_buf_pa,
					GFP_KERNEL);
	#endif 
	if (tfa98xx->dma_buf == NULL) {
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	if (tfa98xx->dma_buf) {
		#ifdef CONFIG_64BIT
		dma_free_coherent(&i2c->dev, TFA98XX_DMA_BUFFER_BYTE_COUNT, tfa98xx->dma_buf,
				tfa98xx->dma_buf_pa);
		#else
		dma_free_coherent(NULL, TFA98XX_DMA_BUFFER_BYTE_COUNT, tfa98xx->dma_buf,
				tfa98xx->dma_buf_pa);
		#endif
		tfa98xx->dma_buf = NULL;
	}
#endif 
