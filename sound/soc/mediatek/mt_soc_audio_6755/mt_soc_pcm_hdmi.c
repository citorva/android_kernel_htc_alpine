...
#if 0 
	Afe_Set_Reg(AFE_TDM_CON2, 0, 0x00010000); 
#endif
	Afe_Set_Reg(AFE_I2S_CON, 0, 0x00000001); 
			  ...
			  struct snd_pcm_hw_params *hw_params)
{
	...
	kal_uint32 volatile u4tmpMrg1 = 0; 
...
static int mtk_pcm_hdmi_prepare(struct snd_pcm_substream *substream)
{
	...
	if (runtime->format == SNDRV_PCM_FORMAT_S32_LE ||
		...
		SetTDMLrckWidth(Tdm_Lrck);
		SetTDMbckcycle(Soc_Aud_I2S_WLEN_WLEN_32BITS);
		SetTDMChannelsSdata(runtime->channels); 
...
static int mtk_pcm_hdmi_start(struct snd_pcm_substream *substream)
{
	...
	Afe_Set_Reg(AUDIO_TOP_CON1, 1,  0x00000002);  
	Afe_Set_Reg(AUDIO_TOP_CON1, 0,  0x00000002);  

#if 0 
	Afe_Set_Reg(AFE_TDM_CON2, 0, 0x00060000); 
	Afe_Set_Reg(AFE_TDM_CON2, 1, 0x00010000); 
#endif
	Afe_Set_Reg(AUDIO_TOP_CON1, 1,  0x00000002);  
	Afe_Set_Reg(AFE_I2S_CON, 0 << 12, 0x00001000);
	Afe_Set_Reg(AFE_I2S_CON, 0 << 28, 0x10000000); 
	Afe_Set_Reg(AFE_I2S_CON, 0 << 31, 0x80000000); 
	Afe_Set_Reg(AUDIO_TOP_CON1, 1 << 4, 0x00000010); 
	Afe_Set_Reg(AUDIO_TOP_CON1, 1,  0x00000002);  

	Afe_Set_Reg(AFE_I2S_CON, 1 << 31,  0x80000000); 
	Afe_Set_Reg(AFE_I2S_CON, 0 << 12,  0x00001000); 
	Afe_Set_Reg(AFE_I2S_CON, Soc_Aud_I2S_WLEN_WLEN_16BITS << 1 ,
		    0x00000002); 
	Afe_Set_Reg(AFE_I2S_CON, 1 << 2 ,  0x00000004); 
	Afe_Set_Reg(AFE_I2S_CON, 1 << 3 ,  0x00000008); 

	

	SetSampleRate(Soc_Aud_Digital_Block_MEM_I2S,  runtime->rate);

	Afe_Set_Reg(AFE_I2S_CON, 1, 0x00000001); 
	Afe_Set_Reg(AUDIO_TOP_CON1, 0 << 4, 0x00000010); 
	Afe_Set_Reg(AUDIO_TOP_CON1, 0,  0x00000002);  
