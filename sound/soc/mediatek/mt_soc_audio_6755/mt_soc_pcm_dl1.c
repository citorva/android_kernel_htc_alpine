...
static int mtk_pcm_dl1_start(struct snd_pcm_substream *substream)
{
	...
	Ana_Set_Reg(AFE_DL_SRC2_CON0_L, 0x0001, 0xffffffff);	
	Ana_Set_Reg(PMIC_AFE_TOP_CON0, 0x0000, 0xffffffff);	
	Afe_Set_Reg(FPGA_CFG1, 0x1, 0xffff);	
...
static int mtk_soc_dl1_probe(struct platform_device *pdev)
{
	...
	if (ret < 0)
		pr_err("%s of_platform_populate failed! (err: %d)\n", __func__, ret);
...
static int i2s0_pinctrl_dev_probe(struct platform_device *pdev)
{
#ifdef CONFIG_OF
	AudDrv_GPIO_I2S0_probe(&pdev->dev);
#endif
	return 0;
}

static int i2s0_pinctrl_dev_remove(struct platform_device *pdev)
{
	return 0;
}

#ifdef CONFIG_OF
static const struct of_device_id i2s0_pinctrl_dt_match[] = {
	{ .compatible = "i2s0_pinctrl", },
	{ }
};
MODULE_DEVICE_TABLE(of, i2s0_pinctrl_dt_match);
#endif

static struct platform_driver i2s0_pinctrl_driver = {
	.probe  = i2s0_pinctrl_dev_probe,
	.remove = i2s0_pinctrl_dev_remove,
	.driver = {
		.name = "i2s0_pinctrl",
		.owner = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = i2s0_pinctrl_dt_match,
#endif
	},
};
...
static int __init mtk_soc_platform_init(void)
{
	...
	ret = platform_driver_register(&i2s0_pinctrl_driver);
	return ret;
...
static void __exit mtk_soc_platform_exit(void)
{
	...
	platform_driver_unregister(&i2s0_pinctrl_driver); 
