...
static int mtmachine_i2s0dl1_startup(struct snd_pcm_substream *substream)
{
	pr_debug("%s \n", __func__);
...
static void mtmachine_i2s0dl1_shutdown(struct snd_pcm_substream *substream)
{
	pr_debug("%s \n", __func__);
...
static struct snd_soc_ops mt_machine_audio_i2s0dl1_ops = {
	.startup = mtmachine_i2s0dl1_startup,
	.shutdown = mtmachine_i2s0dl1_shutdown,
};
...
static struct snd_soc_dai_link mt_soc_dai_common[] = {
	 ...
	 },
	{
	 ...
	 {
	 .name = "EXT_AMP",
	 .stream_name = "External_Amp",
	 .cpu_dai_name   = "MM_STUB",
	 .platform_name  = MT_SOC_DUMMY_PCM,
	 .codec_name = TFA_CODEC_NAME,
	 .codec_dai_name = TFA_CODEC_DAI_NAME,
	 .init = mt_soc_audio_init,
	 .ops = &mt_machine_audio_i2s0dl1_ops,
	 },
#endif
	{
		.name = "Voice_USB",
