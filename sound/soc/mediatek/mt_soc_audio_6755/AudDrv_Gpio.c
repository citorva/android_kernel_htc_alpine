...
struct pinctrl *pinctrlaud_i2s0; 
...
enum audio_system_gpio_i2s0_type {
	GPIO_I2S0_MODE0,
	GPIO_I2S0_MODE1,
	GPIO_I2S0_MODE_NUM
};

static struct audio_gpio_attr aud_i2s0_gpios[GPIO_I2S0_MODE_NUM] = {
	[GPIO_I2S0_MODE0] = {"audi2s0-mode0", false, NULL},
	[GPIO_I2S0_MODE1] = {"audi2s0-mode1", false, NULL},
};

void AudDrv_GPIO_I2S0_probe(void *dev) {
	int ret;
	int i = 0;

	pr_warn("%s\n", __func__);

	pinctrlaud_i2s0 = devm_pinctrl_get(dev);
	if (IS_ERR(pinctrlaud_i2s0)) {
		ret = PTR_ERR(pinctrlaud_i2s0);
		pr_err("Cannot find pinctrlaud_i2s0!\n");
		return;
	}

	for (i = 0; i < ARRAY_SIZE(aud_i2s0_gpios); i++) {
		aud_i2s0_gpios[i].gpioctrl = pinctrl_lookup_state(pinctrlaud_i2s0, aud_i2s0_gpios[i].name);
		if (IS_ERR(aud_i2s0_gpios[i].gpioctrl)) {
			ret = PTR_ERR(aud_i2s0_gpios[i].gpioctrl);
			pr_err("%s pinctrl_lookup_state %s fail %d\n", __func__, aud_i2s0_gpios[i].name,
			       ret);
		} else {
			aud_i2s0_gpios[i].gpio_prepare = true;
		}
	}
}
...
int AudDrv_GPIO_I2S0_Select(int bEnable)
{
	...
	pr_info("%s mode %d\n", __func__, bEnable);
	if (bEnable == 1) {
		if (aud_i2s0_gpios[GPIO_I2S0_MODE1].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud_i2s0, aud_i2s0_gpios[GPIO_I2S0_MODE1].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_I2S0_MODE1] pins\n");
		}
	} else {
		if (aud_i2s0_gpios[GPIO_I2S0_MODE0].gpio_prepare) {
			retval =
			    pinctrl_select_state(pinctrlaud_i2s0, aud_i2s0_gpios[GPIO_I2S0_MODE0].gpioctrl);
			if (retval)
				pr_err("could not set aud_gpios[GPIO_I2S0_MODE0] pins\n");
		}
