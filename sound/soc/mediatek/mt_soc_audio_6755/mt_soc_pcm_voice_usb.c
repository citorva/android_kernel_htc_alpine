...
static void set_echo_ref_path(int connect)
{
	...
	if (connect == Soc_Aud_InterCon_Connection) {
			 ...
			 (struct snd_pcm_substream *substream)
{
	...
	if (GetMemoryPathEnable(usb_mem_blk[stream]) == true) {
		...
		pr_usbp("%s(), DMAReadIdx 0x%x, åRemained 0x%x, Cur_ReadIdx 0x%x, memory_index 0x%x, consumed 0x%x\n",
			__func__, Afe_Block->u4DMAReadIdx,
