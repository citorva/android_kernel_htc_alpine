...
static int htc_debug_read(struct seq_file *m, void *v)
{
    ...
    if(first_read){
        pnum = get_partition_num_by_name("para");

        if (pnum < 0) {
            pr_err("[SECURITY] unknown partition number for para partition\n");
            return 0;
        }
        ...
        if (IS_ERR(filp)) {
            pr_err("[SECURITY] unable to open file: %s\n", filename);
            return PTR_ERR(filp);
        }
                        ...
                        size_t count, loff_t *ppos)
{
    ...
    if (count != sizeof(buf)){
        pr_err("[SECURITY]count != sizeof(buf)\n");
        return -EFAULT;
    }
...
static int __init sysinfo_proc_init(void)
{
    ...
    if (entry == NULL) {
        pr_err("%s:[SECURITY] unable to create /proc%s entry\n", __func__,PROCNAME);
        return -ENOMEM;
    }
